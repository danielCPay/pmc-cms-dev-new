...
class yetiforce extends rcube_plugin
{
		...
		if ('mail' == $this->rc->task) {
			...
			if ($this->loadCurrentUser()) {
				...
				if ('preview' === $this->rc->action || 'show' === $this->rc->action || '' == $this->rc->action) {
					...
					asort($composeAddressModules);
					$this->viewData['compose']['composeAddressModules'] = $composeAddressModules;

					$composeLabelModules = [];
					foreach (\App\Config::component('Mail', 'RC_COMPOSE_LABEL_MODULES') as $moduleName) {
						if (\App\Privilege::isPermitted($moduleName)) {
							$composeLabelModules[$moduleName] = \App\Language::translate($moduleName, $moduleName);
						}
					}
					asort($composeLabelModules);
					$this->viewData['compose']['composeLabelModules'] = $composeLabelModules;
					...
					$this->rc->output->add_handler('yetiforce.labelbutton', [$this, 'labelButton']);
					$this->rc->output->add_handler('yetiforce.chooserecord', [$this, 'chooseRecord']);
					$this->rc->output->add_handler('yetiforce.baserecord', [$this, 'baseRecord']);
					...
					if ($id = rcube_utils::get_input_value('_id', rcube_utils::INPUT_GPC)) {
						...
						if (isset($_SESSION['compose_data_' . $id]['param']['crmrecord'])) {
							...
							$label = \App\Record::getLabel($_SESSION['compose_data_' . $id]['param']['crmrecord']);
							$this->rc->output->set_env('yf_crmRecordLabel', $label);
						}
	...
	public function authenticate($args): array
	{
		if (!empty($_GET['_autologin']) && ($row = $this->getAutoLogin())) {
			$host = false;
			if (is_string($this->rc->config->get('default_host'))) {
			...
			if ($host) {
				...
				if ($this->rc->config->get('oauth_provider')) {
					...
					$args['pass'] = $this->rc->decrypt($_SESSION['password']);

					$this->rc->config->set('login_password_maxlen', strlen($args['pass']) * 2);
				}
			}
			$db = $this->rc->get_dbh();
	...
	public function login_after($args): array
	{
		...
		if (empty($pass) && array_key_exists('token', $args)) {
			$pass = $args['token']['refresh_token'];
		}
	 ...
	 * @param array $args
	 */
	public function oauth_refresh_token($args): array
	{
		$pass = $args['refresh_token'] ? $this->rc->decrypt($args['refresh_token']) : null;

		if (!empty($pass)) {
			$sql = 'UPDATE ' . $this->rc->db->table_name('users') . ' SET password = ? WHERE user_id = ?';
			$userId = $this->rc->get_user_id();
			if (!$userId) {
				$host = rcube_utils::parse_host($this->rc->config->get('default_host'));
				$host = parse_url($host);
				$host = $host['host'];
				$user = rcube_user::query(strtolower($args['username'] ?: $args['identity']['email']), $host);
				$userId = $user->ID;
			}
			$currentPath = getcwd();
			chdir($this->rc->config->get('root_directory'));
			...
			$pass = \App\Encryption::getInstance()->encrypt($pass);
			chdir($currentPath);
			\call_user_func_array([$this->rc->db, 'query'], array_merge([$sql], [$pass, $userId]));
		}
		return $args;
	}
	...
	public function getContentEmailTemplate(): void
	{
		...
		if ($this->loadCurrentUser() && \App\Privilege::isPermitted('EmailTemplates', 'DetailView', $templateId)) {
			$template = \Vtiger_Record_Model::getInstanceById($templateId, 'EmailTemplates');
			$mail = \App\Mail::getTemplate($templateId);
			if (($recordId = rcube_utils::get_input_value('record_id', rcube_utils::INPUT_GPC)) && \App\Record::isExists($recordId)) {
				$recordModel = \Vtiger_Record_Model::getInstanceById(
				...
				// add attachments from By Document Type and By Email Status
				// $mail['attachments'] = [ 'ids' => [] ];
				...
				$mail['attachments'] = ['ids' => array_merge($ids, $mail['attachments']['ids'])];
			} else {
			...
			if ($mail['email_to']) {
				$toModel = Vtiger_Record_Model::getInstanceById($mail['email_to']);
				$to = $textParser->setContent($toModel->get('result_text'))->parse()->getContent(true);
				$mail['to'] = $to;
			}
			if ($mail['email_cc']) {
				$ccModel = Vtiger_Record_Model::getInstanceById($mail['email_cc']);
				$cc = $textParser->setContent($ccModel->get('result_text'))->parse()->getContent(true);
				$mail['cc'] = $cc;
			}
		}
	...
	/**
	 * `yetiforce.labelbutton`  handler.
	 *
	 * @param array $args
	 *
	 * @return string
	 */
	public function labelButton(array $args): string
	{
		if (empty($this->viewData['compose']['composeLabelModules'])) {
			return '';
		}
		$content = '';
		foreach ($this->viewData['compose']['composeLabelModules'] as $moduleName => $value) {
			$text = html::span(['class' => "yfm-$moduleName"], '') . " <span class=\"inner\">$value</span>";
			$content .= html::a([
				'class' => 'btn btn-sm btn-outline-dark mr-1 mt-1',
				'href' => '#',
				...
				'data-module' => $moduleName,
				'title' => $value,
				'onclick' => "return rcmail.command('yetiforce.selectLabel','$moduleName','{$args['part']}')",
			], $text);
		}
		return $content;
	}
	...
	/**
	 * identity_create hook handler.
	 * Add auto login for current CRM user
	 *
	 * @param array $args
	 */
	public function identity_create($args): array
	{
		$id = intval($args['record']['user_id']);
		if (!empty($_SESSION['crm']['id'])) {
			$crmId = $_SESSION['crm']['id'];
		} else if (!empty($_COOKIE['YTSID']) && \App\Session::load()) {
			$sessionData = \App\Session::getById($_COOKIE['YTSID']);
			$crmId = $sessionData['authenticated_user_id'] ?? '';
		}
		
		// insert into roundcube_users_autologin
		$sql = 'INSERT INTO ' . $this->rc->db->table_name('users_autologin') . ' (`rcuser_id`, `crmuser_id`, `active`) values (?, ?, 0)';
		\call_user_func_array([$this->rc->db, 'query'], array_merge([$sql], [$id, $crmId]));

		return $args;
	}

	/**
	 * `yetiforce.baseRecord` handler.
	 *
	 * @param array $args
	 *
	 * @return string
	 */
	public function baseRecord(array $args): string
	{
		$content = '';
		...
		if (\App\Record::isExists($recordId)) {
			...
			$recordUrl = $record->getDetailViewUrl();
			$recordLabel = $record->getDisplayName();
			...
			$content .= html::a([
				...
				. html::span(['style' => "color: #d4dbde", 'id' => 'baseRecordEmpty',], 'No record selected');
			$content .= html::tag('input', [
				'type' => 'hidden',
				'id' => 'baseRecordId',
				'value' => -1,
	...
	/**
	 * `yetiforce.chooseRecord` handler.
	 *
	 * @param array $args
	 *
	 * @return string
	 */
	public function chooseRecord(array $args): string
	{
		$content = '';
		$modules = [];
		...
		$moduleNames = \App\Mail::getTemplateList('', '', true);
		$moduleNames = array_unique(array_column($moduleNames, 'module_name'));
		$moduleFilter = \App\Config::component('Mail', 'RC_COMPOSE_RECORD_MODULES_FILTER', []);
		foreach ($moduleNames as $moduleName) {
			if (\in_array($moduleName, $moduleFilter)) {
				continue;
			}
			$modules[$moduleName] = \App\Language::translate($moduleName, $moduleName);
		}
		asort($modules);
		...
		foreach ($modules as $moduleName => $value) {
			...
			$content .= html::a([
				...
				'data-module' => $moduleName,
				'title' => $value,
				'onclick' => "return rcmail.command('yetiforce.chooseRecord','$moduleName')",
			], $text);
		}
		return $content;
	}
