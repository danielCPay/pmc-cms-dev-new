...
class rcmail_oauth
{
    ...
    public function get_redirect_uri()
    {
        ...
        return 'https://' . $_SERVER['HTTP_HOST'] . $_SERVER['DOCUMENT_URI'] . '/login/oauth';
    }
        ...
        if (!empty($oauth_token_uri) && !empty($oauth_client_secret)) {
            try {
                ...
                if (!empty($data['access_token'])) {
                    ...
                    $this->rcmail->plugins->exec_hook('oauth_refresh_token', array_merge($data, [
                        'username' => $username,
                        'identity' => $identity,
                    ]));
                ...
                $this->last_error = "OAuth token request failed: " . $e->getMessage() . "/" . var_export($data, true);
