...
class DateTime
{
	...
	public static function formatToViewDate($dateTime)
	{
		switch (\App\User::getCurrentUserModel()->getDetail('view_date_format')) {
			...
			case 'PLL_COMBINED':
				return '<span>' . \Vtiger_Util_Helper::formatDateDiffInStrings($dateTime) . ', ' . static::formatToDay($dateTime, false, true) . '</span>';
			default:
				break;
		}
		return '-';
	 ...
	 * @return string
	 */
	public static function formatToDay($dateTime, $allday = false, $withSeconds = false)
	{
		...
		if (!$allday) {
			...
			$displayTime = $hours . ':' . $minutes . ($seconds !== '' ? ':' . $seconds : '');
