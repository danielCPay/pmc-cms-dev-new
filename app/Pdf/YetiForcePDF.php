...
class YetiForcePDF extends PDF
{
	...
	public function output($fileName = '', $dest = '')
	{
		...
		if ('I' !== $dest && 'D' !== $dest) {
			...
			\App\Log::warning("Before conversion");
			\App\Utils::process("pdf2ps $fileName $fileName.ps");
			\App\Log::warning("conversion 1");
			\App\Utils::process("ps2pdf $fileName.ps $fileName");
			\App\Log::warning("conversion 2");
