...
class QueryGenerator
{
	...
	public function loadRelatedFields()
	{
		...
		foreach ($this->relatedFields as $field) {
			[$sourceField,] = explode(':', $field['sourceField'], 2);
			$joinTableName = $this->getModuleField($sourceField)->getTableName();
			...
			$lastSourceField = last(explode(':', $sourceField));
			$relatedFieldModel = $this->addRelatedJoin($field);
			$fields[str_replace(':', '_', $sourceField) . "{$field['relatedModule']}{$relatedFieldModel->getName()}"] = "{$relatedFieldModel->getTableName()}{$lastSourceField}.{$relatedFieldModel->getColumnName()}";
			// HACK check for empty field name, may occurr when related field is not accessible
			if (!isset($checkIds[$sourceField][$field['relatedModule']]) && \Vtiger_CRMEntity::getInstance($field['relatedModule'])->tab_name_index[$relatedFieldModel->getTableName()]) {
				$checkIds[$sourceField][$field['relatedModule']] = $field['relatedModule'];
				$fields[str_replace(':', '_', $sourceField) . "{$field['relatedModule']}id"] = $relatedFieldModel->getTableName() . $lastSourceField . '.' . \Vtiger_CRMEntity::getInstance($field['relatedModule'])->tab_name_index[$relatedFieldModel->getTableName()];
			}
		}
		return $fields;
	}
	...
	private function parseConditions(?array $conditions): array
	{
		...
		foreach ($conditions['rules'] as $rule) {
			if (isset($rule['condition'])) {
				...
				[$fieldName, $moduleName, $sourceFieldName] = array_pad(explode(':', $rule['fieldname'], 3), 3, false);
				if (!empty($sourceFieldName)) {
	...
	private function getCondition(string $fieldName, $value, string $operator, bool $userFormat = false)
	{
		if ($fieldName == 'special-current-role') {
			...
			if (!empty($_REQUEST) && $user->isAdmin()) {
				...
				$result = false !== strpos(\Settings_Roles_Record_Model::getInstanceById($value)->get('parentrole'), $user->getRoleInstance()->get('parentrole'));
			} else {
				$result = false === strpos($user->getRoleInstance()->get('parentrole'), \Settings_Roles_Record_Model::getInstanceById($value)->get('parentrole'));
			}
			...
			$queryField->setValue($this->processValue($value));
	...
	private function getRelatedCondition(array $condition)
	{
		...
		$queryField->setValue($this->processValue($condition['value']));
		$queryField->setOperator($condition['operator']);
		return $queryField->getCondition();
	}

	/** 
	 * Processes condition value for related field.
	 */
	private function processValue($rawValue) {
		if (\is_string($rawValue)) {
			[$valueFieldName, $valueModuleName, $valueSourceFieldName] = array_pad(explode(':', $rawValue), 3, false);
		}
		if (!empty($valueSourceFieldName) && preg_match('/\\d{2}:\\d{2}/', $rawValue) === 0) {
			// add join and use related field name as $value
			$relatedFieldModel = $this->getRelatedModuleField($valueFieldName, $valueModuleName);
			if (!$relatedFieldModel || !$relatedFieldModel->isActiveField()) {
				Log::warning("Field in related module is inactive or does not exist. Related module: {$valueModuleName} | Related field: {$valueFieldName}");
			}
			$tableName = $relatedFieldModel->getTableName();
			$sourceFieldModel = $this->getModuleField($valueSourceFieldName);
			$relatedTableName = $tableName . $valueSourceFieldName;
			...
			$value = "$relatedTableName.{$relatedFieldModel->getColumnName()}";
		} else {
			$value = $rawValue;
		}
	...
	public function addCondition($fieldName, $value, $operator, $groupAnd = true, $userFormat = false)
	{
		...
		if ($condition) {
			...
			Log::error('Wrong condition: ' . $fieldName . '/' . $operator . '/' . $value);
	...
	public function addRelatedJoin($fieldDetail)
	{
		...
		$module = \Vtiger_Module_Model::getInstance($this->getModule());
		// shortcut and workaround for related fields with multiple relations on first level
		if (strpos($fieldDetail['sourceField'], ':') === false && in_array($fieldDetail['relatedModule'], $module->getField($fieldDetail['sourceField'])->getReferenceList())) {
			$sourceFieldModel = $this->getModuleField($fieldDetail['sourceField']);

			// add join
			$tableName = $relatedFieldModel->getTableName();
			$relatedTableName = $tableName . $sourceFieldModel->getName();
			$relatedTableIndex = $relatedFieldModel->getModule()->getEntityInstance()->tab_name_index[$tableName];
			$this->addJoin(['LEFT JOIN', "$tableName $relatedTableName", "{$sourceFieldModel->getTableName()}.{$sourceFieldModel->getColumnName()} = $relatedTableName.$relatedTableIndex"]);
		...
		$remainingSourceField = "{$fieldDetail['sourceField']}:{$fieldDetail['relatedField']}";
		
		$previousSourceField = "";
		[$sourceField, $relatedField, $remainingSourceField] = explode(':', $remainingSourceField, 3);
		while (!empty($relatedField)) {
			$sourceFieldModel = $module->getField($sourceField);
			// HACK tries to match proper relation; doesn't work well if there is more than one match - should try to match all and add several conditions
			foreach ($sourceFieldModel->getReferenceList() as $reference) {
				$relatedModule = \Vtiger_Module_Model::getInstance($reference);
				$relatedFieldModel = $relatedModule->getField($relatedField);
			...
			// add join
			$tableName = $relatedFieldModel->getTableName();
			$relatedTableName = $tableName . $sourceField;
			$relatedTableIndex = $relatedModule->getEntityInstance()->tab_name_index[$tableName];
			$this->addJoin(['LEFT JOIN', "$tableName $relatedTableName", "{$sourceFieldModel->getTableName()}$previousSourceField.{$sourceFieldModel->getColumnName()} = $relatedTableName.$relatedTableIndex"]);

			$module = $relatedModule;
			$previousSourceField = $sourceField;
			[$sourceField, $relatedField, $remainingSourceField] = explode(':', "$relatedField:$remainingSourceField", 3);
		}
	...
	public function getQueryRelatedField($relatedInfo, ?\Vtiger_Field_Model $field = null)
	{
		if (!\is_array($relatedInfo)) {
			[$fieldName, $relatedModule, $sourceFieldName] = array_pad(explode(':', $relatedInfo, 3), 3, false);
			$relatedInfo = [
				'sourceField' => $sourceFieldName,
			...
			if (!empty($groupInfo)) {
				...
				foreach ($groupInfo as $fieldSearchInfo) {
					if ($fieldSearchInfo) {
						...
						[$fieldName, $moduleName, $sourceFieldName] = array_pad(explode(':', $fieldNameInfo, 3), 3, false);
						if (!empty($sourceFieldName)) {
	...
	public function parseSearchParams(array $searchParams): array
	{
		...
		foreach ($searchParams as $key => $conditions) {
			...
			foreach ($conditions as $condition) {
				[$fieldName, , $sourceFieldName] = array_pad(explode(':', $condition[0], 3), 3, false);
				if (!$sourceFieldName) {
