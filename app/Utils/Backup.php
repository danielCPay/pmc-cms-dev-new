	...
	public static function readCatalog(string $catalogToRead)
	{
		...
		foreach ((new \DirectoryIterator($catalogPath)) as $element) {
			if ($element->isDot()) {
				...
				if ($element->isDir() && substr($element->getBaseName(), 0, 1) !== ".") {
					if ($element->isReadable()) {
						$record['url'] = "{$requestUrl}&catalog={$urlDirectory}{$record['name']}";
					}
					$returnStructure['catalogs'][] = $record;
				} else {
					// 20210225 Michał Kamiński List all files
					// if (!$element->isReadable() || !\in_array($element->getExtension(), $allowedExtensions)) {
					if (!$element->isReadable() || $element->getBaseName() == '.placeholder' || substr($element->getBaseName(), 0, 4) === ".git" ) {
						continue;
					}
		...
		if (!empty($returnStructure['catalogs'])) {
				asort($returnStructure['catalogs']);
		}
		if (!empty($returnStructure['files'])) {
				asort($returnStructure['files']);
		}
