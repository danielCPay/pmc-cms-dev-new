...
class Record
{
	...
	public static function updateLabel(string $moduleName, int $id, bool $insertMode = false, ?string $updater = null): void
	{
		...
		if (!$metaInfo) {
			...
			if ($insertMode && $updater !== 'searchlabel') {
				$dbCommand->delete('u_#__crmentity_label', ['crmid' => $id])->execute();
			}
			if ($insertMode && $updater !== 'label') {
				$dbCommand->delete('u_#__crmentity_search_label', ['crmid' => $id])->execute();
			}
			...
			if (!$insertMode) {
				...
				if (!empty($search) && !($searchRowCount = $dbCommand->update('u_#__crmentity_search_label', ['searchlabel' => $search], ['crmid' => $id])->execute())) {
			...
			if (($insertMode || !$searchRowCount) && 'label' !== $updater && !empty($search)) {
	 ...
	 * @return void
	 */
	public static function updateLabelOnSave(\Vtiger_Record_Model $recordModel, $retry = 0): void
	{
		try {
			$label = '';
			...
			if (!$metaInfo || (empty($metaInfo['fieldnameArr']) && empty($metaInfo['searchcolumnArr']))) {
				...
				if ($recordModel->isNew()) {
					...
					$dbCommand->update('u_#__crmentity_label', ['label' => $label], ['and', ['crmid' => $recordModel->getId()], ['!=', 'label', $label]])->execute();
					if (!empty($search)) {
						$dbCommand->update('u_#__crmentity_search_label', ['searchlabel' => $search], ['and', ['crmid' => $recordModel->getId()], ['!=', 'searchlabel', $search]])->execute();
					}
				}
			}
			...
			if ($exception->errorInfo[1] == 1213 && $retry < 3) {
				// retry transaction
				++$retry;
				usleep(100000 * $retry);
				static::updateLabelOnSave($recordModel, $retry);
			} else {
				throw $exception;
			}
		}
