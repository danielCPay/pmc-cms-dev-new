...
class Mail
{
	...
	public static function getTemplateList(string $moduleName = '', string $type = '', bool $hideSystem = true, ?int $userId = null)
	{
		...
		$queryGenerator->setFields(['id', 'name', 'module_name', 'number']);
		if ($moduleName) {
	 ...
	 * @return int|null
	 */
	public static function getTemplateIdFromName(string $name): ?int {
		$cacheName = 'TemplateIdFromName';
		if (Cache::has($cacheName, '')) {
			$templates = Cache::get($cacheName, '');
		} else {
			$queryGenerator = new \App\QueryGenerator('EmailTemplates');
			$queryGenerator->setFields(['id']);
			$queryGenerator->permissions = false;
			$templates = $queryGenerator->createQuery()->select(['name', 'emailtemplatesid'])->createCommand()->queryAllByGroup();
			Cache::save($cacheName, '', $templates, Cache::LONG);
		}
		return $templates[$name] ?? null;
	}
