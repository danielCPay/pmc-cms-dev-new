...
class PrivilegeUtil
{
	...
	/**
	 * Function to get the vtiger_role related user ids, but only active.
	 *
	 * @param int $roleId RoleId :: Type varchar
	 *
	 * @return array $users -- Role Related User Array in the following format:
	 */
	public static function getActiveUsersByRole($roleId)
	{
		$users = (new \App\Db\Query())->select(['userid'])->from('vtiger_user2role')->innerJoin('vtiger_users', 'vtiger_users.id = vtiger_user2role.userid and vtiger_users.status = \'Active\'')->where(['roleid' => $roleId])->column();
		$users = array_map('intval', $users);
		return $users;
	}
