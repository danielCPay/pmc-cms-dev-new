...
class CustomView
{
	...
	private function getColumnsByCvidFromDb($cvId)
	{
		...
		if (is_numeric($cvId)) {
			...
			while ($row = $dataReader->read()) {
				if (!empty($row['source_field_name']) && !static::isSourceFieldActive($row['source_field_name'], $this->getModule())) {
					continue;
				}
				// special handling for Pre-Suit role
				$userModel = \App\User::getCurrentUserModel();
				switch ($userModel->getRole()) {
					case \Cases_ListView_Model::PRE_SUIT_ROLE:
						if ($row['field_name'] === 'purchase_price') {
							continue 2;
						}
						break;
				}
	...
	public static function getConditions($id): array
	{
		...
		while ($condition = $dataReader->read()) {
			...
			$fieldName = "{$condition['field_name']}" . ($condition['module_name'] ? ":{$condition['module_name']}" : '') . ($condition['source_field_name'] ? ':' . $condition['source_field_name'] : '');
	...
	/**
	 * Function to get possibly multi-level source field.
	 * 
	 * @param string $sourceField
	 * @param \Vtiger_Module_Model $module
	 * @return bool
	 * 
	 * @throws \Exception Throws exception if one of the source fields is not a reference field
	 */
	public static function isSourceFieldActive($sourceField, \Vtiger_Module_Model $module) {
		// split string separated by ':' into two strings, one containing part before first ':' and another with the remaining part
		// if there is no ':' in the string, the second string will be empty
		[$sourceField, $remainingSourceField] = explode(':', $sourceField, 2);

		$field = $module->getFieldByName($sourceField);
		if (!$field) {
			throw new \Exception("Field {$module->getName()}.$sourceField does not exist.");
		}
		if (!$field->isReferenceField() && $remainingSourceField) {
			throw new \Exception("Field {$module->getName()}.$sourceField is not a reference field");
		}
		if (!$field->isActiveField() && $remainingSourceField) {
			throw new \Exception("Field {$module->getName()}.$sourceField is not active");
		}
		...
		if ($remainingSourceField) {
			foreach ($field->getReferenceList() as $referenceModule) {
				...
				if (static::isSourceFieldActive($remainingSourceField, $module)) {
					return true;
				}
			}
		} else {
