...
class BaseField
{
	...
	public function getTableName()
	{
		...
		if ($this->related) {
			$sourceField = last(explode(':', $this->related['sourceField']));
			$table .= $sourceField;
		}
	...
	public function operatorMlti()
	{
		...
		foreach ($values as $value) {
			$value = trim($value);
			$conditions[] = ['like', $this->getColumnName(), "%{$value}%", false];
		}
		return $conditions;
	}
	...
	public function operatorSmlr()
	{
		...
		foreach ($values as $value) {
			foreach (explode(' ', $value) as $value) {
				$value = trim($value);
				$conditions[] = $this->getColumnName() . ' like ' . \App\Db::getInstance()->quoteValue('%' . $value . '%') . ' collate utf8_unicode_520_ci';
			}
		}
		return $conditions;
	}
	...
	public function getOperator()
	{
		...
		if ('a' === $this->operator) {
			...
			if (false !== strpos($this->getValue(), '*')) {
				$operator = 'wca';
			} else if (false !== strpos($this->getValue(), '\'')) {
				$operator = 'smlr';
			} else if (false !== strpos($this->getValue(), ' ')) {
				$operator = 'mlti';
			}
