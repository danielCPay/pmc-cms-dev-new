...
class TextParser
{
	 ...
	 * @var string[]
	 */
	protected static $baseFunctions = ['general', 'translate', 'record', 'recordRaw', 'relatedRecord', 'relatedRecordRaw', 'relatedRecordLevel', 'sourceRecord', 'organization', 'employee', 'params', 'custom', 'relatedRecordsList', 'recordsList', 'date', 'inventory', 'userVariable', 'barcode', 'relatedRecordId', 'relatedRecordDeep', 'coalesce'];
	...
	/**
	 * Is the parsing text content XML?
	 *
	 * @var bool
	 */
	public $isXml = false;
	...
	/**
	 * Use raw versions of function
	 * 
	 * $var bool
	 */
	public $useRawPlaceholders = false;
	...
	{
		$content = \App\Utils\Completions::processPlaceholders($content, $this->useRawPlaceholders, $this->recordModel ? $this->recordModel->getModuleName() : $this->moduleName);
	...
	public function date($param)
	{
		...
		return (new \DateTimeField($date))->getDisplayDate();
	}
	...
	{
		if (!isset($this->recordModel) || ($this->permissions && $isPermitted && !Privilege::isPermitted($this->moduleName, 'DetailView', $this->record))) {
			return '';
		}
		...
		if ($this->recordModel->has($key)) {
			...
			return $this->isXml ? htmlspecialchars($this->getDisplayValueByField($fieldModel, false, $params), ENT_XML1) : $this->getDisplayValueByField($fieldModel, false, $params);
		}
		switch ($key) {
			...
			case 'RecordId':
				return $this->record;
			case 'RecordLabel':
				return $this->isXml ? htmlspecialchars($this->recordModel->getName(), ENT_XML1) : $this->recordModel->getName();
	...
	/**
	 * Parsing record data, returning raw values.
	 *
	 * @param string $params
	 * @param mixed  $isPermitted
	 *
	 * @return string
	 */
	protected function recordRaw($params, $isPermitted = true)
	{
		if (!isset($this->recordModel) || ($this->permissions && $isPermitted && !Privilege::isPermitted($this->moduleName, 'DetailView', $this->record))) {
			return '';
		}
		...
		if ($this->recordModel->has($key)) {
			...
			$result = $this->isXml ? htmlspecialchars($this->recordModel->get($key), ENT_XML1) : $this->recordModel->get($key);

			$paramsParsed = $params ? \App\TextParser::parseFieldParam($params) : [];
			if (isset($paramsParsed['uppercase']) && ((bool) $paramsParsed['uppercase']) ) {
	 ...
	 * @return mixed
	 */
	protected function relatedRecord($params, bool $raw = false)
	{
		[$fieldName, $relatedField, $relatedModule, $params] = array_pad(explode('|', $params, 4), 4, '');
		if (
		...
		$instance = static::getInstanceByModel($relatedRecordModel);
		foreach (['withoutTranslations', 'language', 'emailoptout', 'isHtml', 'isXml'] as $key) {
			if (isset($this->{$key})) {
				$instance->{$key} = $this->{$key};
			}
		}
		return $raw ? $instance->recordRaw($relatedField) : $instance->record("$relatedField|$params");
	}

	/**
	 * Parsing related record data returning raw value.
	 *
	 * @param string $params
	 *
	 * @return mixed
	 */
	protected function relatedRecordRaw($params)
	{
		return $this->relatedRecord($params, true);
	}
	 ...
	 * @return mixed
	 */
	protected function relatedRecordId($params)
	{
		...
		return $relatedRecordModel->get($relatedField);
	}
	...
	/**
	 * Parsing related record data - recursive.
	 *
	 * @param string $params
	 *
	 * @return mixed
	 */
	protected function relatedRecordDeep($params)
	{
		[$fieldName, $relatedModule, $relatedRecord] = array_pad(explode('|', $params, 3), 3, '');
		if (
			!isset($this->recordModel)
		...
		if (substr_count($relatedRecord, '|') > 2) {
			return $instance->relatedRecordDeep($relatedRecord);
		} else {
			return $instance->relatedRecord($relatedRecord);
		}
	}
	...
	protected function getDisplayValueByField(\Vtiger_Field_Model $fieldModel, $value = false, $params = null)
	{
		...
		$result = '';
		if ($this->withoutTranslations) {
			$result = $this->getDisplayValueByType($value, $model, $fieldModel, $params);
		}
		$result = $fieldModel->getUITypeModel()->getTextParserDisplayValue($value, $model, $params);

		$paramsParsed = $params ? \App\TextParser::parseFieldParam($params) : [];
		if (isset($paramsParsed['uppercase']) && $paramsParsed['uppercase'] !== 'false' ) {
			$result = strtoupper($result);
		}
		if (isset($paramsParsed['substring'])) {
			[$start, $end] = explode(':', $paramsParsed['substring'], 2);
			$result = mb_substr($result, is_numeric($start) ? $start : 0, is_numeric($end) ? $end : null);
		}
	...
	public function getRecordVariable($fieldType = false)
	{
		...
		foreach ($moduleModel->getBlocks() as $blockModel) {
			foreach ($blockModel->getFields() as $fieldModel) {
				if ($fieldModel->isViewable() && !($fieldType && $fieldModel->getFieldDataType() !== $fieldType) && $fieldModel->getFieldDataType() !== 'ViewRelatedField') {
				...
				foreach (\Vtiger_Module_Model::getInstance($relatedModule)->getBlocks() as $blockModel) {
					foreach ($blockModel->getFields() as $fieldName => $fieldModel) {
						if (
							...
							&& $fieldModel->getFieldDataType() !== 'ViewRelatedField'
						) {
	...
	/**
	 * Special function to coalesce several variables. 
	 * Accepts comma-separated list of TextParser variables in double quotes and executes them one by one 
	 * until it finds one that returns some value.
	 */
	public function coalesce(string $param) {
		...
		do {
			...
			$variable = trim(trim($variable), '"');
			$variable = $this->parseData("\$({$variable})\$", $this->record);
			if ($variable) {
				return $variable;
			}
