...
class Purifier
{
	...
	public static function purifyByType($input, $type, $convert = false)
	{
		if (\is_array($input)) {
			...
			if (property_exists('App\Validator', $type)) {
				...
				switch ($type) {
					...
					case 'Number':
						...
						if ((is_numeric($dbFormat) && Fields\Double::formatToDisplay($dbFormat, false) === Fields\Double::truncateZeros($input)) || $input == 0) {
							$value = $input;
						}
						break;
					case 'Double':
			...
			if (null === $value) {
				\App\Log::error('purifyByType: ' . $input . '/' . $type, 'IllegalValue');
				throw new \App\Exceptions\IllegalValue('ERR_NOT_ALLOWED_VALUE||' . $input, 406);
			}
		}
		return $value;
