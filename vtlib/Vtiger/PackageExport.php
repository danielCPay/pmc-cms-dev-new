...
class PackageExport
{
	...
	/** @var \vtlib\ModuleBasic|bool $moduleInstance */
	 ...
	 */
	public function export(Module $moduleInstance, $todir = '', $zipFileName = '', $directDownload = false, $isBatch = false)
	{
	...
	public function __copyLanguageFiles(\App\Zip $zip, $module)
	{
		...
		if (file_exists('custom/languages')) {
			foreach (new \RecursiveIteratorIterator(new \RecursiveDirectoryIterator('custom/languages', \RecursiveDirectoryIterator::SKIP_DOTS), \RecursiveIteratorIterator::SELF_FIRST) as $item) {
				if ($item->isFile() && $item->getFilename() === $module . '.json') {
					$zip->addFile($item->getPath() . \DIRECTORY_SEPARATOR . $item->getFilename());
				}
			}
	 ...
	 */
	public function exportModule($isBatch = false)
	{
		...
		if (!$isBatch) {
			// 20210226 Michał Kamiński no export time node to allow easier change detection
		...
		if (!$isBatch) {
			// 20210309 Michał Kamiński don't export version info to allow easier change detection
