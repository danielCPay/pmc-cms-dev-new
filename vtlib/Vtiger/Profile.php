...
class Profile
{
	...
	public static function initForModule(ModuleBasic $moduleInstance)
	{
		...
		$roProfileId = -1;
		foreach (\Settings_GlobalPermission_Record_Model::getGlobalPermissions() as $profileId => $profileData) {
			if ($profileData['profilename'] === 'Provider\'s Litigation Assistance') {
				$roProfileId = $profileId;
			}
		}
		foreach ($profileids as &$profileid) {
			$permissions = $roProfileId == $profileid ? 1 : 0;
