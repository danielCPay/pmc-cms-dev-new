...
Vtiger_Edit_Js('Users_Edit_Js',{/**
		 ...
		 */registerChangeEventForCurrencySeparator:function registerChangeEventForCurrencySeparator(){var form=jQuery('form');jQuery('[name="currency_decimal_separator"]',form).on('change',function(e){var element=jQuery(e.currentTarget),selectedValue=element.val(),groupingSeparatorValue=jQuery('[name="currency_grouping_separator"]',form).data('selectedValue');if(groupingSeparatorValue==selectedValue){var message=app.vtranslate('JS_DECIMAL_SEPARATOR_AND_GROUPING_SEPARATOR_CANT_BE_SAME');Vtiger_Helper_Js.showMessage({text:message,type:'error'});var previousSelectedValue=element.data('selectedValue');element.find('option').removeAttr('selected'),element.find('option[value="'+previousSelectedValue+'"]').attr('selected','selected'),element.trigger('change');}else element.data('selectedValue',selectedValue);}),jQuery('[name="currency_grouping_separator"]',form).on('change',function(e){var element=jQuery(e.currentTarget),selectedValue=element.val(),decimalSeparatorValue=jQuery('[name="currency_decimal_separator"]',form).data('selectedValue');if(decimalSeparatorValue==selectedValue){var message=app.vtranslate('JS_DECIMAL_SEPARATOR_AND_GROUPING_SEPARATOR_CANT_BE_SAME');Vtiger_Helper_Js.showMessage({text:message,type:'error'});var previousSelectedValue=element.data('selectedValue');element.find('option').removeAttr('selected'),element.find('option[value="'+previousSelectedValue+'"]').attr('selected','selected'),element.trigger('change');}else element.data('selectedValue',selectedValue);});},/**
		 * Function to register change event for login method
		 */registerChangeEventForLoginMethod:function registerChangeEventForLoginMethod(){var form=jQuery('form'),loginMethodElement=jQuery('[name="login_method"]',form),passwordValidationSwitch=function(){var parents=jQuery('div[data-field="user_password"],div[data-field="confirm_password"]',form);jQuery('span.redColor',parents).remove(),'PLL_LDAP'===loginMethodElement.val()?jQuery('[name="user_password"],[name="confirm_password"]',form).addClass('ignoreMe').validationEngine('hide'):(parents.find('label.fieldLabel').prepend('<span class="redColor">*</span>'),jQuery('[name="user_password"],[name="confirm_password"]',form).removeClass('ignoreMe'));};// disable validation for password fields if necessary
passwordValidationSwitch(loginMethodElement.val()),loginMethodElement.on('change',function(e){var element=jQuery(e.currentTarget),selectedValue=element.val();passwordValidationSwitch();});}},{duplicateCheckCache:{},userExistCheckCache:{},passCheckCache:{},//Hold the conditions for a hour format
