...
Settings_Vtiger_Edit_Js(
	...
	{},
	{
		...
		registerOauthRequest() {
			const container = this.getForm();
			const button = container.find('.js-oauth-request');
			button.on('mousedown', function (e) {
				const username = container.find('[name="username"]').val();
				const domain = username.split('@')[1];
				if (!domain) {
					var params = {};
					params['text'] = app.vtranslate('JS_ENTER_VALID_EMAIL_ADDRESS');
					Settings_Vtiger_Index_Js.showMessage(params);
				} else {
					// open new window navigating to gmailCallback.php?domain=[domain]
					window.open(
						'gmailCallback.php?domain=' + domain,
						'Gmail',
