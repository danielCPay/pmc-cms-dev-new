...
$.Class('Settings_LayoutEditor_Js',{},{updatedBlockSequence:{},inActiveFieldsList:!1,updatedBlockFieldsList:[],updatedBlocksList:[],blockNamesList:[],/**
		 * Function to set the inactive fields list used to show the inactive fields
		 */setInactiveFieldsList:function setInactiveFieldsList(){var thisInstance=this,contents=$('#layoutEditorContainer').find('.contents'),json=contents.find('.inActiveFieldsArray');0<json.length&&(thisInstance.inActiveFieldsList=JSON.parse(json.val()));},/**
		 * Function to regiser the event to make the blocks sortable
		 */makeBlocksListSortable:function makeBlocksListSortable(){var thisInstance=this,contents=$('#layoutEditorContainer').find('.contents'),table=contents.find('.blockSortable');contents.sortable({containment:contents,items:table,revert:!0,tolerance:'pointer',cursor:'move',update:function update(){thisInstance.updateBlockSequence();}});},/**
...
var lastElementInLeftContainer=leftSideContainer.children(':last');rightSideContainer.append(lastElementInLeftContainer);}},/**
		 ...
		 */updateFieldSequence:function updateFieldSequence(){var thisInstance=this,progressIndicatorElement=$.progressIndicator({position:'html',blockInfo:{enabled:!0}}),params={};params.module=app.getModuleName(),params.parent=app.getParentModuleName(),params.action='Field',params.mode='move',params.updatedFields=thisInstance.updatedBlockFieldsList,AppConnector.request(params).done(function(){progressIndicatorElement.progressIndicator({mode:'hide'}),window.location.reload();var params={};params.text=app.vtranslate('JS_FIELD_SEQUENCE_UPDATED'),Settings_Vtiger_Index_Js.showMessage(params);}).fail(function(){progressIndicatorElement.progressIndicator({mode:'hide'});});},/**
		 * Function to check if picklist exist
		 */checkPicklistExist:function checkPicklistExist(fieldNameValue){var aDeferred=$.Deferred();return AppConnector.request({module:app.getModuleName(),parent:app.getParentModuleName(),action:'Field',mode:'checkPicklistExist',fieldName:fieldNameValue.toLowerCase()}).done(function(data){data.result?Vtiger_Helper_Js.showConfirmationBox({message:app.vtranslate('JS_EXIST_PICKLIST_NAME')}).done(function(){aDeferred.resolve(!0);}).fail(function(){aDeferred.resolve(!1);}):aDeferred.resolve(!0);}),aDeferred.promise()},/**
		 * Function to register click evnet add custom field button
		 */registerAddCustomFieldEvent:function registerAddCustomFieldEvent(){var _this2=this,thisInstance=this,container=$('#layoutEditorContainer'),contents=container.find('.contents');contents.find('.addCustomField').on('click',function(e){var blockId=$(e.currentTarget).closest('.editFieldsTable').data('blockId'),addFieldContainer=container.find('.createFieldModal').clone(!0,!0);addFieldContainer.removeClass('d-none').show();var callBackFunction=function(data){App.Fields.Picklist.showSelect2ElementView(data.find('select'),{width:'100%'});var form=data.find('.createCustomFieldForm');form.attr('id','createFieldForm'),App.Fields.Picklist.showSelect2ElementView(form.find('[name="pickListValues"]'),{tags:!0,tokenSeparators:[',']}),_this2.registerFieldTypeChangeEvent(form),_this2.registerMultiReferenceFieldsChangeEvent(form),_this2.registerMultiReferenceFilterFieldChangeEvent(form),_this2.registerReferenceModuleChangeEvent(form),_this2.registerViewRelatedChangeEvent(form);var params=app.getvalidationEngineOptions(!0);params.onValidationComplete=function(form,valid){if(valid&&thisInstance.validateFieldsValue(form)){var saveButton=form.find(':submit'),fieldNameValue=$('[name="fieldName"]',form).val(),fieldTypeValue=$('[name="fieldType"]',form).val();saveButton.attr('disabled','disabled'),'Picklist'==fieldTypeValue||'MultiSelectCombo'==fieldTypeValue?thisInstance.checkPicklistExist(fieldNameValue).done(function(result){result?thisInstance.saveCustomField(blockId,form):saveButton.removeAttr('disabled');}):thisInstance.saveCustomField(blockId,form);}return !1},form.validationEngine(params);};app.showModalWindow(addFieldContainer,function(data){'function'==typeof callBackFunction&&callBackFunction(data);},{width:'1000px'});});},/**
		 * Function to validate fields value
		 */validateFieldsValue:function validateFieldsValue(form){var message,fieldTypeValue=$('[name="fieldType"]',form).val(),fieldNameValue=$('[name="fieldName"]',form).val();if('Picklist'==fieldTypeValue||'MultiSelectCombo'==fieldTypeValue){var i,select2Element,pickListValueElement=$('#pickListValues',form),pickListValuesArray=pickListValueElement.val(),pickListValuesArraySize=pickListValuesArray.length,specialChars=/["]/;if('status'===fieldNameValue.toLowerCase()||'picklist'===fieldNameValue.toLowerCase())return message=app.vtranslate('JS_RESERVED_PICKLIST_NAME'),$('[name="fieldName"]',form).validationEngine('showPrompt',message,'error','bottomLeft',!0),!1;for(i=0;i<pickListValuesArraySize;i++)if(specialChars.test(pickListValuesArray[i]))return select2Element=app.getSelect2ElementFromSelect(pickListValueElement),message=app.vtranslate('JS_SPECIAL_CHARACTERS')+' " '+app.vtranslate('JS_NOT_ALLOWED'),select2Element.validationEngine('showPrompt',message,'error','bottomLeft',!0),!1;var lowerCasedpickListValuesArray=$.map(pickListValuesArray,function(item){return item.toLowerCase()}),uniqueLowerCasedpickListValuesArray=$.uniqueSort(lowerCasedpickListValuesArray),uniqueLowerCasedpickListValuesArraySize=uniqueLowerCasedpickListValuesArray.length;if(0<pickListValuesArraySize-uniqueLowerCasedpickListValuesArraySize)return select2Element=app.getSelect2ElementFromSelect(pickListValueElement),message=app.vtranslate('JS_DUPLICATES_VALUES_FOUND'),select2Element.validationEngine('showPrompt',message,'error','bottomLeft',!0),!1}if('Tree'==fieldTypeValue||'CategoryMultipicklist'==fieldTypeValue){var treeListElement=form.find('select.TreeList');if('-'==treeListElement.val())return message=app.vtranslate('JS_FIELD_CAN_NOT_BE_EMPTY'),form.find('.TreeList').validationEngine('showPrompt',message,'error','bottomLeft',!0),!1}if('ServerAccess'==fieldTypeValue){var serverListElement=form.find('select[name="server"]');if('-'==serverListElement.val())return message=app.vtranslate('JS_FIELD_CAN_NOT_BE_EMPTY'),serverListElement.validationEngine('showPrompt',message,'error','bottomLeft',!0),!1}return !0},/**
		 * Function to save field and show message
		 */saveCustomField:function saveCustomField(blockId,form){var _this3=this,saveButton=form.find(':submit');this.addCustomField(blockId,form).done(function(data){var result=data.result,params={};data.success?(app.hideModalWindow(),params.text=app.vtranslate('JS_CUSTOM_FIELD_ADDED'),Settings_Vtiger_Index_Js.showMessage(params),_this3.showCustomField(result)):(message=data.error.message,app.showNotify({title:513==data.error.code?form.find('.fieldLabelForm').text():form.find('.fieldNameForm').text(),type:'error',text:data.error.message}),saveButton.removeAttr('disabled'));});},/**
...
length1>length2?sortable2.append(fieldCopy.removeClass('d-none newCustomFieldCopy')):sortable1.append(fieldCopy.removeClass('d-none newCustomFieldCopy')),thisInstance.makeFieldsListSortable();},/**
		 ...
		 */displayNewCustomBlock:function displayNewCustomBlock(result){var contents=$('#layoutEditorContainer').find('.contents'),beforeBlockId=result.beforeBlockId,beforeBlock=contents.find('.block_'+beforeBlockId),newBlockCloneCopy=contents.find('.newCustomBlockCopy').clone(!0,!0);newBlockCloneCopy.data('blockId',result.id).find('.blockLabel').append($('<strong class="align-middle">'+result.label+'</strong>')),newBlockCloneCopy.find('.js-block-visibility').data('blockId',result.id),result.isAddCustomFieldEnabled&&newBlockCloneCopy.find('.addCustomField').removeClass('d-none'),beforeBlock.after(newBlockCloneCopy.removeClass('d-none newCustomBlockCopy').addClass('editFieldsTable block_'+result.id)),newBlockCloneCopy.find('.blockFieldsList').sortable({connectWith:'.blockFieldsList'});},/**
		 * Function to update the sequence for all blocks after adding new Block
		 */updateNewSequenceForBlocks:function updateNewSequenceForBlocks(sequenceList){var contents=$('#layoutEditorContainer').find('.contents');$.each(sequenceList,function(blockId,sequence){contents.find('.block_'+blockId).data('sequence',sequence);});},/**
		 * Function to update the block list with the new block label in the clone container
		 */appendNewBlockToBlocksList:function appendNewBlockToBlocksList(result){var contents=$('#layoutEditorContainer').find('.contents'),hiddenAddBlockModel=contents.find('.addBlockModal'),blocksListSelect=hiddenAddBlockModel.find('[name="beforeBlockId"]'),option=$('<option>',{value:result.id,text:result.label});blocksListSelect.append(option.attr('data-label',result.label));},/**
		 * Function to update the block list to remove the deleted custom block label in the clone container
		 */removeBlockFromBlocksList:function removeBlockFromBlocksList(blockId){var contents=$('#layoutEditorContainer').find('.contents'),hiddenAddBlockModel=contents.find('.addBlockModal'),blocksListSelect=hiddenAddBlockModel.find('[name="beforeBlockId"]');blocksListSelect.find('option[value="'+blockId+'"]').remove();},/**
		 * Function to register the change event for block visibility
		 */registerBlockVisibilityEvent:function registerBlockVisibilityEvent(){var thisInstance=this,contents=$('#layoutEditorContainer').find('.contents');contents.on('click','.js-block-visibility',function(e){var currentTarget=$(e.currentTarget);thisInstance.updateBlockStatus(currentTarget);});},/**
...
thisInstance.showHiddenFields(form),form.on('submit',function(e){thisInstance.createReactivateFieldslist(form),app.hideModalWindow(),e.preventDefault();});};app.showModalWindow(inActiveFieldsContainer,function(data){'function'==typeof callBackFunction&&callBackFunction(data);},{width:'1000px'});}});},/**
		 ...
		 */showHiddenFields:function showHiddenFields(form){var self=this,detail=$('#detailViewLayout');$.each(self.inActiveFieldsList,function(blockId,fields){form.find('.inActiveList').append('<h5 class="">'+detail.find('.js-block-container[data-block-id="'+blockId+'"] .js-block-label').text()+'</h5>'),$.each(fields,function(key,value){form.find('.inActiveList').append('<div class="checkbox"><label><input type="checkbox" class="inActiveField mr-2" value="'+key+'" data-block="'+blockId+'"/>'+value+'</label></div>');}),form.find('.inActiveList').append('<hr>');});},/**
		 ...
		 */reActivateHiddenFields:function reActivateHiddenFields(blockId,fields){var self=this,progressIndicatorElement=$.progressIndicator({position:'html',blockInfo:{enabled:!0}}),params={module:app.getModuleName(),parent:app.getParentModuleName(),action:'Field',mode:'unHide',blockId:blockId,fieldIdList:JSON.stringify(fields)},messageParams={};'[]'===params.fieldIdList?(messageParams.text=app.vtranslate('JS_NO_ITEM_SELECTED'),messageParams.type='error',progressIndicatorElement.progressIndicator({mode:'hide'}),Settings_Vtiger_Index_Js.showMessage(messageParams)):AppConnector.request(params).done(function(data){for(var index in data.result)self.showCustomField(data.result[index]);progressIndicatorElement.progressIndicator({mode:'hide'});}).fail(function(){progressIndicatorElement.progressIndicator({mode:'hide'});});},/**
		 * Function to register the click event for delete custom block
		 */registerDeleteCustomBlockEvent:function registerDeleteCustomBlockEvent(){var thisInstance=this,contents=$('#layoutEditorContainer').find('.contents');contents.on('click','.js-delete-custom-block-btn',function(e){var currentTarget=$(e.currentTarget),table=currentTarget.closest('div.editFieldsTable'),blockId=table.data('blockId'),message=app.vtranslate('JS_LBL_ARE_YOU_SURE_YOU_WANT_TO_DELETE');Vtiger_Helper_Js.showConfirmationBox({message:message}).done(function(){thisInstance.deleteCustomBlock(blockId);}).fail(function(){});});},/**
		 ...
		 */relatedModulesTabClickEvent:function relatedModulesTabClickEvent(){var thisInstance=this,contents=$('#layoutEditorContainer').find('.contents'),relatedContainer=contents.find('#relatedTabOrder'),relatedTab=contents.find('.relatedListTab');relatedTab.on('click',function(){0<relatedContainer.find('.relatedTabModulesList').length||thisInstance.showRelatedTabModulesList(relatedContainer);});},/**
		 ...
		 */registerSwitch:function registerSwitch(){var container=$('#layoutEditorContainer');container.find('.js-switch--inventory').on('click',function(event){event.preventDefault();var switchBtn=$(event.currentTarget),state=switchBtn.data('value'),message=app.vtranslate('JS_EXTENDED_MODULE');Vtiger_Helper_Js.showConfirmationBox({message:'<span class="message-medium">'+message+'</span>',className:'test'}).done(function(){var progress=$.progressIndicator({message:app.vtranslate('JS_SAVE_LOADER_INFO'),position:'html',blockInfo:{enabled:!0}}),params={};params.sourceModule=container.find('[name="layoutEditorModules"]').val(),params.type=state,app.saveAjax('changeModuleType',null,params).done(function(data){data.result&&(data.result.success?(container.find('.js-switch--inventory').prop('disabled',!0),Settings_Vtiger_Index_Js.showMessage({type:'success',text:data.result.message}),progress.progressIndicator({mode:'hide'})):window.location.reload());});});});},/**
		 * Function to adding inventory field
		 */registerAddInventoryField:function registerAddInventoryField(){var thisInstance=this,container=thisInstance.getInventoryViewLayout();container.find('.addInventoryField').on('click',function(e){var currentTarget=$(e.currentTarget),selectedModule=$('#layoutEditorContainer').find('[name="layoutEditorModules"]').val(),blockId=currentTarget.closest('.inventoryBlock').data('block-id'),progress=$.progressIndicator();app.showModalWindow(null,'index.php?module=LayoutEditor&parent=Settings&view=CreateInventoryFields&mode=step1&sourceModule='+selectedModule+'&block='+blockId,function(modalContainer){app.showScrollBar(modalContainer.find('.well'),{height:'300px'}),thisInstance.registerStep1(modalContainer,blockId),progress.progressIndicator({mode:'hide'});});});},/**
		 ...
		 * Loading list of fields for a related module
		 */registerMultiReferenceFieldsChangeEvent:function registerMultiReferenceFieldsChangeEvent(form){var thisInstance=this;thisInstance.cacheMRVField=form.find('[name="MRVField"]').clone(!0,!0),thisInstance.cacheMRVFilter=form.find('[name="MRVFilterField"]').clone(!0,!0),form.find('[name="MRVModule"]').on('change',function(){thisInstance.loadMultiReferenceFields(form);});},registerReferenceModuleChangeEvent:function registerReferenceModuleChangeEvent(form){form.find('[name="referenceModule"]').on('change',function(e){if($(e.currentTarget).select2('data')&&1===$(e.currentTarget).select2('data').length){var params={};params.module=app.getModuleName(),params.parent=app.getParentModuleName(),params.action='Filter',params.mode='getFilters',params.rmodule=form.find('[name="referenceModule"]').val(),form.find('[name="referenceModuleFilter"]').select2('destroy'),form.find('[name="referenceModuleFilter"] option').remove(),AppConnector.request(params).done(function(data){$.each(data.result,function(index,value){var option=$('<option>',{value:value.cvid,text:value.viewname});form.find('[name="referenceModuleFilter"]').append(option);}),App.Fields.Picklist.showSelect2ElementView(form.find('[name="referenceModuleFilter"]'),{width:'100%'}),form.find('.preDefinedModuleFilterList').removeClass('d-none');});}else form.find('.preDefinedModuleFilterList').addClass('d-none');});},/**
		 * Loading list of fields for a related module
		 ...
		 * Context help
		 * @param {jQuery} container
		 */registerContextHelp:function registerContextHelp(){$(document).on('click','.js-context-help',function(e){var customConfig={toolbar:'Min'},element=$(e.currentTarget),progressInstance=$.progressIndicator({blockInfo:{enabled:!0}});AppConnector.request({module:app.getModuleName(),parent:app.getParentModuleName(),view:'HelpInfo',field:element.data('field-id')}).done(function(data){app.showModalWindow(data,function(modalContainer){progressInstance.progressIndicator({mode:'hide'}),app.showPopoverElementView(modalContainer.find('.js-help-info')),App.Fields.Text.Editor.register(modalContainer.find('.js-context-area:visible'),customConfig),modalContainer.find('.js-lang').on('change',function(e){var previous=modalContainer.find('.js-editor:not([disabled])');App.Fields.Text.destroyEditor(previous),previous.closest('.js-context-block').addClass('d-none'),previous.prop('disabled',!0);var element=$(e.currentTarget).val(),textArea=modalContainer.find('#'+element+'.js-context-area');textArea.prop('disabled',!1),textArea.closest('.js-context-block').removeClass('d-none'),App.Fields.Text.destroyEditor(textArea),modalContainer.find('.js-help-info').attr('data-content',textArea.val()),App.Fields.Text.Editor.register(textArea,customConfig);}),modalContainer.find('form').on('submit',function(e){e.preventDefault();var form=$(e.currentTarget),params=form.serializeFormData();'undefined'==typeof params.views&&(params.views=form.find('[name="views"]').val()),app.saveAjax('contextHelp','',params).done(function(){app.showNotify({type:'success',text:app.vtranslate('JS_SAVE_CHANGES')});var prefix=form.find('.js-lang').val(),textArea=form.find('#'+prefix+'.js-context-area');form.find('.js-help-info').attr('data-content',textArea.val());});});});});});},registerViewRelatedChangeEvent:function registerViewRelatedChangeEvent(form){form.find('[name="referredField"]').on('change',function(){var params={};params.module=app.getModuleName(),params.parent=app.getParentModuleName(),params.action='ViewRelated',params.mode='getFields',params.sourceModule=$('#selectedModuleName').val(),params.rfield=form.find('[name="referredField"]').val(),form.find('[name="viewField"]').select2('destroy'),form.find('[name="viewField"] option,optgroup').remove(),AppConnector.request(params).done(function(data){$.each(data.result,function(index,block){var optionGroup=$('<optgroup>',{label:block.label,"data-module":block.module});$.each(block.fields,function(index,field){var option=$('<option>',{value:field.name,text:field.label});optionGroup.append(option);}),form.find('[name="viewField"]').append(optionGroup);}),App.Fields.Picklist.showSelect2ElementView(form.find('[name="viewField"]'),{width:'100%'});});});},/**
		 * register events for layout editor
		 */registerEvents:function registerEvents(){var thisInstance=this;thisInstance.registerBlockEvents(),thisInstance.registerFieldEvents(),thisInstance.setInactiveFieldsList(),thisInstance.registerAddCustomBlockEvent(),thisInstance.relatedModulesTabClickEvent(),thisInstance.registerModulesChangeEvent(),thisInstance.registerRelModulesChangeEvent(),1===$('#relatedTabOrder').length&&(thisInstance.registerRelatedListEvents(),thisInstance.makeRelatedModuleSortable()),thisInstance.registerSwitch(),thisInstance.registerAddInventoryField(),thisInstance.registerEditInventoryField(),thisInstance.registerInventoryFieldSequenceSaveClick(),thisInstance.registerDeleteInventoryField(),this.registerFieldSequenceSaveClick();},registerBasicEvents:function registerBasicEvents(){var container=$('#layoutEditorContainer');this.registerEvents(),this.registerCopyClipboard(),this.registerContextHelp(container);}}),$(document).ready(function(){var instance=new Settings_LayoutEditor_Js;instance.registerBasicEvents();}),Vtiger_WholeNumberGreaterThanZero_Validator_Js('Vtiger_FloatingDigits_Validator_Js',{/**
		 ...
		 * @return true if validation is successfull
		 * @return false if validation error occurs
		 */validate:function validate(){var fieldValue=this.getFieldValue();return this.validateValue(fieldValue)},validateValue:function validateValue(fieldValue){var specialChars=/[&\<\>\:\'\"\,]/;if(specialChars.test(fieldValue)){var errorInfo=app.vtranslate('JS_SPECIAL_CHARACTERS')+' & < > \' " : , '+app.vtranslate('JS_NOT_ALLOWED');return this.setError(errorInfo),!1}return !0}}),Vtiger_Base_Validator_Js('Vtiger_PicklistFieldValues_Validator_Js',{/**
		 *Function which invokes field validation
		 *@param accepts field element as parameter
		 * @return error if validation fails true on success
		 ...
		 * @return true if validation is successfull
		 * @return false if validation error occurs
		 */validate:function validate(){return this.validateValue(this.getElement().val())},validateValue:function validateValue(fieldValue){var _this4=this,specialChars=/[\<\>\"\,\#]/,r=!0;return $.each(fieldValue,function(i,val){specialChars.test(val)&&(_this4.setError(app.vtranslate('JS_SPECIAL_CHARACTERS')+' < > " , # '+app.vtranslate('JS_NOT_ALLOWED')),r=!1);}),r}});
