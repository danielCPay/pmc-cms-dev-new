			...
			conditionGroups.each(function (index, domElement) {
				...
				conditions.each(function (i, conditionDomElement) {
					...
					let fieldType = fieldSelectElement.find('option:selected').data('fieldinfo').type,
						...
						key,
						field;
					if ($.inArray(fieldType, ['picklist', 'multipicklist', 'multiReferenceValue', 'role']) > -1) {
						for (key in fieldList) {
							...
							} else if (field === 'value' && valueSelectElement.is('select') && $.inArray(fieldType, ['picklist', 'role']) > -1) {
...
Vtiger_Sharedowner_Field_Js('Workflows_Sharedowner_Field_Js', {}, {
	getUi: function () {
		var html = '<select class="select2" multiple data-value="value" name="' + this.getName() + '">';
		var pickListValues = this.getPickListValues();
...
Vtiger_Picklist_Field_Js('Workflows_Role_Field_Js', {}, {});
