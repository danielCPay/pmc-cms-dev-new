...
Vtiger_AdvanceFilter_Js('Workflows_AdvanceFilter_Js',{},{validationSupportedFieldConditionMap:{email:['e','n'],date:['is'],datetime:['is']},//Hols field type for which there is validations always needed
allConditionValidationNeededFieldList:['double','integer'],// comparators which do not have any field Specific UI.
comparatorsWithNoValueBoxMap:['is record open','is record closed','has changed','not has changed','is empty','is not empty','is added','is Watching Record','is Not Watching Record'],getFieldSpecificType:function getFieldSpecificType(fieldSelected){var fieldInfo=fieldSelected.data('fieldinfo'),type=fieldInfo.type;return type},getModuleName:function getModuleName(){return app.getModuleName()},/**
...
if(thisInstance.isEmptyFieldSelected(fieldSelectElement))return !0;var key,field,fieldType=fieldSelectElement.find('option:selected').data('fieldinfo').type,rowValues={};if(-1<$.inArray(fieldType,['picklist','multipicklist','multiReferenceValue','role'])){for(key in fieldList)if(field=fieldList[key],'value'===field&&valueSelectElement.is('input')){var pickListValues=valueSelectElement.data('picklistvalues'),valuesArr=valueSelectElement.val().split(','),newValuesArr=[];for(i=0;i<valuesArr.length;i++)'undefined'==typeof pickListValues[valuesArr[i]]?newValuesArr.push(valuesArr[i]):newValuesArr.push(pickListValues[valuesArr[i]]);rowValues[field]=newValuesArr.join(',');}else if('value'===field&&valueSelectElement.is('select')&&-1<$.inArray(fieldType,['picklist','role']))rowValues[field]=valueSelectElement.val();else if('value'===field&&valueSelectElement.is('select')&&-1<$.inArray(fieldType,['multipicklist','multiReferenceValue','categoryMultipicklist'])){var value=valueSelectElement.val();rowValues[field]=null==value?value:value.join(',');}else rowValues[field]=$('[name="'+field+'"]',rowElement).val();}else for(key in fieldList)field=fieldList[key],rowValues[field]='value'===field?valueSelectElement.val():$('[name="'+field+'"]',rowElement).val();('false'===$('[name="valuetype"]',rowElement).val()||0===$('[name="valuetype"]',rowElement).length)&&(rowValues.valuetype='rawtext'),rowValues.groupid=0===index?0:1,rowElement.is(':last-child')&&(rowValues.column_condition=''),iterationValues[columnIndex]=rowValues,columnIndex++;}),!$.isEmptyObject(iterationValues)&&(values[index+1]={},values[index+1].columns=iterationValues),0<groupElement.find('div.groupCondition').length&&(values[index+1].condition=groupElement.find('div.groupCondition [name="condition"]').val()))}),values},/**
		 ...
		 */getUi:function getUi(){var html,element,comparatorSelectedOptionVal=this.get('comparatorElementVal'),dateSpecificConditions=this.get('dateSpecificConditions');if(0<comparatorSelectedOptionVal.length){if('between'===comparatorSelectedOptionVal||'custom'===comparatorSelectedOptionVal)return html='<div class="date"><input class="form-control dateRangeField" data-calendar-type="range" name="'+this.getName()+'" data-date-format="'+this.getDateFormat()+'" type="text" value="'+this.getValue()+'"></div>',element=jQuery(html),this.addValidationToElement(element);if(this._specialDateComparator(comparatorSelectedOptionVal))return html='<input name="'+this.getName()+'" type="text" value="'+this.getValue()+'" data-validation-engine="validate[funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" data-validator="[{"name":"PositiveNumber"}]"><input type="hidden" name="valuetype" value="'+this.get('workflow_valuetype')+'" />',jQuery(html);if(comparatorSelectedOptionVal in dateSpecificConditions){var startValue=dateSpecificConditions[comparatorSelectedOptionVal].startdate,endValue=dateSpecificConditions[comparatorSelectedOptionVal].enddate;return html='<input name="'+this.getName()+'"  type="text" ReadOnly="true" value="'+startValue+','+endValue+'">',jQuery(html)}if('is today'===comparatorSelectedOptionVal);else return this._super()}else return html='<input type="text" class="getPopupUi date form-control" name="'+this.getName()+'"  data-date-format="'+this.getDateFormat()+'"  value="'+this.getValue()+'" /><input type="hidden" name="valuetype" value="'+this.get('workflow_valuetype')+'" />',element=jQuery(html),this.addValidationToElement(element)},_specialDateComparator:function _specialDateComparator(comp){var specialComparators=['less than days ago','more than days ago','in less than','in more than','days ago','days later'];for(var index in specialComparators)if(comp==specialComparators[index])return !0;return !1}}),Vtiger_Date_Field_Js('Workflows_Datetime_Field_Js',{},{/**
		 * Function to get the user date format
		 */getDateFormat:function getDateFormat(){return this.get('date-format')},/**
		 * Function to get the ui
		 * @return - input text field
		 */getUi:function getUi(){var html,element;return this._specialDateTimeComparator(this.get('comparatorElementVal'))?(html='<input name="'+this.getName()+'" type="text" value="'+this.getValue()+'" data-validator="[{name:PositiveNumber}]"><input type="hidden" name="valuetype" value="'+this.get('workflow_valuetype')+'" />',element=$(html)):(html='<input type="text" class="getPopupUi date form-control" name="'+this.getName()+'"  data-date-format="'+this.getDateFormat()+'"  value="'+this.getValue()+'" /><input type="hidden" name="valuetype" value="'+this.get('workflow_valuetype')+'" />',element=$(html)),element},_specialDateTimeComparator:function _specialDateTimeComparator(comp){var specialComparators=['less than hours before','less than hours later','more than hours later','more than hours before','days ago','days later','hours ago','hours later'];for(var index in specialComparators)if(comp==specialComparators[index])return !0;return !1}}),Vtiger_Currency_Field_Js('Workflows_Currency_Field_Js',{},{getUi:function getUi(){var html='<input type="text" class="getPopupUi marginLeftZero form-control" name="'+this.getName()+'" value="'+this.getValue()+'"  /><input type="hidden" name="valuetype" value="'+this.get('workflow_valuetype')+'" />',element=jQuery(html);return this.addValidationToElement(element)}}),Vtiger_Time_Field_Js('Workflows_Time_Field_Js',{},{/**
		 * Function to get the ui
		 * @return - input text field
		 ...
		 * Function to get the ui
		 * @return - input text field
		 */getUi:function getUi(){var html='<input type="text" class="getPopupUi form-control boolean" name="'+this.getName()+'" value="'+this.getValue()+'" /><input type="hidden" name="valuetype" value="'+this.get('workflow_valuetype')+'" />',element=jQuery(html);return this.addValidationToElement(element)}}),Vtiger_Owner_Field_Js('Workflows_Owner_Field_Js',{},{getUi:function getUi(){var html='<select class="select2" data-value="value" name="'+this.getName()+'">',pickListValues=this.getPickListValues(),selectedOption=this.getValue();for(var optGroup in pickListValues){html+='<optgroup label="'+optGroup+'">';var optionGroupValues=pickListValues[optGroup];for(var option in optionGroupValues)html+='<option value="'+option+'" ',option==selectedOption&&(html+=' selected '),html+='>'+optionGroupValues[option]+'</option>';html+='</optgroup>';}html+='</select>';var selectContainer=jQuery(html);return this.addValidationToElement(selectContainer),selectContainer}}),Vtiger_Sharedowner_Field_Js('Workflows_Sharedowner_Field_Js',{},{getUi:function getUi(){var html='<select class="select2" multiple data-value="value" name="'+this.getName()+'">',pickListValues=this.getPickListValues(),selectedOption=app.htmlDecode(this.getValue()),selectedOptionsArray=selectedOption.split(',');for(var optGroup in pickListValues){html+='<optgroup label="'+optGroup+'">';var optionGroupValues=pickListValues[optGroup];for(var option in optionGroupValues)html+='<option value="'+option+'" ',-1!=jQuery.inArray(option,selectedOptionsArray)&&(html+=' selected '),html+='>'+optionGroupValues[option]+'</option>';html+='</optgroup>';}html+='</select>';var selectContainer=jQuery(html);return this.addValidationToElement(selectContainer),selectContainer}}),Vtiger_Picklist_Field_Js('Workflows_Picklist_Field_Js',{},{}),AdvanceFilter_Categorymultipicklist_Field_Js('Workflows_Categorymultipicklist_Field_Js',{},{}),Workflows_Owner_Field_Js('Workflows_Usercreator_Field_Js',{},{}),Vtiger_Picklist_Field_Js('Workflows_Country_Field_Js',{},{}),Vtiger_Picklist_Field_Js('Workflows_Role_Field_Js',{},{});
