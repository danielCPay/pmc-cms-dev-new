	...
	{},
	{
		...
		getValues(tasktype) {
			...
			$('.js-conditions-row', $('#save_fieldvaluemapping')).each((i, conditionDomElement) => {
				...
				} else if (fieldType == 'multipicklist' || fieldType == 'sharedOwner') {
					for (key in fieldList) {
						field = fieldList[key];
		...
		getFieldSpecificUi: function (fieldSelectElement) {
			...
			if (fieldModel.data.type === 'picklist') {
				return (function () {
						var html =
		...
		registerVTEntityWorkflowEvents: function () {
			$('.workflowModule').on('change', function (e) {
				var params = {
					module: app.getModuleName(),
					parent: app.getParentModuleName(),
					action: 'TaskAjax',
					mode: 'VTEntityWorkflowGetFieldsAndWorkflows',
					for_workflow: $('[name="for_workflow"]').val(),
				};
				var relatedModule = $(e.currentTarget).val();
				if (relatedModule) {
				...
				if (relatedModule.startsWith('PARENT')) {
					$('.otherWorkflowFieldValueVersion').show();
				} else {
					$('.otherWorkflowFieldValueVersion').hide();
				}
				AppConnector.request(params).done(function (data) {
					progressIndicatorElement.progressIndicator({ mode: 'hide' });

					$('.otherWorkflowId option').remove();
					$('.otherWorkflowField option').remove();
					data.result.fields.forEach(f => {
						$('.otherWorkflowField').append('<option value="' + f.id + '">' + f.name + '</option>');
					});
					data.result.workflows.forEach(w => {
						$('.otherWorkflowId').append('<option value="' + w.id + '">' + w.name + '</option>');
					});
					if (data.result.fields.length) {
						$('.otherWorkflowField').val(data.result.fields[0].id);
					}
					if (data.result.workflows.length) {
						$('.otherWorkflowId').val(data.result.workflows[0].id);
					}
				});
			});
		},
