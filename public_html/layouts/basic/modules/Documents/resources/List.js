...
Vtiger_List_Js(
	'Documents_List_Js',
	{
		massMove: function (url) {
			...
			var validationResult = !this.getSelectedRecordCount();
			if (validationResult != true) {
				var postData = Vtiger_List_Js.getInstance().getSearchParams();
		...
		massDownload: function (url) {
			...
			if (validationResult != true) {
				...
				$.extend(postData, Vtiger_List_Js.getInstance().getSearchParams());
				app.openUrlMethodPost(url, postData);
				progressIndicatorElement.progressIndicator({ mode: 'hide' });
			} else {
				listInstance.noRecordSelectedAlert();
			}
