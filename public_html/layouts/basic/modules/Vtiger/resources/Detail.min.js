...
jQuery.Class('Vtiger_Detail_Js',{detailInstance:!1,getInstance:function getInstance(){if(!1==Vtiger_Detail_Js.detailInstance){var instance,moduleClassName=app.getModuleName()+'_'+app.getViewName()+'_Js';instance='undefined'==typeof window[moduleClassName]?new Vtiger_Detail_Js:new window[moduleClassName],Vtiger_Detail_Js.detailInstance=instance;}return Vtiger_Detail_Js.detailInstance},/*
		 ...
		 * @params: Action url
		 */triggerAjaxAction:function triggerAjaxAction(moduleName,action){var detailInstance=Vtiger_Detail_Js.getInstance(),actionParams={type:'POST',module:moduleName,action:action,record:detailInstance.getRecordId()};AppConnector.request(actionParams).done(function(data){data.result.notify&&app.showNotify({title:app.vtranslate('JS_MESSAGE'),text:data.result.notify.text,type:data.result.notify.type});}).fail(function(error,err){app.showNotify({type:'error',text:err});});},/*
		 * function to trigger Detail view actions
...
init:function init(){},loadWidgetsEvents:function loadWidgetsEvents(){var thisInstance=this;app.event.on('DetailView.Widget.AfterLoad',function(e,widgetContent,relatedModuleName){'Calendar'===relatedModuleName&&thisInstance.reloadWidgetActivitesStats(widgetContent.closest('.activityWidgetContainer')),'ModComments'===relatedModuleName&&thisInstance.registerCommentEventsInDetail(widgetContent.closest('.updatesWidgetContainer')),widgetContent.find('[name="relatedModule"]').length&&thisInstance.registerShowSummary(widgetContent),'OSSMailView'===relatedModuleName&&(Vtiger_Index_Js.registerMailButtons(widgetContent),widgetContent.find('.showMailModal').on('click',function(e){e.preventDefault();var progressIndicatorElement=jQuery.progressIndicator();app.showModalWindow('',$(e.currentTarget).data('url')+'&noloadlibs=1',function(data){Vtiger_Index_Js.registerMailButtons(data),progressIndicatorElement.progressIndicator({mode:'hide'});});})),thisInstance.registerEmailEvents(widgetContent),'DetailView'===relatedModuleName&&thisInstance.registerBlockStatusCheckOnLoad(),thisInstance.registerCollapsiblePanels(widgetContent.closest('.js-detail-widget')),thisInstance.registerAutoCompleteFields(widgetContent);});},loadWidgets:function loadWidgets(){var _this=this,container=this.getForm(),widgetList=jQuery('[class^="widgetContainer_"]'),length=widgetList.length;widgetList.each(function(index,widget){widget=$(widget),widget.is(':visible')&&_this.loadWidget(widget),length===index+1&&(container.validationEngine('detach'),container.validationEngine(app.validationEngineOptionsForRecord));}),this.registerRelatedModulesRecordCount();},loadWidget:function loadWidget(widgetContainer,params){var thisInstance=this,contentContainer=$('.js-detail-widget-content',widgetContainer),relatedModuleName=void 0;if(this.registerFilterForAddingModuleRelatedRecordFromSummaryWidget(widgetContainer),relatedModuleName=widgetContainer.find('[name="relatedModule"]').length?widgetContainer.find('[name="relatedModule"]').val():widgetContainer.data('name'),void 0===params){var urlParams=widgetContainer.data('url');if(null==urlParams)return;var index,queryParameters=urlParams.split('&'),keyValueMap={};for(index=0;index<queryParameters.length;index++){var queryParamComponents=queryParameters[index].split('=');keyValueMap[queryParamComponents[0]]=queryParamComponents[1];}params=keyValueMap;}var aDeferred=$.Deferred();return contentContainer.progressIndicator({}),AppConnector.request({type:'POST',async:!1,dataType:'html',data:params}).done(function(data){if(contentContainer.progressIndicator({mode:'hide'}),contentContainer.html(data),App.Fields.Picklist.showSelect2ElementView(widgetContainer.find('.select2')),app.registerModal(contentContainer),App.Components.DropFile.register(contentContainer),relatedModuleName){var relatedController=Vtiger_RelatedList_Js.getInstanceByUrl(widgetContainer.data('url'),thisInstance.getSelectedTab());relatedController.setRelatedContainer(contentContainer),relatedController.registerRelatedEvents(),thisInstance.widgetRelatedRecordView(widgetContainer,!0);var chart=contentContainer.find('[name="typeChart"]');if(chart.length&&'undefined'!=typeof window.Vtiger_Widget_Js){var widgetInstance=Vtiger_Widget_Js.getInstance(contentContainer,chart.val());widgetInstance.init(contentContainer),widgetInstance.loadChart();}}app.event.trigger('DetailView.Widget.AfterLoad',contentContainer,relatedModuleName,thisInstance),aDeferred.resolve(params);}).fail(function(){contentContainer.progressIndicator({mode:'hide'}),aDeferred.reject();}),aDeferred.promise()},widgetRelatedRecordView:function widgetRelatedRecordView(container,load){var cacheKey=this.getRecordId()+'_'+container.data('id'),relatedRecordCacheID=app.moduleCacheGet(cacheKey);if(null!==relatedRecordCacheID){var newActive=container.find('.js-carousel-item[data-id = \''+relatedRecordCacheID+'\']');newActive.length&&(container.find('.js-carousel-item.active').removeClass('active'),container.find('.js-carousel-item[data-id = \''+relatedRecordCacheID+'\']').addClass('active'));}var controlBox=container.find('.control-widget'),prev=controlBox.find('.prev'),next=controlBox.find('.next'),active=container.find('.js-carousel-item.active');1>=container.find('.js-carousel-item').length||!active.next().length?next.addClass('disabled'):next.removeClass('disabled'),1>=container.find('.js-carousel-item').length||!active.prev().length?prev.addClass('disabled'):prev.removeClass('disabled'),load&&(next.on('click',function(){if(!$(this).hasClass('disabled')){var active=container.find('.js-carousel-item.active');active.removeClass('active');var nextElement=active.next();nextElement.addClass('active'),nextElement.next().length||next.addClass('disabled'),active.prev()&&prev.removeClass('disabled'),app.moduleCacheSet(cacheKey,nextElement.data('id'));}}),prev.on('click',function(){if(!$(this).hasClass('disabled')){var active=container.find('.js-carousel-item.active');active.removeClass('active');var prevElement=active.prev();prevElement.addClass('active'),prevElement.prev().length||prev.addClass('disabled'),active.next()&&next.removeClass('disabled'),app.moduleCacheSet(cacheKey,prevElement.data('id'));}}));},loadContents:function loadContents(url,data){var thisInstance=this,aDeferred=jQuery.Deferred(),detailContentsHolder=this.getContentHolder(),params=url;return 'undefined'!=typeof data&&(params={},params.url=url,params.data=data),AppConnector.requestPjax(params).done(function(responseData){detailContentsHolder.html(responseData),responseData=detailContentsHolder.html(),thisInstance.registerBlockStatusCheckOnLoad(),App.Fields.Picklist.changeSelectElementView(detailContentsHolder),App.Fields.Date.register(detailContentsHolder),thisInstance.getForm().validationEngine(),app.event.trigger('DetailView.LoadContents.AfterLoad',responseData),aDeferred.resolve(responseData);}),aDeferred.promise()},getUpdateFieldsArray:function getUpdateFieldsArray(){return this.updatedFields},/**
		 ...
		 */getForm:function getForm(){return !1==this.detailViewForm&&(this.detailViewForm=jQuery('#detailView')),this.detailViewForm},getRecordId:function getRecordId(){return app.getRecordId()},getRelatedModuleName:function getRelatedModuleName(){if(1==jQuery('.relatedModuleName',this.getContentHolder()).length)return jQuery('.relatedModuleName',this.getContentHolder()).val()},saveFieldValues:function saveFieldValues(fieldDetailList){var aDeferred=jQuery.Deferred(),recordId=this.getRecordId(),data={};'undefined'!=typeof fieldDetailList&&(data=fieldDetailList),data.record=recordId,data.module=app.getModuleName(),data.action='SaveAjax';var params={};return params.data=data,params.async=!1,params.dataType='json',this.preSaveValidation(JSON.parse(JSON.stringify(params)),this.getForm()).then(function(response){!0===response?AppConnector.request(params).done(function(responseData){aDeferred.resolve(responseData);}).fail(function(jqXHR,textStatus,errorThrown){aDeferred.reject(jqXHR,textStatus,errorThrown),app.errorLog(jqXHR,textStatus,errorThrown);}):aDeferred.resolve({success:!1});}),aDeferred.promise()},preSaveValidation:function preSaveValidation(params,form){var aDeferred=$.Deferred();return form.find('#preSaveValidation').val()?(document.progressLoader=$.progressIndicator({message:app.vtranslate('JS_SAVE_LOADER_INFO'),position:'html',blockInfo:{enabled:!0}}),params.data.mode='preSaveValidation',AppConnector.request(params).done(function(data){document.progressLoader.progressIndicator({mode:'hide'});for(var response=data.result,i=0;i<response.length;i++)!0!==response[i].result&&app.showNotify({text:response[i].message?response[i].message:app.vtranslate('JS_ERROR'),type:'error'});0>=data.result.length?aDeferred.resolve(!0):aDeferred.resolve(!1);}).fail(function(textStatus,errorThrown){document.progressLoader.progressIndicator({mode:'hide'}),app.showNotify({text:app.vtranslate('JS_ERROR'),type:'error'}),app.errorLog(textStatus,errorThrown),aDeferred.resolve(!1);})):aDeferred.resolve(!0),aDeferred.promise()},getRelatedListCurrentPageNum:function getRelatedListCurrentPageNum(){return jQuery('input[name="currentPageNum"]',this.getContentHolder()).val()},/**
		 * function to hide comment block.
		 */removeCommentBlock:function removeCommentBlock(){$('.js-add-comment-block',$('.js-comments-body',this.getContentHolder())).remove();},/**
		 ...
		 */showCommentBlock:function showCommentBlock(){$('.js-add-comment-block',$('.js-comments-body',this.getContentHolder())).show();},/**
...
var selectedValue=targetPickList.data('selectedValue');-1==jQuery.inArray(selectedValue,targetPickListMap)?(thisInstance.targetPicklistChange=!0,thisInstance.targetPicklist=targetPickList.closest('td')):(thisInstance.targetPicklistChange=!1,thisInstance.targetPicklist=!1);var listOfAvailableOptions=targetPickList.data('availableOptions');'undefined'==typeof listOfAvailableOptions&&(listOfAvailableOptions=jQuery('option',targetPickList),targetPickList.data('available-options',listOfAvailableOptions));var targetOptions=new jQuery,optionSelector=[];optionSelector.push('');for(var _i=0;_i<targetPickListMap.length;_i++)optionSelector.push(targetPickListMap[_i]);jQuery.each(listOfAvailableOptions,function(i,e){var picklistValue=jQuery(e).val();-1!=jQuery.inArray(picklistValue,optionSelector)&&(targetOptions=targetOptions.add(jQuery(e)));});var targetPickListSelectedValue='';targetPickListSelectedValue=targetOptions.filter('[selected]').val(),1==targetPickListMap.length&&(targetPickListSelectedValue=targetPickListMap[0]),targetPickList.html(targetOptions).val(targetPickListSelectedValue).trigger('change');}});}),sourcePickListElements.trigger('change');}}},/**
		 ...
		 */addComment:function addComment(currentTarget,data){var self=this,mode=currentTarget.data('mode'),closestAddCommentBlock=currentTarget.closest('.js-add-comment-block'),commentTextAreaElement=closestAddCommentBlock.find('.js-comment-content'),commentInfoBlock=currentTarget.closest('.js-comment-single');if(commentTextAreaElement.html(''),'add'==mode){var commentId=data.result.id,commentHtml=self.getCommentUI(commentId);commentHtml.done(function(data){var commentBlock=closestAddCommentBlock.closest('.js-comment-details'),detailContentsHolder=self.getContentHolder(),noCommentsMsgContainer=$('.js-noCommentsMsgContainer',detailContentsHolder);if(noCommentsMsgContainer.remove(),0<commentBlock.length){closestAddCommentBlock.remove();var childComments=commentBlock.find('ul');if(0>=childComments.length){var currentChildCommentsCount=commentInfoBlock.find('.js-view-thread-block').data('data-child-comments-count');commentInfoBlock.find('.js-child-comments-count').text(currentChildCommentsCount+1);var parentCommentId=commentInfoBlock.find('.js-comment-info-header').data('commentid');self.getChildComments(parentCommentId).done(function(responsedata){$(responsedata).appendTo(commentBlock),commentInfoBlock.find('.js-view-thread-block').hide(),commentInfoBlock.find('.hideThreadBlock').show();});}else $('<li class="js-comment-details commentDetails">'+data+'</li>').appendTo(commentBlock.find('.js-comments-body'));}else $('<li class="js-comment-details commentDetails">'+data+'</li>').prependTo(closestAddCommentBlock.closest('.contents').find('.commentsList'));commentInfoBlock.find('.js-comment-container').show(),app.event.trigger('DetailView.SaveComment.AfterLoad',commentInfoBlock,data);});}else if('edit'==mode){var modifiedTime=commentInfoBlock.find('.js-comment-modified-time'),commentInfoContent=commentInfoBlock.find('.js-comment-info'),commentEditStatus=commentInfoBlock.find('.js-edited-status'),commentReason=commentInfoBlock.find('.js-edit-reason-span');commentInfoContent.html(data.result.commentcontent),commentReason.html(data.result.reasontoedit),modifiedTime.html(data.result.modifiedtime),modifiedTime.attr('title',data.result.modifiedtimetitle),commentEditStatus.hasClass('d-none')&&commentEditStatus.removeClass('d-none'),''!=data.result.reasontoedit&&commentInfoBlock.find('.js-edit-reason').removeClass('d-none'),commentInfoContent.show(),commentInfoBlock.find('.js-comment-container').show(),closestAddCommentBlock.remove(),app.event.trigger('DetailView.SaveComment.AfterUpdate',commentInfoBlock,data);}},/**
		 * Register all comment events
		 * @param {jQuery} detailContentsHolder
		 */registerCommentEvents:function registerCommentEvents(detailContentsHolder){var self=this;detailContentsHolder.on('click','.js-close-comment-block',function(e){var commentInfoBlock=$(e.currentTarget.closest('.js-comment-single'));commentInfoBlock.find('.js-comment-container').show(),commentInfoBlock.find('.js-comment-info').show(),self.removeCommentBlock();}),detailContentsHolder.on('click','.js-reply-comment',function(e){self.removeCommentBlock(),self.hideButtonAction();var commentInfoBlock=$(e.currentTarget).closest('.js-comment-single');commentInfoBlock.find('.js-comment-container').hide(),self.getCommentBlock().appendTo(commentInfoBlock).show();}),detailContentsHolder.on('click','.js-edit-comment',function(e){self.removeCommentBlock(),self.hideButtonAction();var commentInfoBlock=$(e.currentTarget).closest('.js-comment-single'),commentInfoContent=commentInfoBlock.find('.js-comment-info'),editCommentBlock=self.getEditCommentBlock();editCommentBlock.find('.js-comment-content').html(commentInfoContent.html()),editCommentBlock.find('.js-reason-to-edit').html(commentInfoBlock.find('.js-edit-reason-span').text()),commentInfoContent.hide(),commentInfoBlock.find('.js-comment-container').hide(),editCommentBlock.appendTo(commentInfoBlock).show();}),detailContentsHolder.on('click','.js-detail-view-save-comment',function(e){var element=$(e.currentTarget);element.is(':disabled')||self.saveComment(e).done(function(){self.registerRelatedModulesRecordCount(),self.loadWidget(detailContentsHolder.find('[data-type=\'Comments\']')).done(function(){element.removeAttr('disabled');});}).fail(function(error,err){element.removeAttr('disabled'),app.errorLog(error,err);});}),detailContentsHolder.on('click','.js-save-comment',function(e){var element=$(e.currentTarget);element.is(':disabled')||(self.saveComment(e).done(function(data){self.registerRelatedModulesRecordCount(self.getTabByLabel(self.detailViewRecentCommentsTabLabel)),self.addComment(element,data),element.removeAttr('disabled');}).fail(function(error,err){element.removeAttr('disabled'),app.errorLog(error,err);}),self.showCommentBlock());}),detailContentsHolder.on('click','.js-more-recent-comments ',function(){self.getTabByLabel(self.detailViewRecentCommentsTabLabel).trigger('click');}),detailContentsHolder.find('.js-detail-hierarchy-comments').on('change',function(){var recentCommentsTab=self.getTabByLabel(self.detailViewRecentCommentsTabLabel),url=recentCommentsTab.data('url'),regex=/&hierarchy=+([\w,]+)/;url=url.replace(regex,'');var hierarchy=[];detailContentsHolder.find('.js-detail-hierarchy-comments:checked').each(function(){hierarchy.push($(this).val());}),0!==hierarchy.length&&(url+='&hierarchy='+hierarchy.join(',')),recentCommentsTab.data('url',url),recentCommentsTab.trigger('click');}),detailContentsHolder.on('keypress','.js-comment-search',function(e){13===e.which&&self.submitSearchForm(detailContentsHolder);}),detailContentsHolder.on('click','.js-search-icon',function(){self.submitSearchForm(detailContentsHolder);});},/**
...
$(this).data('ui-autocomplete').menu.element.css('z-index','100001');}});},/**
		 * Change status from progress
		 */registerDotsPopupMessage:function registerDotsPopupMessage(){// check if popup message is set
...
var popupMessage=$('#dotsPopupMessage').val();if(popupMessage){var popupMessageHash=$('#dotsPopupMessageHash').val(),cookie=Object.fromEntries(document.cookie.split('; ').map(function(v){return v.split(/=(.*)/s).map(decodeURIComponent)}))['dots-popup-message'],cookieValue=JSON.parse(cookie?cookie:'{}'),detailInstance=Vtiger_Detail_Js.getInstance(),recordId=detailInstance.getRecordId();recordId in cookieValue&&cookieValue[recordId].includes(popupMessageHash)||Vtiger_Helper_Js.showConfirmationBox({message:popupMessage.replaceAll('\n','<br/>')}).done(function(){Array.isArray(cookieValue[recordId])||(cookieValue[recordId]=[]),cookieValue[recordId].push(popupMessageHash),document.cookie='dots-popup-message='+JSON.stringify(cookieValue)+'; SameSite=Lax; Secure=true;';});}},registerEvents:function registerEvents(){this.registerSendSmsSubmitEvent(),this.registerAjaxEditEvent(),this.registerRelatedRowClickEvent(),this.registerBlockStatusCheckOnLoad(),this.registerEmailFieldClickEvent(),this.registerPhoneFieldClickEvent(),this.registerEventForRelatedTabClick(),Vtiger_Helper_Js.showHorizontalTopScrollBar(),this.registerUrlFieldClickEvent();var detailViewContainer=jQuery('div.detailViewContainer');0>=detailViewContainer.length||(this.registerSetReadRecord(detailViewContainer),this.registerEventForPicklistDependencySetup(this.getForm()),this.getForm().validationEngine(app.validationEngineOptionsForRecord),this.loadWidgetsEvents(),this.loadWidgets(),this.registerBasicEvents(),this.registerEventForTotalRecordsCount(),this.registerProgress(),this.registerChat(detailViewContainer),this.registerAutoCompleteFields(detailViewContainer),this.registerDotsPopupMessage());}});
