...
jQuery.Class(
	...
	},
	{
		...
		registerAssign: function registerAssign() {
			var thisInstance = this;
			var assignBtn = thisInstance.getContainer().find('[name="dassign"]');
			assignBtn.on('click', function () {
				var url = assignBtn.data('url');
				AppConnector.request(url).done(function (response) {
					if (response.result.url) {
						window.location.href = response.result.url;
					} else if (response.result.notify) {
	...
	{},
	{
		...
		loadCalendarData: function (allEvents) {
			...
			AppConnector.request(params).done(function (events) {
				...
				thisInstance
					.getCalendarView()
					.find('.fc-scroller.fc-day-grid-container')
					.css({
						height: 'auto',
						overflow: 'hidden',
					});
...
YetiForce_Calendar_Widget_Js(
	'YetiForce_WeeklyCalendar_Widget_Js',
	{},
	{
		registerCalendar: function () {
			var thisInstance = this;
			var userDefaultActivityView = 'agendaWeek';
			var container = thisInstance.getContainer();
			//Default time format
			var userDefaultTimeFormat = CONFIG.hourFormat;
			if (userDefaultTimeFormat == 24) {
