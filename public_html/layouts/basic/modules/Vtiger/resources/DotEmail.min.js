...
$.Class('Vtiger_DotEmail_Js',{/*
	 ...
	 */registerPreSubmitEvent:function registerPreSubmitEvent(container){var _this=this;container.find('#send_dot_email').on('click',function(e){e.preventDefault();var loader=$.progressIndicator({message:app.vtranslate('JS_DOT_EMAIL_SENDING'),position:'html',blockInfo:{enabled:!0}}),templateElements=container.find('[name="email_template[]"]'),templateContainer=$(_this).closest('.js-dot-email-template-content');templateContainer.find('.js-dot-email-user-variable').toggleClass('d-none');var templates=[];templateElements.filter(':checked').each(function(){templates.push($(this).val());});var params=container.find('form').serializeFormData();params.templates=templates,params.mode='send',AppConnector.request(params).done(function(data){console.log(data);var response=data.result;response.message&&app.showNotify({text:response.message,type:data.success?'success':'error'});}).fail(function(data,err){app.errorLog(data,err);}),loader.progressIndicator({mode:'hide'}),app.hideModalWindow();});},/**
	 * Register list view check records
	 *
	 * @param   {jQuery}  container
	 */registerListViewCheckRecords:function registerListViewCheckRecords(container){var templateElements=container.find('[name="email_template[]"]');templateElements.on('change',function(){document.progressLoader=$.progressIndicator({message:app.vtranslate('JS_DOT_EMAIL_RECALCULATING'),position:'html',blockInfo:{enabled:!0}});var templateContainer=$(this).closest('.js-dot-email-template-content');templateContainer.find('.js-dot-email-user-variable').toggleClass('d-none');var templates=[];templateElements.filter(':checked').each(function(){templates.push($(this).val());});var params=container.find('form').serializeFormData();params.mode='validateRecords',params.templates=templates,AppConnector.request(params).done(function(data){var response=data.result;if(data.success){var valid=response.valid,info=container.find('.js-records-info').text(response.message).removeClass('d-none');valid&&info.addClass('d-none'),setTimeout(function(){document.progressLoader.progressIndicator({mode:'hide'});},500),container.find('.js-submit-button').each(function(){$(this).attr('disabled',!valid);});}}).fail(function(data,err){app.errorLog(data,err);});});},/**
