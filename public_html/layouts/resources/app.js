	...
	Components: {
		...
		QuickCreate: {
			...
			registerPostLoadEvents(form, params) {
				...
				const isFromQuickCreate = params.isFromQuickCreate || false;
				form.on('submit', (e) => {
...
var app = (window.app = {
	...
	registerPopoverRecord: function (
		...
		app.showPopoverElementView(selectElement, {
			...
			placement: 'right',
			callbackShown: () => {
				if (!selectElement.attr('href') && !selectElement.parent().attr('data-recordurl')) {
					return false;
				}
				let urlAttr = selectElement.attr('href') ? selectElement.eq(0).attr('href') : selectElement.parent().attr('data-recordurl');
				let link = new URL(urlAttr, window.location.origin);
				...
				if (typeof cacheData !== 'undefined') {
					...
					let queued = window.popoverQueue[url];
					if (typeof queued === 'undefined') {
						window.popoverQueue[url] = true;
						AppConnector.request(url).done((data) => {
							window.popoverCache[url] = data;
							delete window.popoverQueue[url];
	...
	registerBlockAnimationEvent: function (container = false) {
		...
		let blockHeader = detailViewContentHolder.find('.blockHeader:not(.has-toggle-event)');
		if (container !== false) {
			blockHeader = container.find('.blockHeader:not(.has-toggle-event)');
		}
		blockHeader.addClass('has-toggle-event');
	...
	setCookie: function (c_name, value, exdays) {
		...
		var c_value = escape(value) + (exdays == null ? '' : '; SameSite=Lax; Secure=true; expires=' + exdate.toUTCString());
	...
	registerPopover(container = $(document)) {
		...
		container.on(
			'mouseenter',
			'.js-popover-tooltip, .js-popover-tooltip--record, .js-popover-tooltip--ellipsis, [data-field-type="reference"], [data-field-type="multireference"], tr.listViewEntries[data-recordurl!=""][data-recordurl] td:nth-child(n+4):not(:has(a.js-popover-tooltip--record))',
			(e) => {
				let currentTarget = $(e.currentTarget);
				if (currentTarget.find('.js-popover-tooltip--record').length && !currentTarget.hasClass('listViewEntryValue')) {
					return;
				}
				if (!currentTarget.hasClass('popover-triggered')) {
					if (currentTarget.hasClass('js-popover-tooltip--record')) {
					...
					} else if (!currentTarget.hasClass('js-popover-tooltip--record') && currentTarget.data('field-type') && currentTarget.find('a').length) {
						...
						} else if (currentTarget.parent().attr('data-recordurl')) {
							app.registerPopoverRecord(currentTarget, {}, container);
						} else {
