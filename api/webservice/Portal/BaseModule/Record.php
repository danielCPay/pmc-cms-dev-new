...
class Record extends \Api\Core\BaseAction
{
	...
	public function get(): array
	{
		...
		if (1 === (int) $this->controller->headers['x-raw-data']) {
			$response['rawData'] = array_filter($rawData, function ($val) {
				return $val !== '' && $val !== null;
			});
			if ($rawInventory) {
				$response['rawInventory'] = array_map(function ($arr) { return array_filter($arr, function ($val) { return $val !== '' && $val !== null; }); }, $rawInventory);
			}
		}
		return $response;
	}
