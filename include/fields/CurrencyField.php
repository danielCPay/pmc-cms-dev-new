...
class CurrencyField
{
	...
	public function getDisplayValue($user = null, $skipConversion = false, $skipFormatting = false)
	{
		...
		if (empty($displayValue) && $displayValue !== 0 && $displayValue !== '0' && $displayValue !== '0.00000000') {
			$displayValue = '';
		}
	...
	{
		if (empty($currencyValue) && $currencyValue !== '0') {
			return '';
		}
	...
	public function currencyDecimalFormat($value, $user = null)
	{
		...
		if ((!empty($value) || $value === 0 || $value === '0.00000000' || $value === '0' ) && $user->getDetail('truncate_trailing_zeros')) {
