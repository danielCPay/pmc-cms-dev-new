							...
							<div class="c-panel__body py-1">
								...
								<div class="form-group">
									...
									<div class="columnsSelectDiv col-md-12">
										...
										{assign var=USED_SELECTED_FIELDS value=[]}
										<div class="">
											<select data-placeholder="{\App\Language::translate('LBL_ADD_MORE_COLUMNS',$MODULE_NAME)}"
														...
														{foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS}
															...
															<option value="{$FIELD_MODEL->getCustomViewSelectColumnName()}"
																	...
																	{if $ELEMENT_POSITION_IN_ARRAY !== false}
																		...
																		{$USED_SELECTED_FIELDS[] = $FIELD_MODEL->getCustomViewSelectColumnName()}
																	{/if}
												...
												{foreach key=SOURCE_FIELD item=RECORD_STRUCTURE_FIELDS from=$GROUP_FIELDS}
													<optgroup label="{$GROUP_LABEL}">
														{foreach item=FIELD_MODEL from=$RECORD_STRUCTURE_FIELDS}
															...
															{assign var=ELEMENT_POSITION_IN_ARRAY value=array_search($CUSTOM_FIELD_NAME, $SELECTED_FIELDS)}
															...
															<option value="{$CUSTOM_FIELD_NAME}"
																	...
																	{if $ELEMENT_POSITION_IN_ARRAY !== false}
																		data-sort-index="{$ELEMENT_POSITION_IN_ARRAY}" selected="selected"
																		{$USED_SELECTED_FIELDS[] = $CUSTOM_FIELD_NAME}
																	{/if}
																	data-js="data-sort-index|data-field-name">
												...
												{/foreach}
												{/foreach}
												{assign var=MISSING_COLUMNS value=array_diff($SELECTED_FIELDS, $USED_SELECTED_FIELDS)}
												{if count($MISSING_COLUMNS) > 0}
													<optgroup label="{\App\Language::translate('LBL_INVALID_COLUMNS', $SOURCE_MODULE)}">
														{foreach item=$SELECTED_FIELD from=$MISSING_COLUMNS}
															<option value="{$FIELD_MODEL->getCustomViewSelectColumnName()}"
																data-field-name="{$FIELD_NAME}"
																data-sort-index="999" selected="selected"
																data-js="data-sort-index|data-field-name">
																<span>!</span>
																{$SELECTED_FIELD}
																<span>!</span>
