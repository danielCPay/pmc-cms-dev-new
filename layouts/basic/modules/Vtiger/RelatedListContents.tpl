		...
		<table class="table tableBorderHeadBody listViewEntriesTable {if $VIEW_MODEL && !$VIEW_MODEL->isEmpty('entityState')}listView{$VIEW_MODEL->get('entityState')}{/if}">
			...
			<tr class="listViewHeaders">
				...
				{foreach item=HEADER_FIELD from=$RELATED_HEADERS}
					...
					<th {if $HEADER_FIELD@last} colspan="2"{/if} nowrap class="{if isset($ORDER_BY[$HEADER_FIELD_NAME])}columnSorted {/if}u-table-column__before-block
						{if !empty($HEADER_FIELD->get('maxwidthcolumn'))} u-table-column__vw-{$HEADER_FIELD->get('maxwidthcolumn')} {/if}js-list__row" data-js="container">
