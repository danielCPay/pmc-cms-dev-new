...
{strip}
	...
	<div class="tpl-Edit-Field-Percentage input-group {$WIDTHTYPE_GROUP}">
		<input id="{$MODULE}_editView_fieldName_{$FIELD_MODEL->getName()}" tabindex="{$FIELD_MODEL->getTabIndex()}" title="{\App\Language::translate($FIELD_MODEL->getFieldLabel(), $MODULE)}" class="input-medium form-control textAlignRight" min="0" max="100" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true} required,{/if}funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" name="{$FIELD_MODEL->getFieldName()}"
