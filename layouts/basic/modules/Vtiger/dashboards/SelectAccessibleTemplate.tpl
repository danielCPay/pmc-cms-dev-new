...
{strip}
	...
	<div class="input-group input-group-sm">
		...
		<select class="widgetFilter select2 owner form-control" aria-label="{\App\Language::translate('LBL_OWNER')}"
			...
			{/if}
			{if in_array('minegroups', $ACCESS_OPTIONS.available)}
				<option value="minegroups" data-name=""
						title="{\App\Language::translate('LBL_MINE_GROUPS')}" {if $OWNER eq 'minegroups' || $ACCESS_OPTIONS['default'] eq 'minegroups'} selected {/if}>{\App\Language::translate('LBL_MINE_GROUPS')}</option>
			{/if}
