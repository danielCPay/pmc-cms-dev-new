		...
		<div class="col-md-4">
			<select class="{if empty($NOCHOSEN)}select2{/if} form-control" name="columnname"
					...
					<optgroup label='{\App\Language::translate('LBL_SPECIAL_OPTIONS')}'>
						{assign var=FIELD_INFO value=['name' => 'current-role', 'label' => 'Current Role' ]}
						<option value="current-role"
							data-fieldtype="V" data-field-name="current-role"
							{if isset($CONDITION_INFO['columnname']) && "current-role" eq App\Purifier::decodeHtml($CONDITION_INFO['columnname'])}
								{assign var=FIELD_TYPE value="role"}
							...
							{/foreach}
							{$FIELD_INFO['picklistvalues'] = $ROLESLIST}
							{$FIELD_INFO['type'] = 'role'}
							data-fieldinfo='{\App\Purifier::encodeHtml(\App\Json::encode($FIELD_INFO))}'>
							{\App\Language::translate('LBL_SPECIAL_CURRENT_ROLE')}
						</option>
					</optgroup>
