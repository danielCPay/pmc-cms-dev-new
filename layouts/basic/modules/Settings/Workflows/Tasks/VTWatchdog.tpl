		...
		<div class="col-md-9">
			<select class="select2 form-control" name="recipients" data-validation-engine="validate[required]">
				...
				<optgroup label="{\App\Language::translate('LBL_SPECIAL_OPTIONS')}">
					{assign var=USER_FIELDS value=\VTWorkflowUtils::getOwnerFields($MODULE_MODEL, true)}
					{foreach from=$USER_FIELDS key=FIELD_NAME item=FIELD_LABEL}
						<option {if $FIELD_NAME eq $TASK_OBJECT->recipients}selected="selected"{/if} value="{$FIELD_NAME}">
							{$FIELD_LABEL}
						</option>
					{/foreach}
					<option {if 'special-current-user' eq $TASK_OBJECT->recipients}selected="selected"{/if} value="special-current-user">
						{\App\Language::translate('LBL_USER_FROM_CURRENT', $SOURCE_MODULE)}
					</option>
				</optgroup>
			</select>
		</div>
