			...
			<span class="col-md-9">
				<select name="assigned_user_id" class="select2 form-control" data-select="allowClear">
					...
					<optgroup label="{\App\Language::translate('LBL_SPECIAL_OPTIONS')}">
						...
						{/if}
						{assign var=USER_FIELDS value=\VTWorkflowUtils::getOwnerFields($MODULE_MODEL)}
						{assign var=USER_OPTIONS value=[]}
						{foreach from=$USER_FIELDS key=FIELD_NAME item=FIELD_LABEL}
							{$USER_OPTIONS["fromField-`$FIELD_NAME`"] = "`$USER_LABEL` `$FIELD_LABEL`"}
						{/foreach}
						{assign var=ACCESSIBLE_ROLES value=Settings_Roles_Record_Model::getAll()}
						{assign var=ROLE_OPTIONS value=[]}
						{assign var=ROLE_LABEL value=\App\Language::translate('LBL_USER_FROM_ROLE',$QUALIFIED_MODULE)}
						{foreach from=$ACCESSIBLE_ROLES key=ROLE_ID item=ROLE_MODEL}
							{assign var=ROLE_NAME value=\App\Language::translate($ROLE_MODEL->getName(),'Users')}
							{$ROLE_OPTIONS["fromRole-`$ROLE_ID`"] = "`$ROLE_LABEL` `$ROLE_NAME`"}
						{/foreach}
						{assign var=ACCESSIBLE_GROUPS value=\VTWorkflowUtils::getUserGroups()}
						{assign var=GROUP_OPTIONS value=[]}
						{assign var=GROUP_LABEL value=\App\Language::translate('LBL_USER_FROM_GROUP',$QUALIFIED_MODULE)}
						{foreach from=$ACCESSIBLE_GROUPS item=USER_GROUP}
							{assign var=USER_GROUP_NAME value=\App\Language::translate($USER_GROUP,'Users')}
							{$GROUP_OPTIONS["fromUserGroup-`$USER_GROUP`"] = "`$GROUP_LABEL` `$USER_GROUP_NAME`"}
						{/foreach}
						{foreach from=$USER_OPTIONS key=KEY item=ITEM}
							<option value="{$KEY}" {if $ASSIGNED_USER_ID eq $KEY} selected="" {/if}>$ITEM</option>
						{/foreach}
						{foreach from=$ROLE_OPTIONS key=KEY item=ITEM}
							<option value="{$KEY}" {if $ASSIGNED_USER_ID eq $KEY} selected="" {/if}>$ITEM</option>
						{/foreach}
						{foreach from=$GROUP_OPTIONS key=KEY item=ITEM}
							<option value="{$KEY}" {if $ASSIGNED_USER_ID eq $KEY} selected="" {/if}>$ITEM</option>
						{/foreach}
          </optgroup>
				</select>
