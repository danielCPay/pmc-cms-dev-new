				...
				<form class="form-horizontal createCustomFieldForm" method="POST">
					<div class="modal-body">
						...
						<div class="form-group row align-items-center supportedType preDefinedModuleList d-none">
							<div class="col-md-3 col-form-label text-right">
								<span class="redColor">*</span>&nbsp;
								{App\Language::translate('LBL_RELATION_VALUE', $QUALIFIED_MODULE)}
							</div>
							<div class="col-md-8 controls">
						...
						<div class="form-group row align-items-center supportedType preDefinedModuleFilterList d-none">
							<div class="col-md-3 col-form-label text-right">
								{App\Language::translate('LBL_RELATION_FILTER_VALUE', $QUALIFIED_MODULE)}
							</div>
							<div class="col-md-8 controls">
								<select class="referenceModuleFilter form-control" name="referenceModuleFilter">
								</select>
							</div>
						</div>
						...
						<div class="form-group row align-items-center supportedType viewRelatedFieldList d-none">
							<div class="col-md-3 col-form-label text-right">
								<span class="redColor">*</span>&nbsp;
								{App\Language::translate('LBL_VIEW_RELATED_SELECT_FIELD', $QUALIFIED_MODULE)}
							</div>
							<div class="col-md-8 controls">
								<select class="referredField form-control" name="referredField">
									...
									{/function}
									{foreach key=REFERRED_FIELD_NAME item=REFERRED_FIELD from=$SELECTED_MODULE_MODEL->getFieldsByReference()}
										<option value="{$REFERRED_FIELD_NAME}" {if empty($REFERRED_FIELD_SELECT)}selected{/if}>{$REFERRED_FIELD->getFullLabelTranslation()} ({getParentModules})</option>
						...
						<div class="form-group row align-items-center supportedType viewRelatedFieldList d-none">
							<div class="col-md-3 col-form-label text-right">
								<span class="redColor">*</span>&nbsp;
								{App\Language::translate('LBL_VIEW_RELATED_SELECT_FIELD_TO_DISPLAY', $QUALIFIED_MODULE)}
							</div>
							<div class="col-md-8 controls">
								<select class="viewField form-control" name="viewField">
									{if !empty($REFERRED_FIELD_MODULE)}
										{foreach key=VIEW_BLOCK_NAME item=VIEW_BLOCK_FIELDS from=$REFERRED_FIELD_MODULE->getFieldsByBlocks()}
											<optgroup label="{App\Language::translate($VIEW_BLOCK_NAME, $REFERRED_FIELD_MODULE_NAME)}">
												{foreach key=VIEW_FIELD_NAME item=VIEW_FIELD from=$VIEW_BLOCK_FIELDS}
													<option value="{$VIEW_FIELD_NAME}">{$VIEW_FIELD->getFullLabelTranslation()}</option>
												{/foreach}
											</optgroup>
										{/foreach}
									{/if}
