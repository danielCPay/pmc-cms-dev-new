		...
		<form class="form-horizontal fieldDetailsForm sendByAjax validateForm" method="POST">
			...
			<div class="row mx-0 mb-2 py-2 border-bottom">
				...
				<div class="col-md-6">
					...
					{if !in_array($FIELD_MODEL->getFieldDataType(),['ViewRelatedField'])}
			...
			<div class="row m-0">
				...
				<div class="col-md-6 px-1">
					...
					{/if}
					{if in_array($FIELD_MODEL->getFieldDataType(),['reference'])}
						<div class="form-group">
							<label for="referenceModuleFilter">
								<strong>{App\Language::translate('LBL_EDIT_RELATION_FILTER_VALUE', $QUALIFIED_MODULE)}</strong>
								{assign var=MODULE value=$FIELD_MODEL->getReferenceList()}
								{assign var=FILTER_VALUES value=Settings_LayoutEditor_Filter_Action::getAllFilters($MODULE[0])}
								{assign var=FIELD_PARAMS value=\App\Json::decode($FIELD_MODEL->get('fieldparams'))}
							</label>
							<div >
								<select name="referenceModuleFilter" class="form-control select2" id="referenceModuleFilter">
									{foreach key=cvid item=filter from=$FILTER_VALUES}
										<option value="{$filter.cvid}" {if isset($FIELD_PARAMS) && $FIELD_PARAMS != "" && $filter.cvid == $FIELD_PARAMS.filterId} selected {/if} >
											{$filter.viewname}
										</option>
									{/foreach}
								</select>
					...
					{if in_array($FIELD_MODEL->getFieldDataType(),['ViewRelatedField'])}
						{assign var=FIELD_PARAMS value=\App\Json::decode($FIELD_MODEL->get('fieldparams'))}
						<div class="form-group">
							<label for="referredField">
								<strong>{App\Language::translate('LBL_VIEW_RELATED_SELECT_FIELD', $QUALIFIED_MODULE)}</strong>
							</label>
							<div>
								<select class="referredField form-control" name="referredField">
									{function getParentModules}
										{foreach item=PARENT_MODULE_NAME from=$REFERRED_FIELD->getReferenceList() name=parents}{App\Language::translate($PARENT_MODULE_NAME, $PARENT_MODULE_NAME)}{if !$smarty.foreach.parents.last}, {/if}{/foreach}
									{/function}
									{foreach key=REFERRED_FIELD_NAME item=REFERRED_FIELD from=$FIELD_MODEL->getModule()->getFieldsByReference()}
										<option value="{$REFERRED_FIELD_NAME}" {if $FIELD_PARAMS.referredField == $REFERRED_FIELD_NAME}selected{/if}>{$REFERRED_FIELD->getFullLabelTranslation()} ({getParentModules})</option>
										{if $FIELD_PARAMS.referredField == $REFERRED_FIELD_NAME}
											{assign var=REFERRED_FIELD_SELECT value=$REFERRED_FIELD}
											{assign var=REFERRED_FIELD_MODULE_NAME value=current($REFERRED_FIELD_SELECT->getReferenceList())}
											{assign var=REFERRED_FIELD_MODULE value=Vtiger_Module_Model::getInstance($REFERRED_FIELD_MODULE_NAME)}
										{/if}
									{/foreach}
									{if empty($REFERRED_FIELD_SELECT)}
										{assign var=REFERRED_FIELD_SELECT value=current($FIELD_MODEL->getModule()->getFieldsByReference())}
										{assign var=REFERRED_FIELD_MODULE_NAME value=current($REFERRED_FIELD_SELECT->getReferenceList())}
										{assign var=REFERRED_FIELD_MODULE value=Vtiger_Module_Model::getInstance($REFERRED_FIELD_MODULE_NAME)}
									{/if}
								</select>
						...
						<div class="form-group">
							<label for="referredField">
								<strong>{App\Language::translate('LBL_VIEW_RELATED_SELECT_FIELD_TO_DISPLAY', $QUALIFIED_MODULE)}</strong>
							</label>
							<div>
								<select class="viewField form-control select2" name="viewField">
									{foreach key=VIEW_BLOCK_NAME item=VIEW_BLOCK_FIELDS from=$REFERRED_FIELD_MODULE->getFieldsByBlocks()}
										<optgroup label="{App\Language::translate($VIEW_BLOCK_NAME, $REFERRED_FIELD_MODULE_NAME)}">
											{foreach key=VIEW_FIELD_NAME item=VIEW_FIELD from=$VIEW_BLOCK_FIELDS}
												<option value="{$VIEW_FIELD_NAME}" {if $FIELD_PARAMS.viewField == $VIEW_FIELD_NAME}selected{/if}>{$VIEW_FIELD->getFullLabelTranslation()}</option>
											{/foreach}
										</optgroup>
									{/foreach}
								</select>
