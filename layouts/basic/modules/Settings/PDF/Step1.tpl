				...
				<div class="col-xs-12 col-xl-6 col-xxl-4 mb-2 order-1 order-xl-2 order-xxl-1">
					<div class="card">
						...
						<div class="card-body">
							...
							<div class="form-group row">
								<label class="col-sm-6 col-form-label">
									{\App\Language::translate('LBL_DOCUMENT_TYPE', 'Documents')}
								</label>
								<div class="col-sm-6 controls">
									<select class="select2 form-control" id="document_type" name="document_type">
										<optgroup label="{\App\Language::translate('LBL_NONE')}">
											<option value="">{\App\Language::translate('LBL_NONE')}</option>
										</optgroup>
										{assign var=PICKLIST_DEP value=\App\Fields\Picklist::getPicklistDependencyDatasource('Documents')}
										{assign var=VALUES_BY_AREA value=$PICKLIST_DEP['document_area']}
										{foreach from=\App\Fields\Picklist::getValuesName('document_area') key=AREA_ID item=AREA_NAME}
											<optgroup label="{\App\Language::translate($AREA_NAME, 'Documents')}">
												{foreach from=$VALUES_BY_AREA[$AREA_NAME]['document_type'] key=TYPE_ID item=TYPE_NAME}
													<option value="{$TYPE_NAME}" {if $PDF_MODEL->get('document_type') eq $TYPE_NAME} selected="selected" {/if}>
														{\App\Language::translate($TYPE_NAME, 'Documents')}
													</option>
												{/foreach}
											</optgroup>
										{/foreach}
