...
{strip}
	...
	<div class="o-calendar__form__wrapper js-calendar__form__wrapper form-horizontal modalSummaryValues" data-js="perfectscrollbar">
		...
		<div class="">
			...
			<div class="form-group row">
				<label class="col-4 mt-2 u-font-weight-500">
					{\App\Language::translate('Notes',$MODULE_NAME)}:
				</label>
				<div class="col-8 mt-2">
					{if $RECORD->get('notes') neq ''}
						{$RECORD->getDisplayValue('notes',false,true,200)}
					{else}
						<span class="muted">{\App\Language::translate('LBL_NO_DESCRIPTION',$MODULE_NAME)}</span>
					{/if}
				</div>
			</div>
			{if $RECORD->get('portfolio') neq ''}
				<div class="form-group row">
					<label class="col-4 mt-2 u-font-weight-500">
						{\App\Language::translate('SINGLE_Portolios','Portfolios')}:
					</label>
					<div class="col-8 mt-2">
						{$RECORD->getDisplayValue('portfolio')}
					</div>
				</div>
			{/if}
			{if $RECORD->get('claim') neq ''}
				<div class="form-group row">
					<label class="col-4 mt-2 u-font-weight-500">
						{\App\Language::translate('SINGLE_Claims','Claims')}:
					</label>
					<div class="col-8 mt-2">
						{$RECORD->getDisplayValue('claim')}
					</div>
				</div>
			{/if}
			{if $RECORD->get('case') neq ''}
				<div class="form-group row">
					<label class="col-4 mt-2 u-font-weight-500">
						{\App\Language::translate('SINGLE_Cases','Cases')}:
					</label>
					<div class="col-8 mt-2">
						{$RECORD->getDisplayValue('case')}
					</div>
				</div>
			{/if}
			{if not $LINK}
				...
				<div class="form-group row">
					<label class="col-4 mt-2 u-font-weight-500">
						{\App\Language::translate('Note',$MODULE_NAME)}:
					</label>
					<div class="col-8 mt-2">
						{if $RECORD->get('note') neq ''}
							{$RECORD->getDisplayValue('note',false,true,200)}
						{else}
							<span class="muted">{\App\Language::translate('LBL_NO_NOTE',$MODULE_NAME)}</span>
						{/if}
					</div>
				</div>
			{/if}
