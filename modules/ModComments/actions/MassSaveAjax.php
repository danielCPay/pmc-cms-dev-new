...
class ModComments_MassSaveAjax_Action extends Vtiger_Mass_Action
{
	...
	private function getRecordModelsFromRequest(App\Request $request)
	{
		...
		$rawRequest = $request->getAllRaw();
		$rawRequest['module'] = $rawRequest['source_module'];
		$requestForRecords = new \App\Request($rawRequest);

		$recordIds = static::getRecordsListFromRequest($requestForRecords);
		$recordModels = [];
