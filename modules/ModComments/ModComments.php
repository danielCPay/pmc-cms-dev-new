...
class ModComments extends ModCommentsCore
{
	...
	{
		$comments = ModComments_Record_Model::getAllParentComments($currentParentId, \App\Record::getType($currentParentId));
		foreach ($comments as $commentRow) {
			$comment = ModComments_Record_Model::getInstanceById($commentRow->getId());

			$comment->set('related_to', $targetParentId);
			$comment->ext['modificationType'] = \ModTracker_Record_Model::TRANSFER_EDIT;
			$comment->save();
		}
	}
