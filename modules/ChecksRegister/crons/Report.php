<?php
/**
 * Cron for sending Checks Delay report
 *
 * @package   Cron
 *
...
/**
 * ChecksRegister_Report_Cron class.
 */
class ChecksRegister_Report_Cron extends \App\CronHandler
{
  const SERVICE_NAME = 'LBL_CHECKSREGISTER_REPORT';
  const REPORTS = ['Check Delay Report' => 'Check Delay Report'];
  ...
  public function process()
  {
    if (\App\Request::_get('service') === self::SERVICE_NAME) {
      foreach (self::REPORTS as $filterName => $templateName) {
        \App\Log::warning("ChecksRegister_Report_Cron::processing $filterName and sending using template $templateName");

        try {
          $mailerContent = [];

          $moduleName = 'ChecksRegister';
          ...
          if ($cntRecords) {
            ...
            $mailerContent['attachments'] = [$generatedFile => 'Check Delay Report.xlsx'];
          }
          ...
          $emailParser = \App\EmailParser::getInstance('ChecksRegister');
          $mailerContent['to'] = [];
          ...
          \App\Log::error("ChecksRegister_Report_Cron::error during generation: " . var_export($e, true));
          \VTWorkflowUtils::createBatchErrorEntryRaw("Batch Errors Sync", '', 'ChecksRegister', "Batch Errors report generation failed", null, "Please contact your administrator");
        }
      }
    }
  }
