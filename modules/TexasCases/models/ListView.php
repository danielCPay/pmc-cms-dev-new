...
/**
 * TexasCases ListView Model Class.
 */
class TexasCases_ListView_Model extends Vtiger_ListView_Model
{
    ...
    if (\App\Cache::has('TexasCases', $key)) {
      return \App\Cache::get('TexasCases', $key);
    }
		...
		\App\Cache::save('TexasCases', $key, $providers);
	...
	public function loadListViewCondition()
	{
		...
		if ($userModel->getRole() === TexasCases_ListView_Model::PROVIDER_LITIGATION_ASSISTANCE_ROLE) {
			$providerFields = TexasCases_MergeRecordsSpecial_View::getProviderFields();
			$providers = TexasCases_ListView_Model::getProviderLitigationAssists($userModel->getId());
