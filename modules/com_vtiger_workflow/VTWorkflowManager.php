...
class VTWorkflowManager
{
	 ...
	 * 
	 * Returns null if workflow does not exists.
	 * 
	 ...
	 */
	public function retrieveByName(string $name, string $moduleName)
	{
		$data = (new App\Db\Query())->from('com_vtiger_workflows')->where(['module_name' => $moduleName, 'summary' => $name])->one();
		if ($data) {
			$workflow = $this->getWorkflowInstance($data['type']);
			$workflow->setup($data);
	...
	/**
	 * Retrieve all workflows with names containing $needle.
	 * 
	 * Returns empty array if no matches
	 * 
	 * @param string $needle Name part of workflow
	 * @param string $moduleName Name of module
	 * 
	 * @return array Array of matching workflow objects
	 */
	public function retrieveAllByName(string $needle, string $moduleName)
	{
		$workflows = [];
		$data = (new App\Db\Query())->from('com_vtiger_workflows')->where(['module_name' => $moduleName])->andWhere(['like', 'summary', $needle])->all();
		foreach ($data as $workflowRow) {
			$workflow = $this->getWorkflowInstance($workflowRow['type']);
			$workflow->setup($workflowRow);
	...
	public function updateNexTriggerTime(Workflow $workflow)
	{
		...
		\App\Log::warning("Scheduled workflow {$workflow->id}: setting trigger time to $nextTriggerTime");
