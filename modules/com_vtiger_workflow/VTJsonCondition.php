...
class VTJsonCondition
{
	...
	public function evaluate($condition, Vtiger_Record_Model $recordModel)
	{
			...
			foreach ($expr as $cond) {
				...
				if (0 == \count($matches)) {
					...
					if (!empty($referenceFieldId)) {
						...
						if ('Users' !== $referenceModule && \App\Record::isExists($referenceFieldId)) {
						...
						} elseif ('Users' === $referenceModule && 'Users' === \App\Fields\Owner::getType($referenceFieldId) && \App\User::getUserModel($referenceFieldId)->isActive()) {
				...
				$expressionResults[$conditionGroup][$i]['cond'] = $cond;
		...
		// for checking reasons of not run WFs
		// if ($recordModel->getId() == 52180) {
		// if ($condition == '[{"fieldname":"date_end_of_withdrawal_period","operation":"is not empty","value":"","valuetype":"rawtext","joincondition":"and","groupjoin":"and","groupid":0},{"fieldname":"claim","operation":"is not empty","value":"","valuetype":"rawtext","joincondition":"","groupjoin":"and","groupid":0},{"fieldname":"claim","operation":"has changed","value":"","valuetype":"rawtext","joincondition":"or","groupjoin":"and","groupid":1},{"fieldname":"date_end_of_withdrawal_period","operation":"has changed","value":"","valuetype":"rawtext","joincondition":"","groupjoin":"and","groupid":1}]') {
		// 	\App\Log::warning(var_export(['finalResult' => $finalResult, 'partialResults' => $expressionResults], true));
		// }
		return $finalResult;
	}
	...
	{
		if ($cond['fieldname'] === 'current-role') { // special handling
			// check if current user role is in specified hierarchy
			$user = \App\User::getCurrentUserModel();
			if (!empty($_REQUEST) && $user->isAdmin()) {
				return true;
			} 
			if ($cond['operation'] === 'is') {
				return false !== strpos(Settings_Roles_Record_Model::getInstanceById($cond['value'])->get('parentrole'), $user->getRoleInstance()->get('parentrole'));
			} else {
				return false === strpos($user->getRoleInstance()->get('parentrole'), Settings_Roles_Record_Model::getInstanceById($cond['value'])->get('parentrole'));
			}
		} else {
			...
			if ('fieldname' === $expressionType) {
				...
				$parsedValue = \App\TextParser::getInstanceByModel($referredRecordModel ? $referredRecordModel : $recordModel)->setContent($value)->parse()->getContent();
				if ($parsedValue) {
					require_once 'modules/com_vtiger_workflow/expression_engine/include.php';
					$parser = new VTExpressionParser(new VTExpressionSpaceFilter(new VTExpressionTokenizer($parsedValue)));
					...
					case 'sharedOwner':
						if ('is' === $condition || 'is not' === $condition) {
							...
							if (!empty($value)) {
								$users = [];
								foreach ($value as $id) {
									if (str_starts_with($id, 'H')) { // ROLE
										$role = Settings_Roles_Record_Model::getInstanceById($id);
										\array_push($users, ...($role->getUsersIds()));
									} else {
										$users[] = $id;
									}
								}
								$value = array_unique($users);
							}
			...
			switch ($condition) {
				...
				case 'contains':
					...
					return false !== strpos($fieldValue, strval($value));
				case 'does not contain':
					if (empty($value)) {
						return true;
					}
				...
				case 'has changed':
					...
					if (false === $hasChanged) {
						return $recordModel->isNew() || $recordModel->shouldTriggerChanged();
					}
					return $fieldValue != $hasChanged || ($hasChanged === 0 && $fieldValue !== 0);
				...
				case 'days ago':
					...
					$trimmedFieldValue = $dataType == 'datetime' ? date('Y-m-d', strtotime($fieldValue)) : $fieldValue;
					$olderDate = date('Y-m-d', strtotime('-' . $value . ' days'));
					if ($trimmedFieldValue == $olderDate) {
						return true;
					}
				...
				case 'days later':
					...
					$trimmedFieldValue = $dataType == 'datetime' ? date('Y-m-d', strtotime($fieldValue)) : $fieldValue;
					$futureDate = date('Y-m-d', strtotime('+' . $value . ' days'));
					if ($trimmedFieldValue == $futureDate) {
						return true;
					}
				...
				case 'hours ago':
					...
					$trimmedFieldValue = $dataType == 'datetime' ? $fieldValue : ($fieldValue . ' 00:00:00');
					$olderDate = date('Y-m-d H:i:s', strtotime('-' . $value . ' hours'));
					$olderDate2 = date('Y-m-d H:i:s', strtotime('-' . ($value + 1) . ' hours'));
					if ($trimmedFieldValue <= $olderDate && $trimmedFieldValue >= $olderDate2) {
						return true;
					}
				...
				case 'hours later':
					...
					$trimmedFieldValue = $dataType == 'datetime' ? $fieldValue : ($fieldValue . ' 00:00:00');
					$futureDate = date('Y-m-d H', strtotime('+' . $value . ' hours'));
					$futureDate2 = date('Y-m-d H:i:s', strtotime('+' . ($value + 1) . ' hours'));
					if ($trimmedFieldValue >= $futureDate && $trimmedFieldValue <= $futureDate2) {
						return true;
					}
				...
				case 'has changed to':
					...
					return ($recordModel->isNew() || $recordModel->shouldTriggerChanged() || false !== $oldValue) && $recordModel->get($cond['fieldname']) == $value;
				case 'is added':
