...
class VTEmailTask extends VTTask
{
	...
	public function doTask($recordModel, $originalRecordModel = null)
	{
		...
		if ($this->fromEmail) {
			...
			if ($fromEmailDetails && \is_array($fromEmailDetails)) {
		...
		if (preg_match('/(?<account>{ACCOUNT = (?<name>.*)})/', $this->subject, $matches)) {
			$account = $matches['account'];
			$name = $textParser->setContent($matches['name'])->parse()->getContent();
			$smtp = \VTWorkflowUtils::getSmtpByName($name);
			if ($smtp) {
				$mailerContent['smtp_id'] = $smtp;
			}

			$this->subject = trim(str_replace($account, '', $this->subject));
		}
