...
class Workflow
{
	...
	public function performTasks(Vtiger_Record_Model $recordModel, ?array $tasks = null)
	{
		...
		$recordId = $recordModel->getId();
		\App\Log::warning('Workflow ' . $this->id . ' (' . $this->moduleName . '::' . $this->description . ') for record ' . $recordModel->getModule()->getName() . '/' . $recordId);
		foreach ($tm->getTasksForWorkflow($this->id) as $task) {
			if ($task->active && (null === $tasks || \in_array($task->id, $tasks))) {
				...
				if (true === (bool) $task->executeImmediately && $delay === 0) {
					try {
						try {
							\App\Cache::staticDelete('RecordModel', $recordId . ':' . $recordModel->getModuleName());
							if ($recordModel->ext['modificationType'] != \ModTracker_Record_Model::TRANSFER_EDIT) {
								// 20230116 workaround attempt for stale reads
								$db = \App\Db::getInstance();
								if ($db->getTransaction()->level == 0) {
									$db->createCommand('COMMIT')->execute();
								}
							}
							$currentRecordModel = Vtiger_Record_Model::getInstanceById($recordId, $recordModel->getModuleName());
						} catch (Exception $e) {
							\App\Log::error("Failed to get fresh record model from DB due to " . var_export($e, true));
							$currentRecordModel = new Vtiger_Record_Model($recordModel->getData());
						}
						$currentRecordModel->ext['isFromWorkflow'] = true;
						$task->doTask($currentRecordModel, $recordModel);
						unset($currentRecordModel->ext['isFromWorkflow']);
					} catch(Error $e) {
						\App\Log::error(print_r([$e, $this, $task], true));
						throw $e;
					}
				} else {
					$taskQueue->queueTask($task->id, $recordId, $delay, $task->getContents($recordModel));
				}
			}
		}
	}
	...
	/**
	 * Function to check if field is used in dependencies
	 *
	 ...
	 * @return mixed
	 */
	public static function isFieldUsed(string $moduleName, string $fieldName)
	{
		$row = (new \App\Db\Query())->from('com_vtiger_workflows')->where(['or', ['and', ['module_name' => $moduleName], ['like', 'test', '"' . $fieldName . '"']], ['like', 'test', '(' . $moduleName . ') ' . $fieldName . ')"']])->one();
		$result = $row ? $row['summary'] . ' (' . $row['workflow_id'] . ')' : false;

		if (!$result) {
			$row = (new \App\Db\Query())
				->select(['com_vtiger_workflows.workflow_id', 'com_vtiger_workflows.summary', 'com_vtiger_workflowtasks.summary as tasksummary'])
				->from('com_vtiger_workflowtasks')
				->innerJoin('com_vtiger_workflows', 'com_vtiger_workflows.workflow_id = com_vtiger_workflowtasks.workflow_id')
				->where(['or', ['like', 'task', "$moduleName::$fieldName"], ['like', 'task', '"entity_type";s:' . strlen($moduleName) . ':"' . $moduleName . '"%"' . $fieldName . '"']])
				->one();
			$result = $row ? $row['summary'] . ' (' . $row['workflow_id'] . ') task ' . $row['tasksummary'] : false;
		}
