...
class VTTaskManager
{
	...
	public function getTasksForWorkflow($workflowId)
	{
		...
		$dataReader = (new \App\Db\Query())->select(['task_id', 'workflow_id', 'task'])->from('com_vtiger_workflowtasks')->where(['workflow_id' => $workflowId])->orderBy(['task_id' => SORT_ASC])->createCommand()->query();
		$tasks = [];
