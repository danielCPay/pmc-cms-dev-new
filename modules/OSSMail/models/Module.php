	...
	public static function getComposeParam(App\Request $request)
	{
		...
		if (('Users' === $moduleName && $record === \App\User::getCurrentUserRealId()) || ('Users' !== $moduleName && !empty($record) && \App\Record::isExists($record) && \App\Privilege::isPermitted($moduleName, 'DetailView', $record))) {
			...
			if (!\in_array($moduleName, array_keys(array_merge(\App\ModuleHierarchy::getModulesByLevel(0), \App\ModuleHierarchy::getModulesByLevel(3)))) || 'Campaigns' === $moduleName) {
				...
				if (\in_array($moduleName, ['Claims', 'Cases', 'PortfolioPurchases', 'Portfolios', 'Providers'])) {
					$return['recordNumber'] = $recordNumber;
					$subject = "[$subject] ";
				} else {
					if (!empty($recordNumber) && 'OSSMailView' !== $moduleName) {
						$return['recordNumber'] = $recordNumber;
				...
				if (($templateId = $request->getInteger('template', 0)) && \App\Record::isExists($templateId, 'EmailTemplates')) {
					...
					if ($templateModel->get('email_to')) {
						$emailTo = Vtiger_Record_Model::getInstanceById($templateModel->get('email_to'))->get('result_text');
					}
          if ($templateModel->get('email_cc')) {
            $emailCc = Vtiger_Record_Model::getInstanceById($templateModel->get('email_cc'))->get('result_text');
          }
					$emailParser = \App\EmailParser::getInstanceByModel($recordModel);
          try {
						if ($emailTo) {
            ...
            unset($emailParser);
					...
					// process attachments
					$attachmentIds = $template['attachments']['ids'] ?? [];
					if (!empty($templateModel->get('having_document_type')) || !empty($templateModel->get('having_email_status'))) {
						...
						if (!empty($templateModel->get('having_document_type'))) {
							$queryGenerator->addCondition('document_type', $templateModel->get('having_document_type'), 'eid');
						}
						if (!empty($templateModel->get('having_email_status'))) {
					...
					foreach ($attachmentIds as $attachmentId) {
						$attachment = Vtiger_Record_Model::getInstanceById(trim($attachmentId));
						$fileDetails = $attachment->getFileDetails();
						$documentPath = "{$fileDetails['path']}{$fileDetails['attachmentsid']}";
						$return['filePath'][] = ['path' => $documentPath, 'name' => $attachment->get('filename')];
					}
				}
			    ...
			    $documentPath = "{$fileDetails['path']}{$fileDetails['attachmentsid']}";
					$return['filePath'] = [['path' => $documentPath, 'name' => $recordModel->get('filename')]];
				}
			}
		...
		if (!$request->isEmpty('attachments')) {
			foreach (explode(',', $request->get('attachments')) as $attachmentId) {
				$attachment = Vtiger_Record_Model::getInstanceById(trim($attachmentId));
				$fileDetails = $attachment->getFileDetails();
				$documentPath = "{$fileDetails['path']}{$fileDetails['attachmentsid']}";
				$return['filePath'][] = ['path' => $documentPath, 'name' => $attachment->get('filename')];
