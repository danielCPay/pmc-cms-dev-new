...
{
	protected static $baseUrl = 'modules/OSSMail/roundcube/';
	/**
	 * Main url.
	 *
	 * @var string
	...
	{
		if ($request->getRaw('oauth') == 1) {
			$this->mainUrl = self::$baseUrl . 'index.php?_task=login&_action=oauth';
			foreach ($request->getAllRaw() as $key => $value) {
				if (\in_array($key, ['module', 'view', 'oauth'])) {
					continue;
				}
				$this->mainUrl .= "&$key=$value";
