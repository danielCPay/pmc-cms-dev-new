...
/**
 * Class CaliforniaCases_Record_Model.
 */
class CaliforniaCases_Record_Model extends Vtiger_Record_Model
{
  /**
  ...
  public function recalculateAll() {
    ...
    \App\Log::warning("CaliforniaCases::recalculateAll:$id/$lockAutomation");

    if (!$lockAutomation) {
      \App\Log::warning("CaliforniaCases::recalculateAll:recalculateMethods = [" . implode(', ', $this->recalculateFunctions) . "]");
  ...
  public function recalculateFromClaims() {
    $id = $this->getId();
    $lockAutomation = $this->get('lock_automation');
    \App\Log::warning("CaliforniaCases::recalculateFromClaims:$id/$lockAutomation");
    ...
    if (!$lockAutomation) {
      ...
      \App\Log::warning("CaliforniaCases::recalculateFromClaims:user is $currentUserId, should be $targetUserId");
      if ($targetUserId != $currentUserId) {
        \App\Log::warning("CaliforniaCases::recalculateFromClaims:setting user to $targetUserId");
      ...
      if ($targetUserId != $currentUserId) {
        \App\Log::warning("CaliforniaCases::recalculateFromClaims:resetting user to $currentUserId");
        \App\User::setCurrentUserId($currentUserId);
        if ($currentBaseUserId) {
          \App\Log::warning("CaliforniaCases::recalculateFromClaims:resetting base user to $currentBaseUserId");
  ...
  public function recalculateFromCollections() {
    $id = $this->getId();
    $lockAutomation = $this->get('lock_automation');
    \App\Log::warning("CaliforniaCases::recalculateFromCollections:$id/$lockAutomation");
  ...
  public function recalculateFromOthers() {
    $id = $this->getId();
    $lockAutomation = $this->get('lock_automation');
    \App\Log::warning("CaliforniaCases::recalculateFromOthers:$id/$lockAutomation");
  ...
  public function recalculateSettlementNegotiations() {
    $id = $this->getId();
    $lockAutomation = $this->get('lock_automation');
    \App\Log::warning("CaliforniaCases::recalculateSettlementNegotiations:$id/$lockAutomation");
  ...
  public function calculatePMCCollectionsLimit(?int $providerId = null) {
    ...
    \App\Log::warning("CaliforniaCases::calculatePMCCollectionsLimit:$id/$providerId"); 
  ...
  public function recalculateFromCase() {
    $id = $this->getId();
    $lockAutomation = $this->get('lock_automation');
    \App\Log::warning("CaliforniaCases::recalculateFromCase:$id/$lockAutomation");
  ...
  public function findSimilarCases() {
    $id = $this->getId();
		...
		\App\Log::warning("CaliforniaCases::findSimilarCases:$id");

    // get cases
    $queryGenerator = (new \App\QueryGenerator('CaliforniaCases'));
    ...
    if (!$anyCondition) {
      $allCases = [];
      \App\Log::warning("CaliforniaCases::findSimilarCases:no conditions");
    } else {
      ...
      \App\Log::warning("CaliforniaCases::findSimilarCases:all cases = " . count($allCases) . ", query = " . $query->createCommand()->getRawSql());
    }
    ...
    if (count($allCases) > 1) {
      ...
      $similarCasesId = (new \App\QueryGenerator('SimilarCaliforniaCases'))
        ->setField('id')
      ...
      if (empty($similarCasesId)) {
        $similarCases = Vtiger_Record_Model::getCleanInstance('SimilarCaliforniaCases');
        ...
        foreach (VTWorkflowUtils::getAllRelatedRecords($similarCases, 'CaliforniaCases') as $caseRow) {
      ...
      foreach (array_unique($similarCasesToCheck) as $similarCaseToCheckId) {
        ...
        $cases = VTWorkflowUtils::getAllRelatedRecords($similarCaseToCheck, 'CaliforniaCases');
        if (count($cases) <= 1) {
