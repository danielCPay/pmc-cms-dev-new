 ...
 */
class CaliforniaCasesWorkflow
{
	/**
	 * RECALCULATE_FROM_CLAIMS algorigthm
	 *
	 * @param \CaliforniaCases_Record_Model $recordModel
	 */
	public static function recalculateFromClaims(Vtiger_Record_Model $recordModel)
	{
		$id = $recordModel->getId();

		\App\Log::warning("CaliforniaCases::Workflows::recalculateFromClaims:$id");
	 ...
	 *
	 * @param \ColoradoCases_Record_Model $recordModel
	 */
	public static function recalculateFromCollections(Vtiger_Record_Model $recordModel)
	{
		$id = $recordModel->getId();

		\App\Log::warning("CaliforniaCases::Workflows::recalculateFromCollections:$id");
	...
	public static function recalculateSettlementNegotiations(Vtiger_Record_Model $recordModel)
	{
		...
		\App\Log::warning("CaliforniaCases::Workflows::recalculateSettlementNegotiations:$id");
	...
	public static function recalculateFromCase(Vtiger_Record_Model $recordModel)
	{
		...
		\App\Log::warning("CaliforniaCases::Workflows::recalculateFromCase:$id");
	...
	public static function recalculateAll(Vtiger_Record_Model $recordModel)
	{
		...
		\App\Log::warning("CaliforniaCases::Workflows::recalculateAll:$id");
	...
	public static function updateNextHearingDate(Vtiger_Record_Model $recordModel)
	{
		...
		\App\Log::warning("CaliforniaCases::Workflows::updateNextHearingDate:" . $id);
		...
		if (!$recordModel->get('lock_automation')) {
			...
			if ($next !== null) {
				...
				\App\Log::trace("CaliforniaCases::updateNextHearingDate:next_hearing_date = $nextDate");
	...
	public static function calculateStatusAge(Vtiger_Record_Model $recordModel)
	{
		...
		\App\Log::warning("CaliforniaCases::Workflows::calculateStatusAge:" . $id);
		...
		\App\Log::trace("CaliforniaCases::Workflows::calculateStatusAge:status_age = $statusAge");
		...
		\App\Log::trace("CaliforniaCases::Workflows::calculateStatusAge:settl_negot_demand_age = $demandAge");
		...
		\App\Log::trace("CaliforniaCases::Workflows::calculateStatusAge:settl_negot_offer_age = $offerAge");
	...
	public static function findSimilarCases(Vtiger_Record_Model $recordModel)
	{
		...
		\App\Log::warning("CaliforniaCases::Workflows::findSimilarCases:$id");
	...
	public static function setNewCaseId(Vtiger_Record_Model $recordModel)
	{
		...
		if (\in_array($typeOfClaim, $lopClaimTypes)) {
			$newId = str_replace('CAC', 'CAL', $recordModel->getRecordNumber());
		} else if (\in_array($typeOfClaim, $hosClaimTypes)) {
			$year = date('y');
			$currentNumber = 
				(new \App\QueryGenerator('CaliforniaCases'))
					->addCondition('case_id', "CAH$year-", 's')
					->createQuery()->max("cast(regexp_replace(case_id, '^CAH$year-(\\\\d+)$', '\\\\1') as integer)") ?? 0;
			$newId = "CAH$year-" . sprintf('%06d', $currentNumber + 1);
		} else {
	...
	public static function setAttorneyByAssignedTo(Vtiger_Record_Model $recordModel)
	{
		...
		\App\Log::warning("CaliforniaCases::Workflows::setAttorneyByAssignedTo:$id/$assignedUserId");
	...
	public static function revertToPreviousStatus(Vtiger_Record_Model $recordModel)
	{
		...
		\App\Log::warning("CaliforniaCases::Workflows::revertToPreviousStatus:$id/$previousStatus/$previousStage");
