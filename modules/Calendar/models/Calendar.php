...
class Calendar_Calendar_Model extends App\Base
{
	...
	public function getQuery()
	{
		...
		if (!empty($this->get('user'))) {
			$possibleOwners = \App\PrivilegeUtil::getAllGroupsByUser($this->get('user'));
			$possibleOwners = array_merge($possibleOwners, $this->get('user'));
			$conditions[] = ['vtiger_crmentity.smownerid' => $possibleOwners];
	 ...
	 * @return array
	 */
	public function getEntity($overrideExtended = null)
	{
		$return = [];
		$currentUser = \App\User::getCurrentUserModel();
		$moduleModel = Vtiger_Module_Model::getInstance($this->getModuleName());
		$extended = $overrideExtended !== null ? $overrideExtended : 'Extended' === App\Config::module('Calendar', 'CALENDAR_VIEW');
