...
class Calendar_Record_Model extends Vtiger_Record_Model
{
	...
	{
		if (\in_array($key, ['date_start', 'due_date']) && strpos($value, ' ') !== false) {
			[$date, $time] = explode(' ', $value);
			switch($key) {
				case 'date_start':
					parent::set('date_start', $date);
					parent::set('time_start', $time);
					break;
				case 'due_date':
					parent::set('due_date', $date);
					parent::set('time_end', $time);
					break;
				default:
					break;
			}
	...
	{
		// if empty end time, set to 30 minutes from start time
		if ($this->isEmpty('due_date')) {
			$dateStart = $this->get('date_start');
			$timeStart = $this->get('time_start');

			$endDateTime = strtotime('+30 minute', strtotime($dateStart . ' ' . $timeStart));
			$this->set('due_date', date('Y-m-d', $endDateTime));
			$this->set('time_end', date('H:i:s', $endDateTime));
		}
		parent::saveToDb();
