...
class Documents_MassAdd_Action extends Vtiger_Mass_Action
{
	...
	public function process(App\Request $request)
	{
		...
		$sourceRecordId = $request->get('return_id');
		$relationModel = Vtiger_Relation_Model::getInstance(
			Vtiger_Module_Model::getInstance($request->get('return_module')), 
			Vtiger_Module_Model::getInstance('Documents'),
			$request->get('relation_id')
		);
		$documentTypeId = $request->get('document_type');
		if (!\App\Record::isExists($documentTypeId, 'DocumentTypes')) {
		...
		foreach ($_FILES as $file) {
			...
			for ($i = 0; $i < $countFiles; ++$i) {
				...
				if ($documentTypeId) {
					$recordeModel->set('document_type', $documentTypeId);
				}
				if ($relationModel->getRelationType() == Vtiger_Relation_Model::RELATION_O2M && !empty($relationModel->getRelationField())) {
					$recordeModel->set($relationModel->getRelationField()->getName(), $sourceRecordId);
				}

				$recordeModel->save();

				if ($relationModel->getRelationType() != Vtiger_Relation_Model::RELATION_O2M || empty($relationModel->getRelationField())) {
					$relationModel->addRelation($sourceRecordId, $recordeModel->getId());
				}
			}
		}
