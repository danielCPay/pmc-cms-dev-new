	...
	public function read()
	{
		...
		foreach ($this->data as $i => $data) {
			...
			foreach ($fieldMapping as $fieldName => $index) {
				...
				} else if (preg_match('/^[0-1]?[0-9]\/[0-3]?[0-9]\/20[0-9]{2}$/', $fieldValueTemp) === 1) {
					// use us dates by default
					[$m, $d, $y] = explode('/', $fieldValueTemp);
					$fieldValue = "$y-$m-$d";
				} else if (preg_match('/^[0-3]?[0-9]\/[0-1]?[0-9]\/20[0-9]{2}$/', $fieldValueTemp) === 1) {
					// reformat european date
					[$d, $m, $y] = explode('/', $fieldValueTemp);
					$fieldValue = "$y-$m-$d";
				}
