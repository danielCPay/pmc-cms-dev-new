	...
	public function createTable()
	{
		...
		foreach ($fieldMapping as $fieldName => $index) {
			if ($field = $moduleFields[$fieldName]) {
				...
				$stringTypes = array_merge(Vtiger_Field_Model::$referenceTypes, ['owner', 'currencyList', 'sharedOwner', 'picklist', 'multipicklist']);
				if (\in_array($field->getFieldDataType(), $stringTypes)) {
					// $columns[$fieldName] = $schema->createColumnSchemaBuilder('string', 255);
					$columns[$fieldName] = $schema->createColumnSchemaBuilder('text');
				} else {
