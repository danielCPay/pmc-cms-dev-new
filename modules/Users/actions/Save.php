...
class Users_Save_Action extends Vtiger_Save_Action
{
	...
	public function checkPermission(App\Request $request)
	{
		...
		if (!$request->isEmpty('record', true)) {
			...
			$isAdmin = ($currentUser->isAdminUser() || \App\Security\AdminAccess::isPermitted($request->getModule()));
			if (!($isAdmin || (App\Config::security('SHOW_MY_PREFERENCES') && (int) $currentUser->getId() === $record))) {
	...
	protected function getRecordModelFromRequest(App\Request $request)
	{
		...
		$this->record->set('currency_grouping_separator', $request->getRaw('currency_grouping_separator'));
