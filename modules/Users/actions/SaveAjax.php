...
class Users_SaveAjax_Action extends Vtiger_SaveAjax_Action
{
	...
	public function checkPermission(App\Request $request)
	{
		...
		$isAdmin = ($currentUserModel->isAdminUser() || \App\Security\AdminAccess::isPermitted($request->getModule()));
		if (!$isAdmin) {
			parent::checkPermission($request);
		}
		if (!$isAdmin && (int) $currentUserModel->getId() !== $request->getInteger('record') && (int) $currentUserModel->getId() !== $request->getInteger('userid')) {
