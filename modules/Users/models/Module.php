...
class Users_Module_Model extends Vtiger_Module_Model
{
	...
	{
		$cacheKey = "$email";
		if (App\Cache::staticHas('Users_Module_Model::getUserIdByEmail', $cacheKey)) {
			return App\Cache::staticGet('Users_Module_Model::getUserIdByEmail', $cacheKey);
		}
		$query = (new \App\Db\Query())->from('vtiger_users')->where(['email1' => $email])->select(['id']);
		$userId = $query->scalar();
		App\Cache::staticSave('Users_Module_Model::getUserIdByEmail', $cacheKey, $userId);
		return $userId;
	}
