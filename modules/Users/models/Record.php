...
class Users_Record_Model extends Vtiger_Record_Model
{
	...
	public function isEditable()
	{
		if (!isset($this->privileges['isEditable'])) {
			$isPermittedEditView = \App\Security\AdminAccess::isPermitted('Users') || $this->isPermitted('EditView');
			return $this->privileges['isEditable'] = $isPermittedEditView && !$this->isLockByFields() && false === Users_Privileges_Model::checkLockEdit($this->getModuleName(), $this) && empty($this->getUnlockFields()) && !$this->isReadOnly();
		}
	...
	public function isCreateable()
	{
		if (!isset($this->privileges['isCreateable'])) {
			$this->privileges['isCreateable'] = \App\Security\AdminAccess::isPermitted('Users') || $this->getModule()->isPermitted('CreateView');
		}
	...
	public static function getRoleDetailByUserId($id) {
		$privilegesModel = Users_Privileges_Model::getInstanceById($id);
		return Settings_Roles_Record_Model::getInstanceById($privilegesModel->get('roleid'));
	}
