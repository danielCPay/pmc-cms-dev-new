...
class Users_DetailView_Model extends Vtiger_DetailView_Model
{
	...
	public function getDetailViewLinks(array $linkParams): array
	{
		...
		/** @var Users_Record_Model $recordModel */
		$recordModel = $this->getRecord();
		$recordId = $recordModel->getId();
		$linkModelList['DETAIL_VIEW_BASIC'] = [];
		if ((
				true === $currentUserModel->isAdminUser() 
				|| \App\Security\AdminAccess::isPermitted($recordModel->getModule()->getName())
				|| $currentUserModel->get('id') === $recordId
			) 
			&& 'Active' === $recordModel->get('status')
		) {
			$detailViewLinks = [];
			if (true === $currentUserModel->isAdminUser()	|| $currentUserModel->get('id') === $recordId) {
				$detailViewLinks[] = [
			...
			if (true === $currentUserModel->isAdminUser() || \App\Security\AdminAccess::isPermitted($recordModel->getModule()->getName())) {
				$detailViewLinks[] = [
					'linktype' => 'DETAIL_VIEW_ADDITIONAL',
			...
			if (true === $currentUserModel->isAdminUser()	&& $currentUserModel->getId() !== $recordId) {
				$detailViewLinks[] = [
					'linktype' => 'DETAIL_VIEW_ADDITIONAL',
