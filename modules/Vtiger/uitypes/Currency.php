...
class Vtiger_Currency_UIType extends Vtiger_Base_UIType
{
	...
	{
		if ((empty($value) && $value !== 0 && $value !== '0') || isset($this->validate[$value])) {
			return;
		}
		if ($isUserFormat) {
	...
	{
		if (!$value && $value !== 0 && $value !== '0' && $value !== '0.00000000' && $value !== 0.0) {
			return '';
		}
	...
	{
		if (!empty($value)) {
			$originalEdit = $this->edit;
			$this->edit = true;
			$value = $this->getDisplayValue($value);
			$this->edit = $originalEdit;
			return $value;
		}
		return $value;
	}
	...
	public function getDetailViewDisplayValue($value, $recordId, $uiType)
	{
		...
		} else if (preg_match('/_chf$/', $this->getFieldModel()->getName())){
			$currencySymbol = 'CHF';
		} else {
	...
	public function getTextParserDisplayValue($value, Vtiger_Record_Model $recordModel, $params)
	{
		...
		return html_entity_decode($this->getDisplayValue($value, $recordModel->getId(), $recordModel, isset($params['raw']) ? ((bool) $params['raw']) : true));
	}
