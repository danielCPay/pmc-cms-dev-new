...
class Vtiger_GetDependentsList_Relation implements RelationInterface
{
	...
	public function transfer(int $relatedRecordId, int $fromRecordId, int $toRecordId): bool
	{
		...
		if ($relationFieldModel && $relationFieldModel->isEditable()) {
			...
			// 20230109 MK According to https://github.com/dotsystemsspzoo/hestia-pdss/issues/314 removed requirement to have edit access to related modules to do merge
			// by replacing $recordModel->isEditable with checks from that method except one for permissions
			// if ($recordModel->isEditable() && $recordModel->get($relationFieldModel->getName()) === $fromRecordId) {
			if ((!$recordModel->isLockByFields() && false === Users_Privileges_Model::checkLockEdit($recordModel->getModuleName(), $recordModel) && empty($recordModel->getUnlockFields()) && !$recordModel->isReadOnly()) && $recordModel->get($relationFieldModel->getName()) === $fromRecordId) {
