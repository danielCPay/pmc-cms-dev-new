...
class Vtiger_PDF_Model extends \App\Base
{
	...
	/**
	 * Returns unique template names by module name.
	 *
	 * @param string $moduleName - module name for which template was created
	 *
	 * @return array of template names
	 */
	public static function getTemplateNamesByModule($moduleName)
	{
		$templates = 
			(new \App\Db\Query())
			->select(['primary_name'])
	 ...
	 * @return int|null
	 */
	public static function getIdByName(string $name, int $recordId): ?int
	{
		if (!\App\Record::isExists($recordId)) {
			return null;
		}
		...
		$query = (new \App\Db\Query())
			->select('pdfid')
			->from(self::$baseTable)
			->where(['primary_name' => $name, 'module_name' => $moduleName, 'status' => 1]);
		$templates = $query->column();
		foreach ($templates as $templateId) {
			$template = Vtiger_PDF_Model::getInstanceById($templateId);
			if ($template->isVisible('Detail') && $template->checkFiltersForRecord($recordId) && $template->checkUserPermissions()) {
				$templateIds[] = $templateId;
			}
		}

		if (empty($templateIds)) {
			throw new Error("No template '$name' matches record $recordId");
		} else if (count($templateIds) > 1) {
			throw new Error("Multiple templates '$name' (" . join(', ', $templateIds) . ") match record $recordId");
		}
	...
	public static function getNameById(int $id): string
	{
		...
		return $template->get('primary_name');
	}
	 ...
	 *
	 * @param int    $recordId   - id of a record
	 * @param int|string $templateId - id of pdf template
	...
	public static function exportToPdf($recordId, $templateId, $filePath = '', $saveFlag = '')
	{
		if (!\is_numeric($templateId)) {
			$templateId = Vtiger_PDF_Model::getIdByName($templateId, $recordId);
		}
	...
	public function parseVariables(string $str)
	{
		if ($str) {
			// process IF
			$str = \App\Utils\Completions::processIfs($str, $this->getParser());
			
			// process TextParser
			$str = $this->getParser()->setContent($str)->parse()->getContent();

			// process SPELLOUT[number]
			$str = \App\Utils\Completions::processSpellout($str);
