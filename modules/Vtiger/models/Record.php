...
class Vtiger_Record_Model extends \App\Base
{
	...
	public $isNew = true;
	public $isMovedToTrash = false;
	public $shouldTriggerChanged = false;
	public $ext = [];
	...
	/**
	 * Should trigger "has changed" conditions in VTJsonCondition.
	 *
	 * @return bool
	 */
	public function shouldTriggerChanged()
	{
		return $this->shouldTriggerChanged;
	}
	...
	public function set($key, $value)
	{
		...
		$uiType = $field ? $field->getUIType() : -1;
		$valueOverride = $value;
		switch ($uiType) {
			case 9: // Percent, 2 decimals
				$valueForChange = round($value, 2);
				if ($value !== null && $value !== '') {
					$valueOverride = $valueForChange;
				}
				break;
			case 71: // Currency, 5 decimals
				$valueForChange = round($value, 5);
				$user = \App\User::getCurrentUserModel();
				$numDecimals = (empty($user->getDetail('no_of_currency_decimals')) && 0 !== (int) $user->getDetail('no_of_currency_decimals')) ? 2 : (int) $user->getDetail('no_of_currency_decimals');
				if ($value !== null && $value !== '') {
					$valueOverride = round($value, $numDecimals);
					if ($this->value[$key] !== null && $this->value[$key] !== '') {
						$this->value[$key] = round($this->value[$key], 5);
					}
				}
				break;
			case 1: // text
		...
		if (!$this->isNew 
			&& !\in_array($key, ['mode', 'id', 'newRecord', 'modifiedtime', 'modifiedby', 'createdtime']) 
			&& (
				$this->value[$key] != $valueForChange 
				|| ($this->value[$key] === '' && $value !== '' && $value !== NULL) 
				|| ($value === '' && $this->value[$key] !== '' && $this->value[$key] !== NULL)
				|| ($this->value[$key] === NULL && $value !== '' && $value !== NULL) 
				|| ($value === NULL && $this->value[$key] !== '' && $this->value[$key] !== NULL)
				|| (\is_string($this->value[$key]) && \is_string($valueForChange) && $this->value[$key] !== $valueForChange)
			)) {
			$this->changes[$key] = $this->get($key);
		}
		$this->value[$key] = $valueOverride;
		return $this;
	}
	...
	private static function saveValueLog($key, $oldValue, $value) {
		$processedOldValue = $oldValue;
		if (\is_array($processedOldValue)) {
			$processedOldValue = implode(',', $processedOldValue);
		}
		$processedValue = $value;
		if (\is_array($processedValue)) {
			$processedValue = implode(',', $processedValue);
		}
		return "$key (" . substr($processedOldValue, 0, 30) . ' => ' . substr($processedValue, 0, 30) . ')';
	}
	...
	{
		\App\Log::warning("Vtiger_Record_Model::save::begin:" . $this->getModuleName() . '/' . ($this->isNew() ? 'NEW' : ('EDIT-' . $this->getId())) . ' (' . implode(', ', array_map(function ($key, $oldValue) { return static::saveValueLog($key, $oldValue, $this->get($key)); }, $this->isNew() ? array_keys($this->getData()) : array_keys($this->getChanges()), $this->isNew() ? $this->getData() : array_values($this->getChanges()))) . ')');
		...
		try {
			...
			$this->addRelations();
			$transaction->commit();
			\App\Log::warning("Vtiger_Record_Model::save::db:" . $this->getId() . ':' . $this->getModuleName());
		} catch (\Exception $e) {
		...
		\App\Log::warning("Vtiger_Record_Model::save::finish:" . $this->getId() . ':' . $this->getModuleName());
	}
	...
	public function getValuesForSave()
	{
		...
		$skipModified = false;
		if (!empty($exceptions = $this->getHandlerExceptions()) && \is_array($exceptions['disableHandlerClasses'])) {
			if (\in_array('ModTracker_ModTrackerHandler_Handler', $exceptions['disableHandlerClasses'])) {
				$skipModified = true;
			}
		}
		$forSave = $this->getEntityDataForSave($skipModified);
		if (!$this->isNew()) {
		...
		foreach ($saveFields as &$fieldName) {
			...
			if ($fieldModel) {
				...
				if (null === $value || ('' === $value && $fieldName !== 'payment_status')) {
					...
					if (!($fieldModel->isReferenceField() && empty($value))) {
	 ...
	 * 
	 * @param bool $skipModified Flag indicating setting modifiedtime and modifiedby should be skipped
	 *
	 * @return array
	 */
	public function getEntityDataForSave($skipModified = false)
	{
		$row = [];
	...
	public function addRelations(): void
	{
		...
		if ('link' === \App\Request::_get('createmode') && \App\Request::_has('return_module') && \App\Request::_has('return_id')) {
			$relationModel = Vtiger_Relation_Model::getInstance(Vtiger_Module_Model::getInstance(\App\Request::_get('return_module')), $this->getModule());
			if ($relationModel) {
				$relationModel->addRelation(\App\Request::_getInteger('return_id'), $recordId);
			}
		}
	}
	...
	{
		\App\Log::warning("Vtiger_Record_Model::delete::begin:" . $this->getModuleName() . '/' . $this->getId());
		$db = \App\Db::getInstance();
		$transaction = $db->beginTransaction();
		try {
			...
			$transaction->commit();
			\App\Log::warning("Vtiger_Record_Model::delete::db:" . $this->getId() . ':' . $this->getModuleName());
		} catch (\Exception $e) {
			$transaction->rollBack();
			throw $e;
		}
		$eventHandler->trigger('EntityAfterDelete');
		\App\Log::warning("Vtiger_Record_Model::delete::finish");
	}
	...
	public static function getInstanceById($recordId, $module = null)
	{
		...
		// 20220518 disable record cache to prevent problems in complicated WF cascades
	...
	public function setRecordFieldValues($parentRecordModel)
	{
		...
		if ($mfInstance) {
			...
			if (!empty($params['autofill'])) {
				$fieldsList = array_diff(array_keys($this->getModule()->getFields()), ['createdtime', 'created_user_id']);
	...
	public function setInventoryItemPart($itemId, string $name, $value)
	{
		if (!$this->isNew()) {
			if (is_numeric($itemId) && ($prevValue = ($this->getInventoryData()[$itemId][$name] ?? false)) != $value) {
				if (!is_numeric($prevValue) || !is_numeric($value) || (is_numeric($prevValue) && is_numeric($value) && round($prevValue, 5) != round($value, 5))) {
		...
		if ($this->isViewable()) {
			if ($this->getModule()->isSummaryViewSupported()) {
				...
				$links['LBL_SHOW_QUICK_DETAILS'] = Vtiger_Link_Model::getInstanceFromValues([
					'linklabel' => 'LBL_SHOW_QUICK_DETAILS',
					'linkhref' => 'ListPreview' !== $defaultViewName && 'TreePreview' !== $defaultViewName,
					'linkurl' => 'index.php?module=' . $this->getModuleName() . '&view=QuickDetailModal&record=' . $this->getId(),
					'linkicon' => 'far fa-caret-square-right',
					'linkclass' => 'btn-sm btn-default',
					'modalView' => true,
	...
	public function isCanAssignToHimself()
	{
		...
		$assignedUserId = $this->getValueByField('assigned_user_id');
		
		return 
			$this->isPermitted('AssignToYourself') 
			&& \App\PrivilegeUtil::MEMBER_TYPE_GROUPS === \App\Fields\Owner::getType($assignedUserId)
			&& in_array($assignedUserId, \App\PrivilegeUtil::getAllGroupsByUser($currentUserId))
			&& \array_key_exists($currentUserId, \App\Fields\Owner::getInstance($this->getModuleName())->getAccessibleUsers('', 'owner'));
	}
	...
	{
		\App\Log::warning("Vtiger_Record_Model::changeState::begin:" . $this->getModuleName() . '/' . $this->getId() . '/' . $state);
		$db = \App\Db::getInstance();
		$transaction = $db->beginTransaction();
		try {
		...
		\App\Log::warning("Vtiger_Record_Model::changeState::finish");
	}
