...
class Vtiger_Module_Model extends \vtlib\Module
{
	...
	{
		$editFields = [];
		$readOnly = \array_key_exists('isFromWorkflow', $recordModel->ext) ? $recordModel->ext['isFromWorkflow'] === true : false;
		foreach (App\Field::getFieldsPermissions($this->getId(), $readOnly) as $field) {
	...
	public function getSideBarLinks($linkParams)
	{
		...
		if (isset($_REQUEST['fixed_search_params'])) {
			$menuUrl .= '&fixed_search_params=' . \App\Request::_getByType('fixed_search_params', 'String');
		}
		...
		if ($sourceModule && ($request->has('sourceRecord') || !$request->isEmpty('sourceRecordData'))) {
			...
			foreach ($sourceModelFields as $fieldName => $fieldModel) {
				if ($fieldModel->isReferenceField()) {
					...
					if (!empty($referenceList)) {
						foreach ($referenceList as $referenceModule) {
							if (isset($fieldMap[$referenceModule]) && $sourceModule != $referenceModule && $fieldMap[$referenceModule] === $fieldName) {
