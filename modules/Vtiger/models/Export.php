...
class Vtiger_Export_Model extends \App\Base
{
	...
	public function initializeFromRequest(App\Request $request)
	{
		...
		if (!$request->isEmpty('fixed_search_params') || !$request->isEmpty('search_params')) {
			$this->queryOptions['searchParams'] = array_merge(App\Condition::validSearchParams($module, $request->getArray('search_params')), App\Condition::validSearchParams($module, $request->getArray('fixed_search_params')));
		}
	...
	public function getExportQuery()
	{
		...
		if (!empty($this->queryOptions['searchParams'])) {
			$filters = $queryGenerator->parseBaseSearchParamsToCondition($this->queryOptions['searchParams']);
			$queryGenerator->parseAdvFilter($filters);
		}
		...
		\App\Log::warning($query->createCommand()->getRawSql());
		return $query;
	}
