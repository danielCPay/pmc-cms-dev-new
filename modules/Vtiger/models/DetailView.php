	...
	public function getDetailViewLinks(array $linkParams): array
	{
		...
		if ($recordModel->isReadOnly()) {
			...
			if ($moduleModel->isPermitted('WorkflowTrigger') && $recordModel->isEditable()) {
				$tree = Vtiger_WorkflowTrigger_Model::getTreeWorkflows($moduleName, $recordId);
				$linkModelList['DETAIL_VIEW_ADDITIONAL'][] = Vtiger_Link_Model::getInstanceFromValues([
					'linktype' => 'DETAIL_VIEW_ADDITIONAL',
					'linklabel' => '',
					...
					'linkclass' => 'btn-outline-warning btn-sm js-workflow-trigger-btn' . (\count($tree) === 0 ? ' d-none' : ''),
				]);
			}
			...
			// MK 20220318 replace original PDF export
			...
			if ($moduleModel->isPermitted('ExportPdf')) {
				$additionalClass = '';
				if (!DocumentPackages_Record_Model::checkActive($moduleName, $recordModel) && !DocumentTemplates_Record_Model::checkActive($moduleName, $recordModel)) {
					$additionalClass = ' d-none';
				}
				$linkModelList['DETAIL_VIEW_BASIC'][] = Vtiger_Link_Model::getInstanceFromValues([
					'linktype' => 'DETAIL_VIEW_BASIC',
					'linklabel' => \App\Language::translate('LBL_GENERATE_DOCUMENT'),
					'dataUrl' => 'index.php?module=' . $moduleName . '&view=DotPDF&fromview=Detail&record=' . $recordId,
					'linkicon' => 'fas fa-file-pdf',
					'linkclass' => 'btn-outline-dark btn-sm showModal js-pdf' . $additionalClass,
					'title' => \App\Language::translate('LBL_GENERATE_DOCUMENT'),
				]);
			}

			$internalMailer = (int) \App\User::getCurrentUserModel()->getDetail('internal_mailer');
			if (1 === $internalMailer && App\Config::main('isActiveSendingMails') && \App\Privilege::isPermitted('OSSMail')) {
				// send email
				$additionalClass = '';
				if (!EmailTemplates_Record_Model::checkActive($moduleName, $recordModel)) {
					$additionalClass = ' d-none';
				}
				$linkModelList['DETAIL_VIEW_BASIC'][] = Vtiger_Link_Model::getInstanceFromValues([
					'linktype' => 'DETAIL_VIEW_BASIC',
					'linklabel' => \App\Language::translate('LBL_SEND_MAIL'),
					'dataUrl' => 'index.php?module=' . $moduleName . '&view=DotEmail&fromview=Detail&record=' . $recordId,
					'linkicon' => 'fas fa-envelope',
					'linkclass' => 'btn-outline-dark btn-sm showModal js-email' . $additionalClass,
					'title' => \App\Language::translate('LBL_SEND_MAIL'),
				]);

				// add link using compose url if default template exists
				$templateId = EmailTemplates_Record_Model::getDefaultTemplateId($moduleName, $recordModel);
				if (!empty($templateId)) {
					$url = OSSMail_Module_Model::getComposeUrl($moduleName, $recordId, 'Detail', 'new') . "&template=$templateId";
					$config = OSSMail_Module_Model::getComposeParameters();
					$linkModelList['DETAIL_VIEW_BASIC'][] = Vtiger_Link_Model::getInstanceFromValues([
						'linktype' => 'DETAIL_VIEW_BASIC',
						'linklabel' => \App\Language::translate('LBL_QUICK_MAIL'),
						'linkurl' => "javascript:Vtiger_Index_Js.sendMailWindow('$url', {$config['popup']});",
						'linkicon' => 'fas fa-envelope-open',
						'linkclass' => 'btn-outline-dark btn-sm',
					]);
				}
			}
