...
class Vtiger_ListView_Model extends \App\Base
{
	...
	public function getHederLinks($linkParams)
	{
		...
		/** @var OpenStreetMap_Module_Model $openStreetMapModuleModel */
	...
	public function getAdvancedLinks()
	{
		...
		// }
		// if ($moduleModel->isPermitted('ExportPdf')) {
		// 	if (DocumentPackages_Record_Model::checkActive($moduleModel->getName()) || DocumentTemplates_Record_Model::checkActive($moduleModel->getName())) {
		// 		$advancedLinks[] = [
		// 			'linktype' => 'DETAIL_VIEW_ADDITIONAL',
		// 			'linklabel' => \App\Language::translate('LBL_EXPORT_PDF'),
		// 			'linkdata' => ['url' => 'index.php?module=' . $moduleModel->getName() . '&view=DotPDF&fromview=List', 'type' => 'modal'],
		// 			'linkclass' => 'js-mass-action',
		// 			'linkicon' => 'fas fa-file-pdf',
		// 			'title' => \App\Language::translate('LBL_EXPORT_PDF')
		// 		];
		...
		if ($moduleModel->isPermitted('ExportPdf')) {
			if (DocumentPackages_Record_Model::checkActive($moduleModel->getName()) || DocumentTemplates_Record_Model::checkActive($moduleModel->getName())) {
				$basicLinks[] = [
					...
					'linkclass' => 'btn-light js-mass-record-event',
					'linkicon' => 'fas fa-file-pdf',
					'linkhint' => \App\Language::translate('LBL_GENERATE_DOCUMENT')
				];
			}

			// send mail
			if (EmailTemplates_Record_Model::checkActive($moduleModel->getName())) {
				$basicLinks[] = [
					'linktype' => 'LISTVIEWBASIC',
					'linkdata' => ['url' => 'index.php?module=' . $moduleModel->getName() . '&view=DotEmail&fromview=List', 'type' => 'modal'],
					'linkclass' => 'btn-light js-mass-record-event',
					'linkicon' => 'fas fa-envelope',
					'linkhint' => \App\Language::translate('LBL_SEND_MAIL')
				];
			}
		}
		...
		if ($this->isEmpty('viewId')) {
			...
			foreach ($fields as $fieldInfo) {
				...
				if (!empty($fieldInfo['source_field_name'])) {
					if (!\App\CustomView::isSourceFieldActive($fieldInfo['source_field_name'], $this->getModule())) {
						continue;
					}
	...
	public function loadListViewOrderBy()
	{
		...
		if (!empty($orderBy) && \is_array($orderBy)) {
			...
			foreach ($this->getQueryGenerator()->getRelatedFields() as $fieldInfo) {
				$relatedFields[$fieldInfo['relatedModule']][$fieldInfo['sourceField']][] = true;
			}
			foreach ($orderBy as $fieldName => $sortFlag) {
				[$fieldName, $moduleName, $sourceFieldName] = array_pad(explode(':', $fieldName, 3), 3, false);
				if ($sourceFieldName && (isset($fields[$sourceFieldName]) || isset($relatedFields[$moduleName][$sourceFieldName]))) {
	...
	public function loadListViewCondition()
	{
		...
		if ($fixedSearchParams = $this->get('fixed_search_params')) {
			$queryGenerator->parseAdvFilter($fixedSearchParams);
		}
			...
			foreach ($relatedFields as $relatedModuleName => $fields) {
				foreach ($fields as $sourceField => $field) {
					...
					foreach ($field as $relatedFieldName) {
						$recordData[$relatedFieldName] = $row[str_replace(':', '_', $sourceField) . $relatedModuleName . $relatedFieldName];
