...
class Vtiger_Field_Model extends vtlib\Field
{
	...
	public function getFullLabelTranslation(?Vtiger_Module_Model $module = null): string
	{
		...
		if ($this->get('source_field_name')) {
			...
			foreach (explode(':', $this->get('source_field_name')) as $sourceField) {
				$fieldModel = $module->getFieldByName($sourceField);
				$translation .= \App\Language::translate($fieldModel->getFieldLabel(), $module->getName()) . ' - ';
				$module = \Vtiger_Module_Model::getInstance($fieldModel->getReferenceList()[0]);
			}
		}
			...
			if (App\Cache::has('FieldDataType', $cacheName)) {
				...
				switch ($uiType) {
					...
					case 328:
						...
						break;
					case 501:
						$fieldDataType = 'ViewRelatedField';
						break;
					default:
	...
	public function isReadOnly()
	{
		...
		return $this->isReadOnly = !$this->getProfileReadWritePermission() || $this->getFieldDataType() == 'ViewRelatedField';
	}
		...
		if ('picklist' === $fieldDataType || 'multipicklist' === $fieldDataType) {
			...
			if ('picklist' === $fieldDataType) {
				...
				if (!empty($fieldValue) && !isset($fieldPickListValues[$fieldValue])) {
					...
					// 20220613 MK Enable editing of picklists with missing values
	...
	public function isAjaxEditable()
	{
		...
		return !(!$this->getUITypeModel()->isAjaxEditable() || !$this->isEditable() || \in_array($this->get('uitype'), $ajaxRestrictedFields));
	}
	 ...
	 */
	public function getFieldInfo($sourceModule = false)
	{
		...
		switch ($fieldDataType) {
			...
			case 'country':
				...
				if ($picklistValues && $this->isEmptyPicklistOptionAllowed() 
					&& \App\Request::_get('module') === 'Workflows' 
					&& \in_array(\App\Request::_get('type'), ['VTUpdateFieldsTask', 'VTUpdateRelatedFieldTask'])) {
					$picklistValues = [ 'special-clear' => '&nbsp;'] + $picklistValues;
				}
			...
			case 'sharedOwner':
				if (!App\Config::performance('SEARCH_OWNERS_BY_AJAX') || \in_array(\App\Request::_get('module'), ['CustomView', 'Workflows', 'PDF', 'MappedFields']) || 'showAdvancedSearch' === \App\Request::_get('mode')) {
					...
					if (\App\Request::_get('module') === 'Workflows') {
						$roleList = [];
						$allRoles = Settings_Roles_Record_Model::getAll();
						foreach ($allRoles as $roleId => $role) {
							$roleList[$roleId] = $role->getName();
						}
						$pickListValues[\App\Language::translate('LBL_ROLES', 'Settings:Workflows')] = $roleList;
					}

					if ($sourceModule && \App\Request::_get('module') === 'Workflows') {
						$specialList = \VTWorkflowUtils::getOwnerFields($sourceModule, true);

						$prefix = \App\Language::translate('LBL_USER_FROM_ROLE','Settings:Workflows');
						foreach (Settings_Roles_Record_Model::getAll() as $roleId => $roleModel) {
							$specialList["fromRole-$roleId"] = "$prefix " . \App\Language::translate($roleModel->getName(),'Users');
						}

						$specialList["fromUser-current"] = \App\Language::translate('LBL_USER_FROM_CURRENT','Settings:Workflows');

						// prepare list of user groups
						$prefix = \App\Language::translate('LBL_USER_FROM_GROUP', 'Settings:Workflows');
						foreach (\VTWorkflowUtils::getUserGroups() as $userGroup) {
							$specialList["fromUserGroup-$userGroup"] = "$prefix " . \App\Language::translate($userGroup,'UserGroups');
						}

						$pickListValues[\App\Language::translate('LBL_SPECIAL_OPTIONS')] = $specialList;
					}
					$this->fieldInfo['picklistvalues'] = $pickListValues;
	...
	public static function getInstanceFromFilter($fieldInfo)
	{
		if (\is_string($fieldInfo)) {
			$fieldInfo = array_combine(['field_name', 'module_name', 'source_field_name'], array_pad(explode(':', $fieldInfo, 3), 3, false));
		}
