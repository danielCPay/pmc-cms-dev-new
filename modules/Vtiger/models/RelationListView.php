...
class Vtiger_RelationListView_Model extends \App\Base
{
	...
	public function loadCondition()
	{
		...
		if ($this->relatedModuleModel->getName() === 'Cases') {
	...
	public function getLinks(): array
	{
		...
		if ('Documents' === $relationModelInstance->getRelationModuleModel()->getName()) {
			$relatedLink['RELATEDLIST_VIEWS'][] = Vtiger_Link_Model::getInstanceFromValues([
				'linktype' => 'RELATEDLIST_VIEWS',
				'linklabel' => 'LBL_RECORDS_PREVIEW_TREE',
				'view' => 'TreePreview',
				'linkicon' => 'fas fa-desktop',
			]);
		}
		if (!$parentRecordModel->isReadOnly()) {
			...
			if ('Documents' === $relationModelInstance->getRelationModuleModel()->getName()) {
				...
				if (\App\Privilege::isPermitted('OSSMail')) {
					$relatedLink['RELATEDLIST_MASSACTIONS'][] = Vtiger_Link_Model::getInstanceFromValues([
						'linktype' => 'RELATEDLIST_MASSACTIONS',
						'linklabel' => 'LBL_MASS_EMAIL_AS_ATTACHMENT',
						'linkurl' => "javascript:Vtiger_RelatedList_Js.triggerMassEmail('index.php?module=OSSMail&view=Compose&type=new&crmModule={$parentRecordModel->getModuleName()}&crmRecord={$parentRecordModel->getId()}&attachments=')",
						'linkclass' => '',
						'linkicon' => 'fas fa-download'
					]);
				}
				if ($relationModelInstance->getRelationModuleModel()->isPermitted('MassTrash')) {
					$relatedLink['RELATEDLIST_MASSACTIONS'][] = Vtiger_Link_Model::getInstanceFromValues([
						'linktype' => 'RELATEDLIST_MASSACTIONS',
						'linklabel' => 'LBL_MASS_MOVE_TO_TRASH',
						'linkurl' => "javascript:Vtiger_RelatedList_Js.triggerMassState('index.php?module={$parentRecordModel->getModuleName()}&action=RelationAjax&mode=massState&state=Trash&src_record={$parentRecordModel->getId()}&relatedModule=Documents&mode=multiple')",
						'linkclass' => '',
