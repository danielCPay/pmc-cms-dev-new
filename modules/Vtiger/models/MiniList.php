...
class Vtiger_MiniList_Model extends Vtiger_Widget_Model
{
	...
	public function getRecords($user)
	{
		...
		if (!$this->listviewRecords) {
			if (!empty($user)) {
				if ($user === 'minegroups') {
					$groupConditions = array_map(function ($grp) { return ['vtiger_crmentity.smownerid' => $grp]; }, App\User::getCurrentUserModel()->getGroups());
					$conditions = ['or', ['vtiger_crmentity.smownerid' => App\User::getCurrentUserId()], ...$groupConditions];
					$this->queryGenerator->addNativeCondition($conditions);
				} else {
