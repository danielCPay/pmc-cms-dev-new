	...
	public function getPagination(App\Request $request)
	{
		...
		if (App\Config::performance('LISTVIEW_COMPUTE_PAGE_COUNT') || -1 == $totalCount) {
			...
			if (!empty($fixedSearchParams) && \is_array($fixedSearchParams)) {
				$transformedFixedSearchParams = $listViewModel->get('query_generator')->parseBaseSearchParamsToCondition($fixedSearchParams);
				$currentSearchParams = $listViewModel->get('search_params');
				...
				$listViewModel->set('search_params', $currentSearchParams);
			}
