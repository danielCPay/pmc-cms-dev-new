...
class Vtiger_RelatedList_View extends Vtiger_Index_View
{
	...
	public function process(App\Request $request)
	{
		...
		if ($request->isEmpty('relatedView', true)) {
			$relatedView = empty($_SESSION['relatedView'][$moduleName][$relatedModuleName]) ? ($relatedModuleName === 'Documents' ? 'TreePreview' : 'List') : $_SESSION['relatedView'][$moduleName][$relatedModuleName];
		} else {
		...
		$pagingModelTree = new Vtiger_Paging_Model();
		$pagingModelTree->set('limit', 0);
		...
		// MK 20220408 allow also TreePreview
		if (\in_array($relatedView, ['ListPreview', 'TreePreview'])) {
		...
		if ($relatedView === 'TreePreview') {
			$allModels = $relationListView->getEntries($pagingModelTree);
			$cntModels = count($allModels);
			$models = array_slice($allModels, $pagingModel->getStartIndex(), $pagingModel->getPageLimit() + 1);
			if (count($models) > $pagingModel->getPageLimit()) {
				$pagingModel->set('nextPageExists', true);
				array_pop($models);
			} else {
				$pagingModel->set('nextPageExists', false);
			}
			$pagingModel->calculatePageRange($cntModels);
			...
			if (true /* TODO use option/selector from session/UI */)  {
				$allTypes = (new \App\QueryGenerator('DocumentTypes'))->setFields(['parent_document_type', 'document_type', 'id'])->createQuery()->all();

				foreach ($allTypes as $type) {
					$fullTree[] = [
						'id' => $type['id'],
						'parent' => \App\Record::isExists($type['parent_document_type'], 'DocumentTypes') ? $type['parent_document_type'] : '#',
						'text' => '&nbsp;' . $type['document_type'],
						'state' => ['selected' => false, 'disabled' => false, 'loaded' => true, 'opened' => true],
						'icon' => 'yfm-TreeDotsFolder',
					];
				}
			}

			$docIcon = 'yfm-Documents';

			foreach ($allModels as $recordRow) {
				$record = Vtiger_Record_Model::getInstanceById($recordRow->getId());
				if (($exists = \App\Record::isExists($record->get('document_type'), 'DocumentTypes'))) {
					$type = Vtiger_Record_Model::getInstanceById($record->get('document_type'));
					// check structure for types
					$currentType = $type;
					do {
						if (!\in_array($currentType->getId(), $existingTypes)) {
							$existingTypes[] = $currentType->getId();
							$tree[] = [
								'id' => $currentType->getId(),
								'parent' => \App\Record::isExists($currentType->get('parent_document_type'), 'DocumentTypes') && !\in_array($currentType->get('parent_document_type'), $allChildren[$currentType->getId()] ?: []) ? $currentType->get('parent_document_type') : '#',
								'text' => '&nbsp;' . $currentType->get('document_type'),
								'state' => ['selected' => false, 'disabled' => false, 'loaded' => true, 'opened' => true],
								'icon' => 'yfm-TreeDotsFolder',
							];
						}
						if ($currentType->get('parent_document_type')) {
							$allChildren[$currentType->get('parent_document_type')] = array_unique(array_merge($allChildren[$currentType->get('parent_document_type')] ?: [], $allChildren[$currentType->getId()] ?: [], [$currentType->getId()]));
						}
					} while (!\in_array($currentType->get('parent_document_type'), $existingTypes) && $currentType = \App\Record::isExists($currentType->get('parent_document_type'), 'DocumentTypes') ? Vtiger_Record_Model::getInstanceById($currentType->get('parent_document_type')) : null);
				
					$docIcon = 'yfm-TreeDotsDoc';
					$info = $record->getFileDetails();
				
					if(!empty($info['type'])) {
						if(strpos($info['type'], 'zip') != false) {
							$docIcon = 'yfm-TreeDotsZip';
						}
						else if(strpos($info['type'], 'pdf') != false) {
							$docIcon = 'yfm-TreeDotsPdf';
						}
						else if(strpos($info['type'], 'jpg') != false || strpos($info['type'], 'jpeg') != false) {
							$docIcon = 'yfm-TreeDotsJpg';
						}
						else if(strpos($info['type'], 'text') != false) {
							$docIcon = 'yfm-TreeDotsTxt';
						}
					}
				}
				...
				$record = [
					'id' => $record->getId(),
					'parent' => $exists && \in_array($record->get('document_type'), $existingTypes) ? $record->get('document_type') : '#',
					'text' => '&nbsp;' . $record->get('notes_title'),
					'state' => ['selected' => false, 'disabled' => false, 'loaded' => 'true', 'opened' => true],
					'icon' => $docIcon,
					'li_attr' => [ 'data-recordurl' => $record->getDetailViewUrl() ],
				];
				$tree[] = $record;
				$fullTree[] = $record;
			}

			$viewer->assign('TREE', $tree);
			$viewer->assign('FULL_TREE', $fullTree);
			$viewer->assign('FULL_TREE_SELECTED', $_COOKIE['switch-tree'] === 'false');
		} else {
