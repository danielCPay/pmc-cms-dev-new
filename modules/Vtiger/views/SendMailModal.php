...
class Vtiger_SendMailModal_View extends Vtiger_BasicModal_View
{
	...
	public function getQuery(App\Request $request)
	{
		...
		$fixedSearchParams = App\Condition::validSearchParams($listView->getQueryGenerator()->getModule(), $request->getArray('fixed_search_params'));
		if (!empty($fixedSearchParams) && \is_array($fixedSearchParams)) {
			$transformedFixedSearchParams = $listView->getQueryGenerator()->parseBaseSearchParamsToCondition($fixedSearchParams);
			...
			$listView->set('search_params', $currentSearchParams);
		}
		if ($sourceModule) {
