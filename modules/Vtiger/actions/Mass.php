...
abstract class Vtiger_Mass_Action extends \App\Controller\Action
{
	...
	public static function getQuery(App\Request $request)
	{
		...
		$customViewModel->set('search_params', array_merge(App\Condition::validSearchParams($moduleName, $request->getArray('search_params')), App\Condition::validSearchParams($moduleName, $request->getArray('fixed_search_params'))));
		$customViewModel->set('entityState', !$request->isEmpty('entityState') ? $request->getByType('entityState') : 'Active');
