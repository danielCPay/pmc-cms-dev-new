...
class Vtiger_SaveAjax_Action extends Vtiger_Save_Action
{
	...
	public function process(App\Request $request)
	{
		...
		$oldValues = $this->record->getData();
		$this->saveRecord($request);
		\App\Cache::staticDelete('RecordModel', $this->record->getId() . ':' . $this->record->getModuleName());
		$newValues = Vtiger_Record_Model::getInstanceById($this->record->getId())->getData();
		$fieldModelList = $this->record->getModule()->getFields();
		$result = [];
		foreach ($fieldModelList as $fieldName => $fieldModel) {
			if (!$fieldModel->isViewable() || $fieldName === 'modifiedtime') {
				continue;
			}
			$recordFieldValue = $newValues[$fieldName];
			$recordFieldValuePrev = $oldValues[$fieldName];
			$prevDisplayValue = $fieldModel->getDisplayValue($recordFieldValuePrev, $this->record->getId(), $this->record);
			$changed = false;
			if ($recordFieldValuePrev !== $recordFieldValue) {
				$changed = true;
			}
			$result[$fieldName] = [
				'success' => $changed,
				'label' => $fieldModel->getFullLabelTranslation(),
				...
				'prev_value' => $recordFieldValuePrev
			];
		}
		...
		$result['_workflowTrigger'] = count(Vtiger_WorkflowTrigger_Model::getTreeWorkflows($this->record->getModuleName(), $this->record->getId()));
