	...
	public function generate(App\Request $request)
	{
		...
		foreach ($recordIds as $recordId) {
			foreach ($templateIds as $templateId) {
				...
				$documentType = $template->get('document_type');
				...
				if (!$singlePdf && ($attach || $emailPdf || ($countTemplates > 1 || (1 === $countTemplates && !isset($skip[$templateId]) && $countRecords > 1)))) {
					...
					$pdfFiles[] = 
						[
							...
							'documentType' => $documentType, 'documentArea' => $documentArea, 
							'fromTemplate' => $template->get('primary_name'),
							'recordId' => $recordId
						];
				...
				if ($singlePdf) {
					...
					if ($saveFlag != 'F') {
						$filePath1 = 'cache' . DIRECTORY_SEPARATOR . 'pdf' . DIRECTORY_SEPARATOR;
						$tmpFileName = tempnam($filePath1, 'PDF' . time());
						$filePath1 .= basename($tmpFileName);
						$pdf->output($filePath1, 'F');

						$pdfFiles[] = 
							[
								'path' => $filePath1, 'name' => $pdf->getFileName(). '.pdf',
								'documentType' => $documentType, 'documentArea' => $documentArea, 
								'fromTemplate' => $template->get('primary_name'),
								'recordId' => $recordId
							];
		...
		if ($singlePdf) {
			...
			$pdfFiles[] = ['path' => $filePath, 'name' => \App\Language::translate('LBL_PDF_MANY_IN_ONE'), 'documentType' => 'Other', 'documentArea' => 'Other', 'fromTemplate' => '', 'recordId' => count($recordIds) === 1 ? $recordId : false];
		} elseif ($emailPdf) {
			...
			foreach ($pdfFiles as &$pdfFile) {
				$tmpFileName = 'cache' . \DIRECTORY_SEPARATOR . 'pdf' . \DIRECTORY_SEPARATOR;
				$tmpFileName = $tmpFileName . basename(tempnam($tmpFileName, 'Attach' . time()));
				if (\copy($pdfFile['path'], $tmpFileName)) {
					$pdfFile['path'] = $tmpFileName;
				}
			}
		...
		foreach ($pdfFiles as $pdfFile) {
			...
			$documentArea = $pdfFile['documentArea'];
			$documentType = $pdfFile['documentType'];
			$fromTemplate = $pdfFile['fromTemplate'];
			$recordId = $pdfFile['recordId'];
			try {
				// create document
				$file = \App\Fields\File::loadFromPath($filePath);
				$file->name = $fileName;
				['crmid' => $fileId, 'attachmentsId' => $attachmentId] = \App\Fields\File::saveFromContent($file, [
					'document_area' => $documentArea,
					'document_type' => $documentType,
					'source' => 'Other',
					'generate_from_template' => $fromTemplate,
					'access_through_cp' => 'Not visible',
					'claim' => $moduleName == 'Claims' ? $recordId : null,
					'claim_opportunity' => $moduleName == 'ClaimOpportunities' ? $recordId : null,
					'agreement' => in_array($moduleName, ['ClaimPurchaseAgreements', 'ClaimLegalServicesAgr', 'ClaimLSFundingAgreements']) == 'Claims' ? $recordId : null,
				]);
