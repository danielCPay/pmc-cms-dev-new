	...
	protected function getBaseModuleRecordIds(App\Request $request)
	{
		...
		if ('all' == $selectedIds[0]) {
			...
			if ($customViewModel) {
				...
				$customViewModel->set('search_params', array_merge(App\Condition::validSearchParams($module, $request->getArray('search_params')), App\Condition::validSearchParams($module, $request->getArray('fixed_search_params'))));
