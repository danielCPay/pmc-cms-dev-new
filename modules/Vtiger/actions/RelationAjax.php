...
class Vtiger_RelationAjax_Action extends \App\Controller\Action
{
	...
	public function addRelation(App\Request $request)
	{
		...
		if ($relatedModule === 'OSSMailView') {
			$relationModel = new OSSMailView_Relation_Model();
		}
		else if ($request->isEmpty('relationId')) {
	...
	public function calculate(App\Request $request)
	{
		...
		$tableName = $fieldQueryModel->getTableName();
		$queryGenerator->addTableToQuery($tableName);
		$columnName = $fieldQueryModel->getField()->getColumnName();
	 ...
	 * @param App\Request $request
	 */
	public function massState(App\Request $request)
	{
		$relatedModuleName = $request->getByType('relatedModule', 2);
		$state = $request->getByType('state');
		$records = $this->getRecordsListFromRequest($request);
		...
		$type = 'success';
		$response = new Vtiger_Response();
		$response->setResult(['notify' => ['text' => $text, 'type' => $type], 'reloadList' => true]);
		$response->emit();
	}
