...
class CustomView_Record_Model extends \App\Base
{
	...
	public function save()
	{
		...
		// MK 20220831 Save directly as public, request of AK and PMC
		// if (App\CustomView::CV_STATUS_PENDING == $status && $currentUserModel->isAdminUser()) {
		if (App\CustomView::CV_STATUS_PENDING == $status) {
	...
	{
		[$fieldName, $fieldModuleName, $sourceFieldName] = array_pad(explode(':', $rule['fieldname'], 3), 3, false);
		$operator = $rule['operator'];
		$value = $rule['value'] ?? '';
		if (!$this->get('advfilterlistDbFormat') && !\in_array($operator, App\Condition::OPERATORS_WITHOUT_VALUES + array_keys(App\Condition::DATE_OPERATORS)) && $fieldName != 'special-current-role') {
	...
	public function setColumnlist()
	{
		...
		foreach ($this->get('columnslist') as $index => $columnInfo) {
			$columnInfo = explode(':', $columnInfo, 3);
