...
class OSSMailScanner_Record_Model extends Vtiger_Record_Model
{
	...
	{
		$accountsPriority = ['CreatedEmail', 'CreatedHelpDesk', 'BindAccounts', 'BindContacts', 'BindCustomers', 'BindLeads', 'BindHelpDesk', 'BindSSalesProcesses'];
	...
	public function manualScanMail(int $uid, string $folder, array $account)
	{
		...
		if (empty($account['actions'])) {
			$params['actions'] = ['CreatedEmail', 'BindAccounts', 'BindContacts', 'BindCustomers', 'BindLeads'];
		}
	...
	public static function mailScan($mbox, array $account, string $folder, int $scan_id, int $countEmails)
	{
		...
		if (OSSMail_Record_Model::isOauth()) {
			if ($lastScanUid) {
				$lastScanUid++;
				\App\Log::beginProfile(__METHOD__ . '|last_mail', 'Mail|IMAP');
				$allMails = $mbox->overview("$lastScanUid:*");
				\App\Log::endProfile(__METHOD__ . '|last_mail', 'Mail|IMAP');
			} else {
				$allMails = $mbox->overview("1:*");
			}
			$uids = array_keys($allMails);
			if (count($uids)) {
				$dbCommand = \App\Db::getInstance()->createCommand();
				foreach ($uids as $uid) {
					try
	...
	{
		\App\Log::warning("OSSMailScanner_Record_Model::executeCron($whoTrigger)");
		...
		/** @var OSSMailScanner_Record_Model $scannerModel */
		$scannerModel = Vtiger_Record_Model::getCleanInstance('OSSMailScanner');
		$countEmails = 0;
		$scanId = $scannerModel->addScanHistory(['user' => $whoTrigger]);
		foreach ($accounts as $account) {
			\App\Log::warning("OSSMailScanner_Record_Model::executeCron($whoTrigger) - {$account['username']}");
			\App\Log::trace('Start checking account: ' . $account['username']);
			if (empty($scannerModel->getFolders($account['user_id'])) || !$this->isConnection($account)) {
				continue;
			}
			foreach ($scannerModel->getFolders($account['user_id']) as $folderRow) {
				...
				\App\Log::warning("OSSMailScanner_Record_Model::executeCron($whoTrigger) - folder $folder");
				...
				if ($mbox && (\is_resource($mbox) || $mbox->getClient()->isConnected())) {
	...
	public function isConnection(array $account)
	{
		...
		try {
			...
			\App\Log::error("OSSMailScanner_Record_Model::isConnection(" . print_r($account, true) . ") - " . print_r($e, true));
			$result = false;
		}
		return $result;
	}
	...
	public function getScanHistory($startNumber = 0)
	{
		...
		while ($row = $dataReader->read()) {
			...
			$output[] = [
				...
				'status' => $this->getHistoryStatus($row['status']),
				'user' => $row['user'],
