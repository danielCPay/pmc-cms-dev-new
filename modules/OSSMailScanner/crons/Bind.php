...
class OSSMailScanner_Bind_Cron extends \App\CronHandler
{
	...
	public function process()
	{
		...
		while ($row = $dataReader->read()) {
			\App\Log::warning("OSSMailScanner_Bind_Cron::process:binding {$row['ossmailviewid']}");
		...
		while ($relationRow = $dataReader->read()) {
			\App\Log::warning("OSSMailScanner_Bind_Cron::process:relation update {$relationRow['tabid']}/{$relationRow['crmid']}");
			...
			if (!empty($where) && $where !== ['or']) {
				\App\Log::warning("OSSMailScanner_Bind_Cron::process:rebinding " . print_r($where, true));
				...
				while ($row = $dataReaderMail->read()) {
					\App\Log::warning("OSSMailScanner_Bind_Cron::process:rebinding {$row['ossmailviewid']}");
					$scannerModel->bindMail($row);
				}
