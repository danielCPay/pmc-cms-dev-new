...
class OSSMailScanner_CreatedEmail_ScannerAction
{
	...
	public function process(OSSMail_Mail_Model $mail)
	{
		...
		if (false === $mail->getMailCrmId()) {
			...
			$content = $this->parseContent($mail, false);
			...
			// check from and to user, add to shownerid (unique)
			$shareUsers = array_unique(array_filter([
				Users_Module_Model::getUserIdByEmail($mail->get('from_email')),
				Users_Module_Model::getUserIdByEmail($mail->get('to_email'))
			]));
			if (!empty($shareUsers)) {
				$record->set('shownerid', $shareUsers);
			}
	 ...
	 * @return string
	 */
	public function parseContent(OSSMail_Mail_Model $mail, $saveEmbeds = true)
	{
		$html = $mail->get('body');
		...
		} else /* skip embeds and small images */ {
			foreach ($doc->getElementsByTagName('img') as $img) {
				$src = trim($img->getAttribute('src'), '\'');
				if ('data:' === substr($src, 0, 5)) {
					continue;
