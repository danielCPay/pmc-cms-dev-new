...
/**
 * Class ColoradoCases_Record_Model.
 */
class ColoradoCases_Record_Model extends Vtiger_Record_Model
{
  /**
  ...
  public function recalculateAll() {
    ...
    \App\Log::warning("ColoradoCases::recalculateAll:$id/$lockAutomation");

    if (!$lockAutomation) {
      \App\Log::warning("ColoradoCases::recalculateAll:recalculateMethods = [" . implode(', ', $this->recalculateFunctions) . "]");
  ...
  public function recalculateFromClaims() {
    $id = $this->getId();
    $lockAutomation = $this->get('lock_automation');
    \App\Log::warning("ColoradoCases::recalculateFromClaims:$id/$lockAutomation");
    ...
    if (!$lockAutomation) {
      ...
      \App\Log::warning("ColoradoCases::recalculateFromClaims:user is $currentUserId, should be $targetUserId");
      if ($targetUserId != $currentUserId) {
        \App\Log::warning("ColoradoCases::recalculateFromClaims:setting user to $targetUserId");
      ...
      if ($targetUserId != $currentUserId) {
        \App\Log::warning("ColoradoCases::recalculateFromClaims:resetting user to $currentUserId");
        \App\User::setCurrentUserId($currentUserId);
        if ($currentBaseUserId) {
          \App\Log::warning("ColoradoCases::recalculateFromClaims:resetting base user to $currentBaseUserId");
  ...
  public function recalculateFromCollections() {
    $id = $this->getId();
    $lockAutomation = $this->get('lock_automation');
    \App\Log::warning("ColoradoCases::recalculateFromCollections:$id/$lockAutomation");
  ...
  public function recalculateFromOthers() {
    $id = $this->getId();
    $lockAutomation = $this->get('lock_automation');
    \App\Log::warning("ColoradoCases::recalculateFromOthers:$id/$lockAutomation");
  ...
  public function recalculateSettlementNegotiations() {
    $id = $this->getId();
    $lockAutomation = $this->get('lock_automation');
    \App\Log::warning("ColoradoCases::recalculateSettlementNegotiations:$id/$lockAutomation");
  ...
  public function calculatePMCCollectionsLimit(?int $providerId = null) {
    ...
    \App\Log::warning("ColoradoCases::calculatePMCCollectionsLimit:$id/$providerId"); 
  ...
  public function recalculateFromCase() {
    $id = $this->getId();
    $lockAutomation = $this->get('lock_automation');
    \App\Log::warning("ColoradoCases::recalculateFromCase:$id/$lockAutomation");
  ...
  public function findSimilarCases() {
    $id = $this->getId();
		...
		\App\Log::warning("ColoradoCases::findSimilarCases:$id");

    // get cases
    $queryGenerator = (new \App\QueryGenerator('ColoradoCases'));
    ...
    if (!$anyCondition) {
      $allCases = [];
      \App\Log::warning("ColoradoCases::findSimilarCases:no conditions");
    } else {
      ...
      \App\Log::warning("ColoradoCases::findSimilarCases:all cases = " . count($allCases) . ", query = " . $query->createCommand()->getRawSql());
    }
    ...
    if (count($allCases) > 1) {
      ...
      $similarCasesId = (new \App\QueryGenerator('SimilarColoradoCases'))
        ->setField('id')
      ...
      if (empty($similarCasesId)) {
        $similarCases = Vtiger_Record_Model::getCleanInstance('SimilarColoradoCases');
        ...
        foreach (VTWorkflowUtils::getAllRelatedRecords($similarCases, 'ColoradoCases') as $caseRow) {
      ...
      foreach (array_unique($similarCasesToCheck) as $similarCaseToCheckId) {
        ...
        $cases = VTWorkflowUtils::getAllRelatedRecords($similarCaseToCheck, 'ColoradoCases');
        if (count($cases) <= 1) {
