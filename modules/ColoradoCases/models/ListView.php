...
/**
 * ColoradoCases ListView Model Class.
 */
class ColoradoCases_ListView_Model extends Vtiger_ListView_Model
{
    ...
    if (\App\Cache::has('ColoradoCases', $key)) {
      return \App\Cache::get('ColoradoCases', $key);
    }
		...
		\App\Cache::save('ColoradoCases', $key, $providers);
	...
	public function loadListViewCondition()
	{
		...
		if ($userModel->getRole() === ColoradoCases_ListView_Model::PROVIDER_LITIGATION_ASSISTANCE_ROLE) {
			$providerFields = ColoradoCases_MergeRecordsSpecial_View::getProviderFields();
			$providers = ColoradoCases_ListView_Model::getProviderLitigationAssists($userModel->getId());
