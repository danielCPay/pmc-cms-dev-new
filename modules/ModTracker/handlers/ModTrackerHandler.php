...
class ModTracker_ModTrackerHandler_Handler
{
	...
	public function entityAfterSave(App\EventHandler $eventHandler)
	{
		...
		if ($status !== ModTracker::$FILEDAV_CHANGED) {
			foreach ($delta as $fieldName => $preValue) {
				$newValue = $recordModel->get($fieldName);
				$fieldModel = $recordModel->getField($fieldName);
				if ($preValue === $newValue || !$fieldModel || ($isNew && empty($newValue) && ($fieldModel->isReferenceField() || $newValue !== 0)) || ($fieldModel->isReferenceField() && empty($newValue) && empty($preValue))) {
					unset($delta[$fieldName]);
				}
			}
		} else if (empty($delta['filesize'])) {
			$delta['filesize'] = $recordModel->get('filesize');
		}
		if (empty($delta)) {
			return false;
		}
		...
		$db->createCommand()->insert('vtiger_modtracker_basic', [
			...
			'changedon' => $recordModel->get('modifiedtime') ?: date('Y-m-d H:i:s'),
			'status' => $status,
		...
		foreach ($delta as $fieldName => $preValue) {
			...
			$fieldModel = $recordModel->getField($fieldName);
			if ($status !== ModTracker::$FILEDAV_CHANGED) {
				if ($preValue === $newValue || ($isNew && empty($newValue) && ($fieldModel->isReferenceField() || $newValue !== 0)) || ($fieldModel->isReferenceField() && empty($newValue) && empty($preValue))) {
					continue;
				}
			}
