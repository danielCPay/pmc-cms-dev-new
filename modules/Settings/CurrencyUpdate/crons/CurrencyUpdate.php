...
class Settings_CurrencyUpdate_CurrencyUpdate_Cron extends \App\CronHandler
{
	...
	public function process()
	{
		...
		if (!empty($activeBankId)) {
			...
			if ($status) {
				\App\Log::warning('Successfully fetched new currency exchange rates for date: ' . $lastWorkingDay . ' from bank: ' . $moduleModel->getActiveBankName());
			} else {
				\App\Log::error('Failed to fetch new currency exchange rates for date: ' . $lastWorkingDay . ' from bank: ' . $moduleModel->getActiveBankName());
			}
		} else {
			\App\Log::warning('Update of system currency rates ignored, no active banking settings.');
		}
