...
class Settings_Workflows_Record_Model extends Settings_Vtiger_Record_Model
{
	...
	public function getParentModules()
	{
		$dependentFields = [];
		$filterModules = ['Calendar', 'Accounts', 'Notification'];
		foreach (\App\Field::getRelatedFieldForModule($this->getModule()->getName(), false) as $module => $value) {
			if (\in_array($module, $filterModules)) {
				continue;
			}
			$dependentFields[$module] = ['fieldname' => $value['fieldname'], 'modulelabel' => \App\Language::translate($module, $module)];
	...
	public function getRelatedFields($relatedModule) {
		$fields = [];
		...
		if ($parentSpecifier === 'CURRENT') {
			$fields[] = [ 'id' => 'special-current', 'name' => \App\Language::translate('LBL_CURRENT') ];
		} else if ($parentSpecifier === 'PARENT') {
			$fieldModels = $moduleModel->getReferenceFieldsForModule($relatedModuleName);
			foreach($fieldModels as $name => $model) {
				$fields[] = [ 'id' => $name, 'name' => $name ];
			}
		} else {
			$relatedModuleModel = Vtiger_Module_Model::getInstance($relatedModuleName);
			$fieldModels = $relatedModuleModel->getReferenceFieldsForModule($moduleModel->getName());
			foreach($fieldModels as $name => $model) {
				$fields[] = [ 'id' => $name, 'name' => $name ];
			}
		}
