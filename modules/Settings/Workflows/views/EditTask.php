...
class Settings_Workflows_EditTask_View extends Settings_Vtiger_Index_View
{
	public function process(App\Request $request)
	{
		...
		/** @var Settings_Workflows_TaskRecord_Model $taskModel */
				...
				foreach ($fieldMapping as $key => $mappingInfo) {
					if (\array_key_exists($mappingInfo['fieldname'], $ownerFieldModels)) {
						if ('assigned_user_id' == $mappingInfo['value']) {
							...
							$fieldMapping[$key]['valuetype'] = 'rawtext';
						} elseif(strpos($mappingInfo['value'], 'fromUserGroup') === 0) {
							$fieldMapping[$key]['valuetype'] = 'rawtext';
		...
		if ('VTEmailTemplateTask' === $taskType) {
			...
			$groups = $query->createCommand()->queryAllByGroup(0);
			$viewer->assign('GROUPS_EMAIL', $groups);
		}
		...
		if ($taskObject->workflowModule && !str_starts_with($taskObject->workflowModule, 'CURRENT')) {
			[, $relatedModuleName] = explode('||', $taskObject->workflowModule);
			$viewer->assign('OTHER_WORKFLOWS', (new VTWorkflowManager())->getWorkflowsForModule($relatedModuleName));
			$viewer->assign('OTHER_FIELDS', $workflowModel->getRelatedFields($taskObject->workflowModule));
		} else {
			$viewer->assign('OTHER_WORKFLOWS', (new VTWorkflowManager())->getWorkflowsForModule($moduleModel->getName()));
		}
