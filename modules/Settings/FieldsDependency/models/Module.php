...
class Settings_FieldsDependency_Module_Model extends Settings_Vtiger_Module_Model
{
	 ...
	 * @return mixed
	 */
	public static function isFieldUsed(int $moduleId, string $fieldName)
	{
		$row = (new \App\Db\Query())->from('s_#__fields_dependency')->where(['and', ['tabid' => $moduleId], ['or', ['like', 'fields', '"' . $fieldName . '"'], ['like', 'conditionsFields', '"' . $fieldName . '"']]])->one();
		$result = $row ? $row['name'] : false;
		return $result;
	}
}
