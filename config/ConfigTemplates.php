...
return [
	'api' => [
		...
		'azureAddressKey' => [
			'description' => 'JWT SAS key for Azure Address service'
		],
		'PROVIDER_PORTAL_API_URL' => [
			'description' => 'URL of Provider Portal API'
		],
		'LAW_FIRM_PORTAL_API_URL' => [
			'description' => 'URL of Law Firm Portal API'
		],
		'FILE_DAV_KEY' => [
			'default' => 'this-is-a-file-dav-key-please-change-it',
			'description' => 'FileDAV key',
		],
		'MONDAY_COM_AUTH_TOKEN' => [
			'description' => 'Monday.com auth token',
		],
		'HIGGSHUB_PRO_AUTH_TOKEN' => [
			'description' => 'HiggsHub.pro auth token',
		],
	],
	'main' => [
		'USE_RTE' => [
		...
		'listMaxEntriesMassEdit' => [
			...
			'validation' => function () {
				$arg = func_get_arg(0);
				return $arg && \App\Validator::naturalNumber($arg) && (100000 >= $arg);
			},
			'sanitization' => function () {
				return (int) func_get_arg(0);
			}
		...
		'maxExecutionMailerCronTime' => [
			'default' => 600,
			'description' => 'The maximum time of executing a mailer cron. Recommended lower than max_exacution_time parameter value.'
		],
		'langInLoginView' => [
			'default' => false,
	...
	'debug' => [
		...
		'apiLogAllRequests' => [
			'default' => false,
			'description' => '[WebServices/API] Log to file all communications data (request)'
		],
		'apiLogAllResponses' => [
			'default' => false,
			'description' => '[WebServices/API] Log to file all communications data (response)'
		],
	...
	'performance' => [
		...
		'INTERVAL_FOR_TOASTS_CHECK' => [
			'default' => 30,
			'description' => 'Time to check for new toasts in seconds',
			'validation' => '\App\Validator::naturalNumber'
		],
		...
		'CRON_BATCH_SIZE_SENDING_MAILS' => [
			'default' => 10,
			'description' => 'The number of emails that cron can send during one loop.',
			'validation' => '\App\Validator::naturalNumber'
		],
		...
		'MAX_REFERENCES_DEPTH' => [
			'default' => 3,
			'description' => 'Max depth for "deep" references',
			'validation' => '\App\Validator::naturalNumber',
		],
	],
	'relation' => [
	...
	'quickbooks' => [
		...
		'realmId' => [
			'default' => '',
			'description' => 'Realm Id - company identifier',
			'validation' => function () {
				return true;
			}
		],
		...
		'refreshToken' => [
			'default' => '',
			'description' => 'Refresh token to request access tokens',
			'validation' => function () {
				return true;
			}
		],
	...
	'docusign' => [
		'enabled' => [
			'default' => false,
			'description' => 'Is interface enabled?'
		],
		'clientId' => [
			'default' => '',
		...
		'clientSecret' => [
			'default' => '',
			'description' => 'Integration key secret'
		],
		'authorizationServer' => [
			'default' => '',
		...
		'allowSilentAuthentication' => [
			'default' => true,
			'description' => 'Can user be silently authenticated if they have an active login session on another tab of the same browser'
		],
		'impersonatedUserId' => [
			'default' => '',
			'description' => 'Id of user being impersonated'
		],
		'privateKey' => [
			'default' => '',
	...
	'datadump' => [
		'outVolume' => [
			'description' => 'Path to directory to dump files to',
		],
		'outDir' => [
			'description' => 'Path to optional subdirectory to dump files to',
		],
		'siteUrl' => [
			'description' => 'Site URL to use in .meta files',
			'validation' => '\App\Validator::url',
		],
		'dumpPort' => [
			'description' => 'Port to use in .meta files',
		],
		'tables' => [
			'description' => 'Array of tables and views to export',
			'validation' => function () {
				$arg = func_get_arg(0);
				return \is_array($arg);
			}
	...
	'dropbox' => [
		...
		'refreshTokens' => [
			'default' => '',
			'description' => 'Refresh tokens for various clients, object client_name => refresh_token',
			'validation' => function () {
				return true;
			}
		],
		'defaultAccount' => [
			'default' => '',
			'description' => 'Account to use if not specified'
		],
	],
	'gcal' => [
		'credentialsFile' => [
			'default' => './config/keys/gcal.json',
			'description' => 'Credentials file path'
		],
		'applicationName' => [
			'default' => 'PMC GCal Integration',
			'description' => 'Application Name'
		],
		'impersonateSubject' => [
			'description' => 'Subject - user to impersonate'
		],
		'pageSize' => [
			'default' => 100,
			'description' => 'Number of events to return on single page'
		],
		'syncToken' => [
		...
		'fullSyncLowerBound' => [
			'default' => '1 year ago',
			'description' => 'strtotime compatible lower bound for full sync'
		],
	],
	'gdrive' => [
		'credentialsFile' => [
		...
		'applicationName' => [
			'default' => 'PMC GDrive Integration',
			'description' => 'Application Name'
		],
		'uploadFolder' => [
			'default' => '',
			'description' => 'Target path to save files to be shared'
		],
